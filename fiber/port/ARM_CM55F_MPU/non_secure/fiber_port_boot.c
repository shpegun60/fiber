...
/* Slice 1 has no mandatory runtime object yet. When the protected runtime is
 * activated, move this definition into that always-linked object and retain it
 * here instead, matching the frozen whole-port cohort ownership contract. */
...
void fiber_port_fpu_require_ready(void)
{
	...
	FIBER_REQUIRE((fiber_portFPCCR_REG & fiber_portFPCCR_LSPACT_BIT) == 0u,
			'E');
}
...
int fiber_port_mpu_rlar_attributes_are_valid(uint32_t attributes)
{
	...
	const uint32_t attribute_index =
			attributes & fiber_portMPU_RLAR_ATTR_INDEX_MASK;

	return ((attributes & ~allowed) == 0u) &&
			((attribute_index == fiber_portMPU_RLAR_ATTR_INDEX0) ||
		...
		const FiberPortMpuMemoryLayout *layout)
{
	 ...
	 * both privilege levels because ARMv8.1-M has no privileged-RW plus
	...
	/* ARMv8.1-M region priority makes overlapping global mappings ambiguous for
	...
	const uintptr_t fpu_prepare = fiber_port_function_address(
			(uintptr_t)&fiber_port_fpu_prepare);
	const uintptr_t fpu_configured = fiber_port_function_address(
			(uintptr_t)&fiber_port_fpu_require_configured);
	const uintptr_t fpu_ready = fiber_port_function_address(
			(uintptr_t)&fiber_port_fpu_require_ready);
...
void fiber_port_context_validate_initial_restore(const FiberContext *ctx)
{
	...
	/* The basic view is intentionally only 21 words.  The remaining words are
	 * available for a later extended-FP save and must start deterministic. */
	for (uint32_t index = fiber_portPROTECTED_BASIC_RESTORE_WORDS + 1u;
			index < fiber_portPROTECTED_CONTEXT_WORDS; ++index) {
		FIBER_REQUIRE(ctx->protected_context.words[index] == 0u, 'F');
	}
		...
		void *arg)
{
	...
	FiberPortProtectedBasicContext *const image = &ctx->protected_context.basic;
	image->r4 = 0u;
	image->r5 = 0u;
	image->r6 = 0u;
	image->r7 = 0u;
	...
	image->r10 = 0u;
	image->r11 = 0u;
	image->r0 = (uint32_t)(uintptr_t)arg;
	image->r1 = 0u;
	image->r2 = 0u;
	image->r3 = 0u;
	image->r12 = 0u;
	image->lr = return_address;
	image->pc =
			(uint32_t)(entry_address & (uintptr_t)fiber_portSTART_ADDRESS_MASK);
	image->xpsr = fiber_portINITIAL_XPSR;
	image->psp = (uint32_t)initial_psp;
	image->psplim = (uint32_t)stack_base;
	image->control =
			fiber_portINITIAL_CONTROL_UNPRIVILEGED;
	image->exc_return = fiber_portINITIAL_EXC_RETURN;
	image->cursor_limit = 0u;
	ctx->protected_context_cursor = &image->cursor_limit;
