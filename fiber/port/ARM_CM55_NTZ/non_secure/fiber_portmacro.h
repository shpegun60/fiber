 ...
 * fiber_portmacro.h
 *
 * Exact Cortex-M55 NTZ Non-secure dictionary.
 ...
 */
#ifndef FIBER_PORT_ARM_CM55_NTZ_FIBER_PORTMACRO_H_
#define FIBER_PORT_ARM_CM55_NTZ_FIBER_PORTMACRO_H_

#include <stddef.h>
#include <stdint.h>

#ifdef FIBER_PORT_NAME
# error "[fiber]: ARM_CM55_NTZ port name must not be predefined"
#endif
#define FIBER_PORT_NAME "ARM_CM55_NTZ"
...
# error "[fiber]: ARM_CM55_NTZ is build-selected only"
#endif

#if !FIBER_PORT_ARMV81M_MAINLINE
# error "[fiber]: ARM_CM55_NTZ requires the ARMv8.1-M Mainline architecture result"
#endif

/* GCC's Cortex-M55 soft-float target reports __ARM_ARCH_8M_MAIN__ rather
 * than __ARM_ARCH_8_1M_MAIN__. CMSIS core identity closes that distinction. */
#if !defined(__ARM_ARCH_8M_MAIN__) && !defined(__ARM_ARCH_8_1M_MAIN__)
# error "[fiber]: ARM_CM55_NTZ requires an ARMv8.1-M Mainline compiler target"
#endif

#if !defined(__CORTEX_M) || (__CORTEX_M != 55)
# error "[fiber]: ARM_CM55_NTZ manifest requires CMSIS __CORTEX_M == 55"
#endif

#if defined(__ARM_FEATURE_CMSE) && ((__ARM_FEATURE_CMSE + 0) >= 3)
# error "[fiber]: ARM_CM55_NTZ does not accept a Secure CMSE build"
#endif

#if !defined(__VTOR_PRESENT) || (__VTOR_PRESENT != 1)
# error "[fiber]: ARM_CM55_NTZ requires __VTOR_PRESENT == 1"
#endif

#if defined(__FPU_PRESENT) && ((__FPU_PRESENT + 0) != 0) && \
		((__FPU_PRESENT + 0) != 1)
# error "[fiber]: ARM_CM55_NTZ CMSIS __FPU_PRESENT must be 0 or 1"
#endif

/* This is the no-FPU ABI. A future M55F profile owns FP context separately. */
#if defined(__FPU_USED) && ((__FPU_USED + 0) != 0)
# error "[fiber]: ARM_CM55_NTZ requires __FPU_USED == 0"
#endif

#if defined(__ARM_FP) && ((__ARM_FP + 0) != 0)
# error "[fiber]: ARM_CM55_NTZ does not permit an FP register ABI"
#endif

#if defined(__VFP_FP__) && !defined(__SOFTFP__)
# error "[fiber]: ARM_CM55_NTZ does not permit a hard-FP compiler ABI"
#endif

#if defined(__ARM_FEATURE_MVE) && ((__ARM_FEATURE_MVE + 0) != 0)
# error "[fiber]: ARM_CM55_NTZ does not permit MVE"
#endif

#if defined(__ARM_FEATURE_PAC_DEFAULT) || defined(__ARM_FEATURE_PAUTH) || \
		defined(__ARM_FEATURE_PAUTH_DEFAULT)
# error "[fiber]: ARM_CM55_NTZ does not permit PAC"
#endif

#if defined(__ARM_FEATURE_BTI_DEFAULT) || defined(__ARM_FEATURE_BTI)
# error "[fiber]: ARM_CM55_NTZ does not permit BTI"
#endif

#if defined(FIBER_PORT_CM55_MPU_TOTAL_REGIONS) || \
		defined(FIBER_PORT_MPU_TOTAL_REGIONS)
# error "[fiber]: ARM_CM55_NTZ is non-MPU; select a separate MPU profile"
#endif

#if !defined(__NVIC_PRIO_BITS) || (__NVIC_PRIO_BITS < 1) || \
		(__NVIC_PRIO_BITS > 8)
# error "[fiber]: ARM_CM55_NTZ requires 1..8 implemented NVIC priority bits"
#endif

#ifdef FIBER_PORT_RUNTIME_SELECTABLE
# error "[fiber]: ARM_CM55_NTZ runtime-selectable state must not be predefined"
#endif
#define FIBER_PORT_RUNTIME_SELECTABLE 1

/* Pinned FreeRTOS ARM_CM55_NTZ non-MPU, no-FPU/no-MVE frame dictionary. */
...
# error "[fiber]: ARM_CM55_NTZ scheduler BASEPRI threshold must be non-zero"
#endif

#if FIBER_PORT_SCHEDULER_BASEPRI > 255u
# error "[fiber]: ARM_CM55_NTZ scheduler BASEPRI threshold must fit in 8 bits"
#endif

FIBER_STATIC_ASSERT((FIBER_PORT_SCHEDULER_BASEPRI &
		((1u << (8u - __NVIC_PRIO_BITS)) - 1u)) == 0u,
		"[fiber]: ARM_CM55_NTZ scheduler BASEPRI uses unimplemented priority bits");
#if __NVIC_PRIO_BITS == 8
FIBER_STATIC_ASSERT((FIBER_PORT_SCHEDULER_BASEPRI & 1u) == 0u,
		"[fiber]: ARM_CM55_NTZ BASEPRI bit 0 is subpriority when all 8 bits exist");
#endif

/* ASCII "C55N" distinguishes this NTZ profile from future ARM_CM55 roles. */
#define FIBER_PORT_CONTEXT_ABI_PORT_ID 0x4335354Eu
...
/* ARMv8.1-M Mainline has no Cortex-M7 r0p1 BASEPRI erratum. Keep synchronized
...
#define FIBER_PORT_CM55_NTZ_CONTEXT_SP_OFFSET 0u
#define FIBER_PORT_CM55_NTZ_CONTEXT_BOOT_OFFSET 4u
#define FIBER_PORT_CM55_NTZ_CONTEXT_SIZE 76u
#define FIBER_PORT_CM55_NTZ_CONTEXT_ALIGNMENT 4u
#define FIBER_PORT_CM55_NTZ_BOOT_SIZE 72u

FIBER_STATIC_ASSERT(sizeof(void *) == 4u,
		"[fiber]: ARM_CM55_NTZ requires 32-bit pointers");
FIBER_STATIC_ASSERT(sizeof(size_t) == 4u,
		"[fiber]: ARM_CM55_NTZ requires 32-bit size_t");
FIBER_STATIC_ASSERT(FIBER_PORT_RUNTIME_SELECTABLE == 1,
		"[fiber]: ARM_CM55_NTZ full runtime must remain selectable");
FIBER_STATIC_ASSERT(FIBER_PORT_SOFTWARE_FRAME_WORDS == 10u,
		"[fiber]: ARM_CM55_NTZ software frame must contain ten words");
FIBER_STATIC_ASSERT(FIBER_PORT_EXC_RETURN_WORD_INDEX == 1u,
		"[fiber]: ARM_CM55_NTZ EXC_RETURN must follow the PSPLIM slot");
FIBER_STATIC_ASSERT(FIBER_PORT_CONTEXT_ABI_FEATURE_MASK == 0x82u,
		"[fiber]: ARM_CM55_NTZ context feature identity changed");
FIBER_STATIC_ASSERT(FIBER_PORT_INITIAL_CONTEXT_BYTES == 72u,
		"[fiber]: ARM_CM55_NTZ initial context geometry changed");
FIBER_STATIC_ASSERT(FIBER_PORT_MAX_SAVED_CONTEXT_BYTES == 76u,
		"[fiber]: ARM_CM55_NTZ maximum context geometry changed");
FIBER_STATIC_ASSERT(offsetof(FiberContext, sp) ==
		FIBER_PORT_CM55_NTZ_CONTEXT_SP_OFFSET,
		"[fiber]: ARM_CM55_NTZ saved-SP offset changed");
FIBER_STATIC_ASSERT(offsetof(FiberContext, boot) ==
		FIBER_PORT_CM55_NTZ_CONTEXT_BOOT_OFFSET,
		"[fiber]: ARM_CM55_NTZ boot offset changed");
FIBER_STATIC_ASSERT(sizeof(FiberContext) ==
		FIBER_PORT_CM55_NTZ_CONTEXT_SIZE,
		"[fiber]: ARM_CM55_NTZ context size changed");
FIBER_STATIC_ASSERT(alignof(FiberContext) ==
		FIBER_PORT_CM55_NTZ_CONTEXT_ALIGNMENT,
		"[fiber]: ARM_CM55_NTZ context alignment changed");
FIBER_STATIC_ASSERT(sizeof(FiberPortBoot) ==
		FIBER_PORT_CM55_NTZ_BOOT_SIZE,
		"[fiber]: ARM_CM55_NTZ boot size changed");
FIBER_STATIC_ASSERT(fiber_portSVC_ORIGIN_EXC_RETURN == 0xFFFFFFB8u,
		"[fiber]: ARM_CM55_NTZ first-start SVC origin changed");
FIBER_STATIC_ASSERT((fiber_portINITIAL_EXC_RETURN & ~4u) ==
		fiber_portSVC_ORIGIN_EXC_RETURN,
		"[fiber]: ARM_CM55_NTZ SVC/PSP EXC_RETURN relationship changed");
...
#endif /* FIBER_PORT_ARM_CM55_NTZ_FIBER_PORTMACRO_H_ */
