 ...
 * fiber_port_boot.c
 *
 * ARM_CM7/r0p1-owned PSP boot helpers.
...
/* The common runtime sees only FiberContext pointers. All context layout,
 * stack geometry, sealing, and restore validation stays in this selected port. */
		...
		void *const arg)
{
	...
	FIBER_REQUIRE((context_end <= stack_raw_begin) ||
			(context_begin >= stack_raw_end), 'C');
	...
	FIBER_REQUIRE((ctx->boot.stack_top &
...
void fiber_port_context_validate_restore(FiberContext *ctx)
{
	...
	/* The core hardware frame starts immediately after the port software
	 * frame. For an extended FP context, s16-s31 are software-saved before
	 * that core frame; the hardware-saved s0-s15/FPSCR block follows it. */
	...
	if ((exc_return & 0x10u) == 0u) {
		core_frame_offset += (uintptr_t)FIBER_HIGH_FP_SOFTWARE_BYTES;
	}
#endif
...
#endif /* ARM_CM7/r0p1 selected boot implementation */
