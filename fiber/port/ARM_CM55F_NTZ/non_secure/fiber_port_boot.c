/* ARM_CM55F_NTZ immutable boot metadata and initial-frame validation. */
...
FIBER_STATIC_ASSERT(fiber_portFRAME_EXC_RETURN ==
		...
		"[fiber]: ARM_CM55F_NTZ EXC_RETURN offset changed");
FIBER_STATIC_ASSERT(fiber_portFRAME_R0 == FIBER_PORT_SOFTWARE_FRAME_WORDS,
		"[fiber]: ARM_CM55F_NTZ hardware frame offset changed");
...
void fiber_port_init_context_frame(FiberContext *const ctx)
{
	...
	/* The pinned FreeRTOS M55 port seeds the same basic image with FPU
	 * enabled. s16-s31 does not exist until PendSV observes bit 4 clear. */
...
/* -------------------------------------------------------------------------- */
/* First-start preparation and restore validation                              */
/* -------------------------------------------------------------------------- */
...
void fiber_port_context_validate_save_current(const FiberContext *const ctx)
{
	...
	/* PendSV may conditionally save s16-s31. A lazy FP preservation in flight
	 * is valid here, but CPACR/FPCCR policy itself must already be correct. */
	fiber_port_fpu_require_configured();

	/* The stored SP is stale while this context runs. Check live PSP before
	 * naked code reads a context field or allocates either software frame. */
...
uintptr_t fiber_port_context_prepare_first_start(FiberContext *const ctx)
{
	...
	/* A newly constructed fiber has no captured s16-s31 state. */
