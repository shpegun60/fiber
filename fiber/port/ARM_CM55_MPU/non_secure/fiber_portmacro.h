 ...
 * fiber_portmacro.h
 *
 * Exact Cortex-M55 MPU Non-secure dictionary, implementation slice 1.
 * This is the FreeRTOS ARM_CM55_NTZ no-FPU/no-MVE/no-TrustZone/no-PAC
 * protected profile with an explicit 8- or 16-region manifest. It exports
 * only the frozen context layout and traits; construction, linker isolation,
 * SVC/PendSV, and the forward runtime ABI arrive in later slices.
 */
#ifndef FIBER_PORT_ARM_CM55_MPU_NON_SECURE_FIBER_PORTMACRO_H_
#define FIBER_PORT_ARM_CM55_MPU_NON_SECURE_FIBER_PORTMACRO_H_

#include <stddef.h>
#include <stdint.h>

#ifdef FIBER_PORT_NAME
# error "[fiber]: ARM_CM55_MPU port name must not be predefined"
#endif
#define FIBER_PORT_NAME "ARM_CM55_MPU"
...
# error "[fiber]: ARM_CM55_MPU is build-selected only"
#endif

#if !FIBER_PORT_ARMV81M_MAINLINE
# error "[fiber]: ARM_CM55_MPU requires the ARMv8.1-M Mainline selected profile"
#endif

/* GNU Arm 14.3 reports the same Armv8-M Mainline preprocessor result for
 * no-feature Cortex-M33 and Cortex-M55 targets. The selected profile and
 * generated CMSIS M55 identity are therefore both required; only the latter
 * identifies this exact CPU when MVE/FP/PAC are absent. */
#if !defined(__ARM_ARCH_8M_MAIN__) && !defined(__ARM_ARCH_8_1M_MAIN__)
# error "[fiber]: ARM_CM55_MPU requires an ARMv8-M Mainline compiler target"
#endif

#if !defined(__CORTEX_M) || (__CORTEX_M != 55)
# error "[fiber]: ARM_CM55_MPU manifest requires CMSIS __CORTEX_M == 55"
#endif

#if !defined(__MPU_PRESENT) || (__MPU_PRESENT != 1)
# error "[fiber]: ARM_CM55_MPU manifest requires __MPU_PRESENT == 1"
#endif

#if !defined(__VTOR_PRESENT) || (__VTOR_PRESENT != 1)
# error "[fiber]: ARM_CM55_MPU manifest requires __VTOR_PRESENT == 1"
#endif

#if defined(__ARM_FEATURE_CMSE) && ((__ARM_FEATURE_CMSE + 0) >= 3)
# error "[fiber]: ARM_CM55_MPU selected profile excludes Secure CMSE builds"
#endif

#if defined(__FPU_PRESENT) && ((__FPU_PRESENT + 0) != 0) && \
		((__FPU_PRESENT + 0) != 1)
# error "[fiber]: ARM_CM55_MPU CMSIS __FPU_PRESENT must be 0 or 1"
#endif

#if defined(__FPU_USED) && ((__FPU_USED + 0) != 0)
# error "[fiber]: ARM_CM55_MPU selected profile requires __FPU_USED == 0"
#endif

#if defined(__ARM_FP) && ((__ARM_FP + 0) != 0)
# error "[fiber]: ARM_CM55_MPU selected profile does not permit an FP register ABI"
#endif

#if defined(__VFP_FP__) && !defined(__SOFTFP__)
# error "[fiber]: ARM_CM55_MPU selected profile does not permit a hard-FP compiler ABI"
#endif

#if defined(__ARM_FEATURE_MVE) && ((__ARM_FEATURE_MVE + 0) != 0)
# error "[fiber]: ARM_CM55_MPU selected profile does not permit MVE"
#endif

#if defined(__ARM_FEATURE_PAC_DEFAULT) || defined(__ARM_FEATURE_PAUTH) || \
		defined(__ARM_FEATURE_PAUTH_DEFAULT)
# error "[fiber]: ARM_CM55_MPU selected profile does not permit PAC"
#endif

#if defined(__ARM_FEATURE_BTI_DEFAULT) || defined(__ARM_FEATURE_BTI)
# error "[fiber]: ARM_CM55_MPU selected profile does not permit BTI"
#endif

#if !defined(__NVIC_PRIO_BITS) || (__NVIC_PRIO_BITS < 1) || \
		(__NVIC_PRIO_BITS > 8)
# error "[fiber]: ARM_CM55_MPU requires 1..8 implemented NVIC priority bits"
#endif

#ifndef FIBER_PORT_CM55_MPU_TOTAL_REGIONS
# error "[fiber]: ARM_CM55_MPU requires FIBER_PORT_CM55_MPU_TOTAL_REGIONS=8 or 16"
#endif

#if (FIBER_PORT_CM55_MPU_TOTAL_REGIONS != 8) && \
		(FIBER_PORT_CM55_MPU_TOTAL_REGIONS != 16)
# error "[fiber]: ARM_CM55_MPU supports exactly 8 or 16 MPU regions"
#endif
...
# error "[fiber]: ARM_CM55_MPU owns its complete SVC namespace"
#endif
...
#define fiber_portMPU_TOTAL_REGIONS FIBER_PORT_CM55_MPU_TOTAL_REGIONS
...
#define fiber_portMPU_CONTEXT_REGION_COUNT \
	FIBER_PORT_CM55_MPU_CONTEXT_REGION_COUNT
...
# error "[fiber]: ARM_CM55_MPU scheduler BASEPRI threshold must be non-zero"
#endif

#if FIBER_PORT_SCHEDULER_BASEPRI > 255u
# error "[fiber]: ARM_CM55_MPU scheduler BASEPRI threshold must fit in 8 bits"
#endif

FIBER_STATIC_ASSERT((FIBER_PORT_SCHEDULER_BASEPRI &
		((1u << (8u - __NVIC_PRIO_BITS)) - 1u)) == 0u,
		"[fiber]: ARM_CM55_MPU BASEPRI uses unimplemented priority bits");
#if __NVIC_PRIO_BITS == 8
FIBER_STATIC_ASSERT((FIBER_PORT_SCHEDULER_BASEPRI & 1u) == 0u,
		"[fiber]: ARM_CM55_MPU BASEPRI bit 0 is subpriority");
#endif

#if FIBER_PORT_CM55_MPU_TOTAL_REGIONS == 8
# define FIBER_PORT_CONTEXT_ABI_LAYOUT_VERSION 0x00010008u
# define FIBER_PORT_CM55_MPU_PROTECTED_CONTEXT_OFFSET 40u
# define FIBER_PORT_CM55_MPU_RUNTIME_FLAGS_OFFSET 124u
# define FIBER_PORT_CM55_MPU_BOOT_OFFSET 128u
# define FIBER_PORT_CM55_MPU_CONTEXT_SIZE 216u
#else
# define FIBER_PORT_CONTEXT_ABI_LAYOUT_VERSION 0x00010010u
# define FIBER_PORT_CM55_MPU_PROTECTED_CONTEXT_OFFSET 104u
# define FIBER_PORT_CM55_MPU_RUNTIME_FLAGS_OFFSET 188u
# define FIBER_PORT_CM55_MPU_BOOT_OFFSET 192u
# define FIBER_PORT_CM55_MPU_CONTEXT_SIZE 280u
#endif

/* ASCII "C55M". This no-TrustZone cohort has a one-past context cursor
 * target, not a SecureContext slot. */
#define FIBER_PORT_CONTEXT_ABI_PORT_ID 0x4335354Du
 ...
 * words. */
#define FIBER_PORT_CM55_MPU_PSP_INITIAL_FRAME_BYTES \
	FIBER_PORT_EXC_BASE_BYTES
#define FIBER_PORT_CM55_MPU_PSP_MAX_FRAME_BYTES \
	(FIBER_PORT_EXC_PER_LEVEL_BYTES + \
	 FIBER_PORT_EXCEPTION_ALIGNMENT_PAD_BYTES)
#define FIBER_PORT_CM55_MPU_STACK_REQUIRED_BYTES \
	((FIBER_PORT_CM55_MPU_PSP_MAX_FRAME_BYTES + \
...
#define FIBER_PORT_CM55_MPU_REGION_RBAR_OFFSET 0u
#define FIBER_PORT_CM55_MPU_REGION_RLAR_OFFSET 4u
#define FIBER_PORT_CM55_MPU_REGION_SIZE 8u
#define FIBER_PORT_CM55_MPU_PROTECTED_R4_OFFSET 0u
#define FIBER_PORT_CM55_MPU_PROTECTED_R5_OFFSET 4u
#define FIBER_PORT_CM55_MPU_PROTECTED_R6_OFFSET 8u
#define FIBER_PORT_CM55_MPU_PROTECTED_R7_OFFSET 12u
#define FIBER_PORT_CM55_MPU_PROTECTED_R8_OFFSET 16u
#define FIBER_PORT_CM55_MPU_PROTECTED_R9_OFFSET 20u
#define FIBER_PORT_CM55_MPU_PROTECTED_R10_OFFSET 24u
#define FIBER_PORT_CM55_MPU_PROTECTED_R11_OFFSET 28u
#define FIBER_PORT_CM55_MPU_PROTECTED_R0_OFFSET 32u
#define FIBER_PORT_CM55_MPU_PROTECTED_R1_OFFSET 36u
#define FIBER_PORT_CM55_MPU_PROTECTED_R2_OFFSET 40u
#define FIBER_PORT_CM55_MPU_PROTECTED_R3_OFFSET 44u
#define FIBER_PORT_CM55_MPU_PROTECTED_R12_OFFSET 48u
#define FIBER_PORT_CM55_MPU_PROTECTED_LR_OFFSET 52u
#define FIBER_PORT_CM55_MPU_PROTECTED_PC_OFFSET 56u
#define FIBER_PORT_CM55_MPU_PROTECTED_XPSR_OFFSET 60u
#define FIBER_PORT_CM55_MPU_PROTECTED_PSP_OFFSET 64u
#define FIBER_PORT_CM55_MPU_PROTECTED_PSPLIM_OFFSET 68u
#define FIBER_PORT_CM55_MPU_PROTECTED_CONTROL_OFFSET 72u
#define FIBER_PORT_CM55_MPU_PROTECTED_EXC_RETURN_OFFSET 76u
#define FIBER_PORT_CM55_MPU_PROTECTED_CURSOR_LIMIT_OFFSET 80u
#define FIBER_PORT_CM55_MPU_PROTECTED_CONTEXT_SIZE 84u
#define FIBER_PORT_CM55_MPU_CURSOR_OFFSET 0u
#define FIBER_PORT_CM55_MPU_MAIR0_OFFSET 4u
#define FIBER_PORT_CM55_MPU_REGIONS_OFFSET 8u
#define FIBER_PORT_CM55_MPU_CONTEXT_ALIGNMENT 8u

#define FIBER_PORT_CM55_MPU_BOOT_BEGIN_OFFSET 0u
#define FIBER_PORT_CM55_MPU_BOOT_END_OFFSET 4u
#define FIBER_PORT_CM55_MPU_BOOT_STACK_BASE_OFFSET 8u
#define FIBER_PORT_CM55_MPU_BOOT_STACK_TOP_OFFSET 12u
#define FIBER_PORT_CM55_MPU_BOOT_AVAIL_OFFSET 16u
#define FIBER_PORT_CM55_MPU_BOOT_ENTRY_OFFSET 20u
#define FIBER_PORT_CM55_MPU_BOOT_ARG_OFFSET 24u
#define FIBER_PORT_CM55_MPU_BOOT_PORT_ID_OFFSET 28u
#define FIBER_PORT_CM55_MPU_BOOT_LAYOUT_VERSION_OFFSET 32u
#define FIBER_PORT_CM55_MPU_BOOT_CONTEXT_SIZE_OFFSET 36u
#define FIBER_PORT_CM55_MPU_BOOT_CONTEXT_ALIGNMENT_OFFSET 40u
#define FIBER_PORT_CM55_MPU_BOOT_FEATURE_MASK_OFFSET 44u
#define FIBER_PORT_CM55_MPU_BOOT_INITIAL_EXC_RETURN_OFFSET 48u
#define FIBER_PORT_CM55_MPU_BOOT_INITIAL_CONTROL_OFFSET 52u
#define FIBER_PORT_CM55_MPU_BOOT_TOTAL_REGIONS_OFFSET 56u
#define FIBER_PORT_CM55_MPU_BOOT_CONTEXT_REGIONS_OFFSET 60u
#define FIBER_PORT_CM55_MPU_BOOT_PROTECTED_WORDS_OFFSET 64u
#define FIBER_PORT_CM55_MPU_BOOT_MAGIC_OFFSET 68u
#define FIBER_PORT_CM55_MPU_BOOT_VERSION_OFFSET 72u
#define FIBER_PORT_CM55_MPU_BOOT_SEALED_OFFSET 74u
#define FIBER_PORT_CM55_MPU_BOOT_GUARD_LO_OFFSET 76u
#define FIBER_PORT_CM55_MPU_BOOT_GUARD_HI_OFFSET 80u
#define FIBER_PORT_CM55_MPU_BOOT_HASH_OFFSET 84u
#define FIBER_PORT_CM55_MPU_BOOT_SIZE 88u
...
FIBER_STATIC_ASSERT(fiber_portSVC_YIELD == 71u,
		"[fiber]: ARM_CM55_MPU yield SVC changed");
FIBER_STATIC_ASSERT(fiber_portSVC_RETURN == 72u,
		"[fiber]: ARM_CM55_MPU task-return SVC changed");
...
FIBER_STATIC_ASSERT(fiber_portMPU_PRIVILEGED_FLASH_REGION_NUMBER == 0u &&
		...
		"[fiber]: ARM_CM55_MPU hardware MPU region numbering changed");
...
FIBER_STATIC_ASSERT(FIBER_PORT_EXC_RETURN_WORD_INDEX == 19u &&
		...
		"[fiber]: ARM_CM55_MPU logical restore geometry changed");
FIBER_STATIC_ASSERT(FIBER_PORT_HAS_CONTROL_SLOT == 1,
		"[fiber]: ARM_CM55_MPU must preserve CONTROL per context");
FIBER_STATIC_ASSERT(FIBER_PORT_HAS_PSPLIM_SLOT == 1,
		"[fiber]: ARM_CM55_MPU must preserve PSPLIM per context");
FIBER_STATIC_ASSERT(FIBER_PORT_HAS_SECURE_CONTEXT_SLOT == 0,
		"[fiber]: ARM_CM55_MPU selected profile must not expose SecureContext");
FIBER_STATIC_ASSERT(FIBER_PORT_HAS_PAC_KEY_SLOT == 0,
		"[fiber]: ARM_CM55_MPU selected profile must not expose PAC state");
FIBER_STATIC_ASSERT(FIBER_PORT_HAS_FPU == 0 &&
		FIBER_PORT_HAS_MVE == 0 && FIBER_PORT_HAS_PAC == 0 &&
		FIBER_PORT_HAS_BTI == 0,
		"[fiber]: ARM_CM55_MPU selected feature cohort changed");
FIBER_STATIC_ASSERT(FIBER_PORT_CONTEXT_ABI_FEATURE_MASK == 0x00001C86u,
		"[fiber]: ARM_CM55_MPU context feature identity changed");
FIBER_STATIC_ASSERT(FIBER_PORT_CM55_MPU_PSP_INITIAL_FRAME_BYTES == 32u &&
		FIBER_PORT_CM55_MPU_PSP_MAX_FRAME_BYTES == 36u &&
		FIBER_PORT_CM55_MPU_STACK_REQUIRED_BYTES == 40u,
		"[fiber]: ARM_CM55_MPU physical PSP geometry changed");
FIBER_STATIC_ASSERT(FIBER_PORT_CM55_MPU_PROTECTED_CONTEXT_OFFSET ==
		(FIBER_PORT_CM55_MPU_REGIONS_OFFSET +
		 (FIBER_PORT_CM55_MPU_CONTEXT_REGION_COUNT *
		  FIBER_PORT_CM55_MPU_REGION_SIZE)),
		"[fiber]: ARM_CM55_MPU protected context offset changed");
FIBER_STATIC_ASSERT(FIBER_PORT_CM55_MPU_BOOT_OFFSET ==
		(FIBER_PORT_CM55_MPU_RUNTIME_FLAGS_OFFSET + 4u),
		"[fiber]: ARM_CM55_MPU boot offset changed");
FIBER_STATIC_ASSERT(offsetof(FiberPortMpuRegionRegisters, rbar) ==
		FIBER_PORT_CM55_MPU_REGION_RBAR_OFFSET &&
		offsetof(FiberPortMpuRegionRegisters, rlar) ==
		FIBER_PORT_CM55_MPU_REGION_RLAR_OFFSET &&
		sizeof(FiberPortMpuRegionRegisters) == FIBER_PORT_CM55_MPU_REGION_SIZE,
		"[fiber]: ARM_CM55_MPU MPU region layout changed");
FIBER_STATIC_ASSERT(offsetof(FiberPortProtectedContext, r4) ==
		FIBER_PORT_CM55_MPU_PROTECTED_R4_OFFSET &&
		offsetof(FiberPortProtectedContext, r5) ==
		FIBER_PORT_CM55_MPU_PROTECTED_R5_OFFSET &&
		offsetof(FiberPortProtectedContext, r6) ==
		FIBER_PORT_CM55_MPU_PROTECTED_R6_OFFSET &&
		offsetof(FiberPortProtectedContext, r7) ==
		FIBER_PORT_CM55_MPU_PROTECTED_R7_OFFSET &&
		offsetof(FiberPortProtectedContext, r8) ==
		FIBER_PORT_CM55_MPU_PROTECTED_R8_OFFSET &&
		offsetof(FiberPortProtectedContext, r9) ==
		FIBER_PORT_CM55_MPU_PROTECTED_R9_OFFSET &&
		offsetof(FiberPortProtectedContext, r10) ==
		FIBER_PORT_CM55_MPU_PROTECTED_R10_OFFSET &&
		offsetof(FiberPortProtectedContext, r11) ==
		FIBER_PORT_CM55_MPU_PROTECTED_R11_OFFSET,
		"[fiber]: ARM_CM55_MPU callee-saved context offsets changed");
FIBER_STATIC_ASSERT(offsetof(FiberPortProtectedContext, r0) ==
		FIBER_PORT_CM55_MPU_PROTECTED_R0_OFFSET &&
		offsetof(FiberPortProtectedContext, r1) ==
		FIBER_PORT_CM55_MPU_PROTECTED_R1_OFFSET &&
		offsetof(FiberPortProtectedContext, r2) ==
		FIBER_PORT_CM55_MPU_PROTECTED_R2_OFFSET &&
		offsetof(FiberPortProtectedContext, r3) ==
		FIBER_PORT_CM55_MPU_PROTECTED_R3_OFFSET &&
		offsetof(FiberPortProtectedContext, r12) ==
		FIBER_PORT_CM55_MPU_PROTECTED_R12_OFFSET &&
		offsetof(FiberPortProtectedContext, lr) ==
		FIBER_PORT_CM55_MPU_PROTECTED_LR_OFFSET &&
		offsetof(FiberPortProtectedContext, pc) ==
		FIBER_PORT_CM55_MPU_PROTECTED_PC_OFFSET &&
		offsetof(FiberPortProtectedContext, xpsr) ==
		FIBER_PORT_CM55_MPU_PROTECTED_XPSR_OFFSET,
		"[fiber]: ARM_CM55_MPU copied hardware-frame offsets changed");
FIBER_STATIC_ASSERT(offsetof(FiberPortProtectedContext, psp) ==
		FIBER_PORT_CM55_MPU_PROTECTED_PSP_OFFSET &&
		offsetof(FiberPortProtectedContext, psplim) ==
		FIBER_PORT_CM55_MPU_PROTECTED_PSPLIM_OFFSET &&
		offsetof(FiberPortProtectedContext, control) ==
		FIBER_PORT_CM55_MPU_PROTECTED_CONTROL_OFFSET &&
		offsetof(FiberPortProtectedContext, exc_return) ==
		FIBER_PORT_CM55_MPU_PROTECTED_EXC_RETURN_OFFSET &&
		offsetof(FiberPortProtectedContext, cursor_limit) ==
		FIBER_PORT_CM55_MPU_PROTECTED_CURSOR_LIMIT_OFFSET &&
		sizeof(FiberPortProtectedContext) ==
		FIBER_PORT_CM55_MPU_PROTECTED_CONTEXT_SIZE,
		"[fiber]: ARM_CM55_MPU special-context offsets changed");
FIBER_STATIC_ASSERT(FIBER_PORT_CM55_MPU_PROTECTED_CURSOR_LIMIT_OFFSET ==
		(FIBER_PORT_CM55_MPU_PROTECTED_CONTEXT_SIZE - 4u),
		"[fiber]: ARM_CM55_MPU cursor limit must remain final");
FIBER_STATIC_ASSERT(offsetof(FiberPortBoot, begin) ==
		FIBER_PORT_CM55_MPU_BOOT_BEGIN_OFFSET &&
		offsetof(FiberPortBoot, end) == FIBER_PORT_CM55_MPU_BOOT_END_OFFSET &&
		offsetof(FiberPortBoot, stack_base) ==
		FIBER_PORT_CM55_MPU_BOOT_STACK_BASE_OFFSET &&
		offsetof(FiberPortBoot, stack_top) ==
		FIBER_PORT_CM55_MPU_BOOT_STACK_TOP_OFFSET &&
		offsetof(FiberPortBoot, avail) ==
		FIBER_PORT_CM55_MPU_BOOT_AVAIL_OFFSET &&
		offsetof(FiberPortBoot, entry) ==
		FIBER_PORT_CM55_MPU_BOOT_ENTRY_OFFSET &&
		offsetof(FiberPortBoot, arg) == FIBER_PORT_CM55_MPU_BOOT_ARG_OFFSET,
		"[fiber]: ARM_CM55_MPU boot base offsets changed");
FIBER_STATIC_ASSERT(offsetof(FiberPortBoot, abi_port_id) ==
		FIBER_PORT_CM55_MPU_BOOT_PORT_ID_OFFSET &&
		offsetof(FiberPortBoot, abi_layout_version) ==
		FIBER_PORT_CM55_MPU_BOOT_LAYOUT_VERSION_OFFSET &&
		offsetof(FiberPortBoot, abi_context_size) ==
		FIBER_PORT_CM55_MPU_BOOT_CONTEXT_SIZE_OFFSET &&
		offsetof(FiberPortBoot, abi_context_alignment) ==
		FIBER_PORT_CM55_MPU_BOOT_CONTEXT_ALIGNMENT_OFFSET &&
		offsetof(FiberPortBoot, abi_feature_mask) ==
		FIBER_PORT_CM55_MPU_BOOT_FEATURE_MASK_OFFSET &&
		offsetof(FiberPortBoot, abi_initial_exc_return) ==
		FIBER_PORT_CM55_MPU_BOOT_INITIAL_EXC_RETURN_OFFSET &&
		offsetof(FiberPortBoot, abi_initial_control) ==
		FIBER_PORT_CM55_MPU_BOOT_INITIAL_CONTROL_OFFSET &&
		offsetof(FiberPortBoot, abi_mpu_total_regions) ==
		FIBER_PORT_CM55_MPU_BOOT_TOTAL_REGIONS_OFFSET &&
		offsetof(FiberPortBoot, abi_mpu_context_regions) ==
		FIBER_PORT_CM55_MPU_BOOT_CONTEXT_REGIONS_OFFSET &&
		offsetof(FiberPortBoot, abi_protected_context_words) ==
		FIBER_PORT_CM55_MPU_BOOT_PROTECTED_WORDS_OFFSET,
		"[fiber]: ARM_CM55_MPU boot ABI offsets changed");
FIBER_STATIC_ASSERT(offsetof(FiberPortBoot, magic) ==
		FIBER_PORT_CM55_MPU_BOOT_MAGIC_OFFSET &&
		offsetof(FiberPortBoot, version) ==
		FIBER_PORT_CM55_MPU_BOOT_VERSION_OFFSET &&
		offsetof(FiberPortBoot, sealed) ==
		FIBER_PORT_CM55_MPU_BOOT_SEALED_OFFSET &&
		offsetof(FiberPortBoot, guard_lo) ==
		FIBER_PORT_CM55_MPU_BOOT_GUARD_LO_OFFSET &&
		offsetof(FiberPortBoot, guard_hi) ==
		FIBER_PORT_CM55_MPU_BOOT_GUARD_HI_OFFSET &&
		offsetof(FiberPortBoot, hash) == FIBER_PORT_CM55_MPU_BOOT_HASH_OFFSET &&
		sizeof(FiberPortBoot) == FIBER_PORT_CM55_MPU_BOOT_SIZE,
		"[fiber]: ARM_CM55_MPU boot seal offsets changed");
FIBER_STATIC_ASSERT(offsetof(FiberContext, protected_context_cursor) ==
		FIBER_PORT_CM55_MPU_CURSOR_OFFSET &&
		offsetof(FiberContext, mair0) == FIBER_PORT_CM55_MPU_MAIR0_OFFSET &&
		offsetof(FiberContext, mpu_regions) ==
		FIBER_PORT_CM55_MPU_REGIONS_OFFSET &&
		offsetof(FiberContext, protected_context) ==
		FIBER_PORT_CM55_MPU_PROTECTED_CONTEXT_OFFSET &&
		offsetof(FiberContext, runtime_flags) ==
		FIBER_PORT_CM55_MPU_RUNTIME_FLAGS_OFFSET &&
		offsetof(FiberContext, boot) == FIBER_PORT_CM55_MPU_BOOT_OFFSET &&
		sizeof(FiberContext) == FIBER_PORT_CM55_MPU_CONTEXT_SIZE &&
		alignof(FiberContext) == FIBER_PORT_CM55_MPU_CONTEXT_ALIGNMENT,
		"[fiber]: ARM_CM55_MPU FiberContext layout changed");
...
#endif /* FIBER_PORT_ARM_CM55_MPU_NON_SECURE_FIBER_PORTMACRO_H_ */
