/* ARM_CM55_MPU/non_secure sealed construction and linker-owned MPU image. */

#include "fiber_port_boot.h"

#define FIBER_CM55_MPU_BOOT \
	FIBER_API_ATTR_SENSITIVE FIBER_GENERAL_REGS_ONLY \
	fiber_portPRIVILEGED_FUNCTION

static FIBER_CM55_MPU_BOOT
		...
		const FiberPortMpuMemoryLayout *layout)
{
	...
	FIBER_REQUIRE(boot->avail >=
			(size_t)FIBER_PORT_CM55_MPU_STACK_REQUIRED_BYTES, 'H');
		...
		void *arg)
{
	...
	FIBER_REQUIRE((stack_top - stack_base) >=
			(uintptr_t)FIBER_PORT_CM55_MPU_STACK_REQUIRED_BYTES, 'H');
