...
void fiber_port_context_validate_save_current(const FiberContext *const ctx)
{
	...
	/* The M55 MVE-FP FreeRTOS path conditionally saves s16-s31. A lazy
	 * extended preservation in flight is valid here; CPACR/FPCCR policy is not. */
	fiber_port_fpu_require_configured();

	/* The stored SP is stale while this context runs. Validate live PSP before
	 * PendSV reads metadata or reserves its conditional high-register frame. */
...
uintptr_t fiber_port_context_prepare_first_start(FiberContext *const ctx)
{
	...
	/* A newly constructed MVE-FP fiber has no captured s16-s31 state. */
