...
# error "[fiber]: ARM_CM55_MVEF_NTZ port name must not be predefined"
#endif
#define FIBER_PORT_NAME "ARM_CM55_MVEF_NTZ"
...
# error "[fiber]: ARM_CM55_MVEF_NTZ is build-selected only"
#endif

#if !FIBER_PORT_ARMV81M_MAINLINE
# error "[fiber]: ARM_CM55_MVEF_NTZ requires the ARMv8.1-M Mainline architecture result"
#endif

#if !defined(__ARM_ARCH_8M_MAIN__) && !defined(__ARM_ARCH_8_1M_MAIN__)
# error "[fiber]: ARM_CM55_MVEF_NTZ requires an ARMv8.1-M Mainline compiler target"
#endif

#if !defined(__CORTEX_M) || (__CORTEX_M != 55)
# error "[fiber]: ARM_CM55_MVEF_NTZ manifest requires CMSIS __CORTEX_M == 55"
#endif

#if defined(__ARM_FEATURE_CMSE) && ((__ARM_FEATURE_CMSE + 0) >= 3)
# error "[fiber]: ARM_CM55_MVEF_NTZ does not accept a Secure CMSE build"
#endif

#if !defined(__VTOR_PRESENT) || (__VTOR_PRESENT != 1)
# error "[fiber]: ARM_CM55_MVEF_NTZ requires __VTOR_PRESENT == 1"
#endif
...
# error "[fiber]: ARM_CM55_MVEF_NTZ FPU facts are selected-port-owned"
#endif

/* __ARM_FEATURE_MVE bit 0 denotes MVE integer and bit 1 denotes MVE FP.
 * Reject an integer-only MVE compiler result before emitting an unrelated FPU
 * diagnostic. The pinned FreeRTOS configENABLE_MVE path adds no MVE-only
 * software slot beyond the conditional extended FP frame. */
#if !defined(__ARM_FEATURE_MVE) || \
		(((__ARM_FEATURE_MVE + 0) & 3) != 3)
# error "[fiber]: ARM_CM55_MVEF_NTZ requires MVE FP compiler support"
#endif

#if !defined(__FPU_PRESENT) || ((__FPU_PRESENT + 0) != 1)
# error "[fiber]: ARM_CM55_MVEF_NTZ requires CMSIS __FPU_PRESENT == 1"
#endif

#if !defined(__FPU_USED) || ((__FPU_USED + 0) != 1)
# error "[fiber]: ARM_CM55_MVEF_NTZ requires CMSIS __FPU_USED == 1"
#endif

#if !defined(__ARM_FP) || ((__ARM_FP + 0) == 0)
# error "[fiber]: ARM_CM55_MVEF_NTZ requires compiler FP code generation"
#endif
...
# error "[fiber]: ARM_CM55_MVEF_NTZ does not permit PAC"
#endif

#if defined(__ARM_FEATURE_BTI_DEFAULT) || defined(__ARM_FEATURE_BTI)
# error "[fiber]: ARM_CM55_MVEF_NTZ does not permit BTI"
#endif

#if defined(FIBER_PORT_CM55_MPU_TOTAL_REGIONS) || \
		defined(FIBER_PORT_MPU_TOTAL_REGIONS)
# error "[fiber]: ARM_CM55_MVEF_NTZ is non-MPU; select a separate MPU profile"
#endif

#if !defined(__NVIC_PRIO_BITS) || (__NVIC_PRIO_BITS < 1) || \
		(__NVIC_PRIO_BITS > 8)
# error "[fiber]: ARM_CM55_MVEF_NTZ requires 1..8 implemented NVIC priority bits"
#endif

#ifdef FIBER_PORT_RUNTIME_SELECTABLE
# error "[fiber]: ARM_CM55_MVEF_NTZ runtime-selectable state must not be predefined"
#endif
#define FIBER_PORT_RUNTIME_SELECTABLE 0

/* Pinned FreeRTOS ARM_CM55_NTZ non-MPU MVE-FP frame dictionary. New tasks start
 * with a basic hardware frame. The eventual PendSV saves s16-s31 only when
 * EXC_RETURN bit 4 is clear; the pinned MVE path adds no VPR software slot. */
...
# error "[fiber]: ARM_CM55_MVEF_NTZ scheduler BASEPRI threshold must be non-zero"
#endif

#if FIBER_PORT_SCHEDULER_BASEPRI > 255u
# error "[fiber]: ARM_CM55_MVEF_NTZ scheduler BASEPRI threshold must fit in 8 bits"
#endif

FIBER_STATIC_ASSERT((FIBER_PORT_SCHEDULER_BASEPRI &
		((1u << (8u - __NVIC_PRIO_BITS)) - 1u)) == 0u,
		"[fiber]: ARM_CM55_MVEF_NTZ scheduler BASEPRI uses unimplemented priority bits");
#if __NVIC_PRIO_BITS == 8
FIBER_STATIC_ASSERT((FIBER_PORT_SCHEDULER_BASEPRI & 1u) == 0u,
		"[fiber]: ARM_CM55_MVEF_NTZ BASEPRI bit 0 is subpriority when all 8 bits exist");
#endif

/* ASCII "C55V" is distinct from scalar no-FPU "C55N" and scalar-FP "C55F". */
#define FIBER_PORT_CONTEXT_ABI_PORT_ID 0x43353556u
...
#define FIBER_PORT_CM55_MVEF_NTZ_CONTEXT_SP_OFFSET 0u
#define FIBER_PORT_CM55_MVEF_NTZ_CONTEXT_BOOT_OFFSET 4u
#define FIBER_PORT_CM55_MVEF_NTZ_CONTEXT_SIZE 76u
#define FIBER_PORT_CM55_MVEF_NTZ_CONTEXT_ALIGNMENT 4u
#define FIBER_PORT_CM55_MVEF_NTZ_BOOT_SIZE 72u

FIBER_STATIC_ASSERT(sizeof(void *) == 4u,
		"[fiber]: ARM_CM55_MVEF_NTZ requires 32-bit pointers");
...
FIBER_STATIC_ASSERT(FIBER_PORT_EXC_RETURN_WORD_INDEX == 1u,
		"[fiber]: ARM_CM55_MVEF_NTZ EXC_RETURN must follow the PSPLIM slot");
FIBER_STATIC_ASSERT(FIBER_PORT_CONTEXT_ABI_FEATURE_MASK == 0x93u,
		"[fiber]: ARM_CM55_MVEF_NTZ context feature identity changed");
FIBER_STATIC_ASSERT(FIBER_PORT_INITIAL_CONTEXT_BYTES == 72u,
		"[fiber]: ARM_CM55_MVEF_NTZ initial context geometry changed");
FIBER_STATIC_ASSERT(FIBER_PORT_MAX_SAVED_CONTEXT_BYTES == 212u,
		"[fiber]: ARM_CM55_MVEF_NTZ maximum context geometry changed");
FIBER_STATIC_ASSERT(fiber_portEXTENDED_EXC_RETURN ==
		(fiber_portINITIAL_EXC_RETURN & ~0x10u),
		"[fiber]: ARM_CM55_MVEF_NTZ extended EXC_RETURN changed");
FIBER_STATIC_ASSERT(fiber_portSVC_ORIGIN_EXC_RETURN == 0xFFFFFFB8u,
		"[fiber]: ARM_CM55_MVEF_NTZ first-start SVC origin changed");
FIBER_STATIC_ASSERT((fiber_portINITIAL_EXC_RETURN & ~4u) ==
		fiber_portSVC_ORIGIN_EXC_RETURN,
		"[fiber]: ARM_CM55_MVEF_NTZ SVC/PSP EXC_RETURN relationship changed");
FIBER_STATIC_ASSERT(offsetof(FiberContext, sp) ==
		FIBER_PORT_CM55_MVEF_NTZ_CONTEXT_SP_OFFSET,
		"[fiber]: ARM_CM55_MVEF_NTZ saved-SP offset changed");
FIBER_STATIC_ASSERT(offsetof(FiberContext, boot) ==
		FIBER_PORT_CM55_MVEF_NTZ_CONTEXT_BOOT_OFFSET,
		"[fiber]: ARM_CM55_MVEF_NTZ boot offset changed");
FIBER_STATIC_ASSERT(sizeof(FiberContext) ==
		FIBER_PORT_CM55_MVEF_NTZ_CONTEXT_SIZE,
		"[fiber]: ARM_CM55_MVEF_NTZ context size changed");
FIBER_STATIC_ASSERT(alignof(FiberContext) ==
		FIBER_PORT_CM55_MVEF_NTZ_CONTEXT_ALIGNMENT,
		"[fiber]: ARM_CM55_MVEF_NTZ context alignment changed");
FIBER_STATIC_ASSERT(sizeof(FiberPortBoot) == FIBER_PORT_CM55_MVEF_NTZ_BOOT_SIZE,
		"[fiber]: ARM_CM55_MVEF_NTZ boot size changed");
...
#endif /* FIBER_PORT_ARM_CM55_MVEF_NTZ_FIBER_PORTMACRO_H_ */
