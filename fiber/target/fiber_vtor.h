...
/* 4) STM32 family hints (vendor reality checks)
     Note: for Cortex-M0+ families VTOR is commonly present on these lines.
     If your specific part disagrees, use -DFIBER_FORCE_VTOR=0/1. */
...
# elif defined(STM32G0)    || defined(STM32G0xx) ||       	\
		...
		defined(STM32C0)    || defined(STM32C0xx)          /* M0+: VTOR present (vendor-specific) */
#  define FIBER_HAS_VTOR 1
...
/* Default to current bank unless explicitly asked for NS. */
...
/* Optional: safe VTOR setter with alignment masking and barriers. */
__STATIC_FORCEINLINE void fiber_set_vectors_base_addr(uintptr_t base)
{
#if FIBER_VTOR_USE_NS || FIBER_HAS_VTOR
	/* If TBLOFF mask is available, clear reserved low bits to satisfy alignment. */
# if defined(SCB_VTOR_TBLOFF_Msk)
	base &= (uintptr_t)SCB_VTOR_TBLOFF_Msk;
# endif
	*FIBER_SCB_VTOR_PTR = (uint32_t)base;
	{ __DSB(); __ISB(); }
#else
	(void)base; /* no VTOR: ignore to avoid undefined register access */
#endif
}
