function Test-ArmCm55MvefMpuSvcContract {
    param(
    ...
    $profileDir = Join-Path $RepositoryRoot "fiber\port\ARM_CM55_MVEF_MPU\non_secure"
    ...
    $fixture = Join-Path $RepositoryRoot "tools\fixtures\arm_cm55_mvef_mpu_svc_probe.c"
    $fixtureLinker = Join-Path $RepositoryRoot "tools\fixtures\arm_cm55_mvef_mpu_svc.ld"
    $competing = Join-Path $RepositoryRoot "tools\fixtures\arm_cm55_mvef_mpu_competing_svc.c"

    foreach ($required in @($bootSource, $svcSource, $pendSvSource, $privateHeader,
            $linkerContract, $fixture, $fixtureLinker, $competing)) {
        if (-not (Test-Path -LiteralPath $required)) {
            throw "ARM_CM55_MVEF_MPU SVC/runtime slice is missing: $required"
        }
    }
    foreach ($forbidden in @(
        ...
        if (Test-Path -LiteralPath (Join-Path $profileDir $forbidden)) {
            throw "ARM_CM55_MVEF_MPU SVC/runtime slice exposed deferred artifact: $forbidden"
        }
    }
    ...
    foreach ($selector in @(
        ...
        if ((Get-Content -LiteralPath $selector -Raw).IndexOf(
            ...
            throw "ARM_CM55_MVEF_MPU SVC slice must not enter global selection: $selector"
        }
    }
    ...
    foreach ($requiredText in @(
        ...
        if ($svcText.IndexOf($requiredText,
            ...
            throw "ARM_CM55_MVEF_MPU SVC/runtime source lost mechanism: $requiredText"
        }
    }
    foreach ($forbiddenText in @(
        ...
        if ($svcText.IndexOf($forbiddenText,
            ...
            throw "ARM_CM55_MVEF_MPU SVC/runtime object acquired PendSV handler ownership: $forbiddenText"
        }
    }
    ...
    if ($privateText.IndexOf("fiber_runtime_port_abi.h",
            [System.StringComparison]::Ordinal) -lt 0) {
        throw "ARM_CM55_MVEF_MPU SVC must consume frozen reverse ABI v1"
    }
    if ($privateText.IndexOf("fiber_port_runtime_abi.h",
            [System.StringComparison]::Ordinal) -lt 0) {
        throw "ARM_CM55_MVEF_MPU mandatory SVC/runtime object must consume the frozen forward ABI"
    }

    $probeDir = Join-Path $BuildRoot "arm-cm55-mvef-mpu-svc"
    ...
    foreach ($regions in @(8, 16)) {
        ...
        foreach ($mode in $modes) {
            ...
            if ($LASTEXITCODE -ne 0) {
                throw "ARM_CM55_MVEF_MPU SVC boot compile failed ($regions/$($mode.Name))"
            }
            & $Compiler @($baseArgs + @("-save-temps=obj", "-c", $svcSource,
                "-o", $svcObject))
            if ($LASTEXITCODE -ne 0) {
                throw "ARM_CM55_MVEF_MPU SVC source compile failed ($regions/$($mode.Name))"
            }
			& $Compiler @($baseArgs + @("-c", $pendSvSource, "-o", $pendSvObject))
			if ($LASTEXITCODE -ne 0) {
				throw "ARM_CM55_MVEF_MPU SVC PendSV component compile failed ($regions/$($mode.Name))"
			}
            & $Compiler @($baseArgs + @("-c", $fixture, "-o", $fixtureObject))
            if ($LASTEXITCODE -ne 0) {
                throw "ARM_CM55_MVEF_MPU SVC fixture compile failed ($regions/$($mode.Name))"
            }
            ...
            foreach ($symbol in $requiredSymbols) {
                if (-not ($svcDefined -match
                    ...
                    throw "ARM_CM55_MVEF_MPU SVC lost strong symbol: $symbol / $regions/$($mode.Name)"
                }
            }
            if ($svcDefined -match '\bPendSV_Handler$') {
                throw "ARM_CM55_MVEF_MPU SVC object emitted PendSV handler ownership: $regions/$($mode.Name)"
            }
            if (-not $mode.Lto) {
                ...
                foreach ($symbol in @(
                    ...
                    if (-not ($symbols -match
                        ...
                        throw "ARM_CM55_MVEF_MPU SVC escaped privileged flash: $symbol / $regions/$($mode.Name)"
                    }
                }
                if (-not ($symbols -match
                        "\bF\s+\.fiber_port_syscalls\s+[0-9a-fA-F]+\s+fiber_port_unprivileged_task_return$")) {
                    throw "ARM_CM55_MVEF_MPU task-return veneer escaped syscall flash: $regions/$($mode.Name)"
                }
				if (-not ($symbols -match
						"\bF\s+\.fiber_port_syscalls\s+[0-9a-fA-F]+\s+fiber_port_runtime_schedule$")) {
					throw "ARM_CM55_MVEF_MPU runtime schedule veneer escaped syscall flash: $regions/$($mode.Name)"
				}
				if (-not ($symbols -match
						"\bF\s+\.fiber_port_unprivileged_functions\s+[0-9a-fA-F]+\s+fiber_port_runtime_memory_barrier$")) {
					throw "ARM_CM55_MVEF_MPU runtime memory barrier escaped unprivileged flash: $regions/$($mode.Name)"
				}
                ...
                foreach ($pattern in @(
                        '(?im)\bsvc\s+70\b', '(?im)\bsvc\s+71\b', '(?im)\bsvc\s+72\b',
                        '(?im)0xe000ed08', '(?im)0xe000ed94',
                        '(?im)0xe000ed98', '(?im)0xe000ed9c',
                        '(?im)0xe000edc0', '(?im)\bmsr\s+psp',
                        '(?im)\bmsr\s+psplim', '(?im)\bmsr\s+control',
                        '(?im)\bmsr\s+basepri', '(?im)\bmov\s+lr,\s*r1', '(?im)\bmvn(?:\.w)?\s+r5,\s*#71',
                        '(?im)\bmvn(?:\.w)?\s+r5,\s*#67',
                        '(?im)\bldmia(?:\.w)?\s+r0!?', '(?im)\bstmia(?:\.w)?\s+r2',
                        '(?im)\bldmdb\s+r1!', '(?im)\bbx\s+lr')) {
                    if ($disassembly -notmatch $pattern) {
                        throw "ARM_CM55_MVEF_MPU SVC generated assembly lost $pattern / $regions/$($mode.Name)"
                    }
                }
                if ($regions -eq 16) {
                    foreach ($pattern in @(
                        ...
                        if ($disassembly -notmatch $pattern) {
                            throw "ARM_CM55_MVEF_MPU 16-region SVC lost $pattern / $($mode.Name)"
                        }
                    }
                }
                foreach ($forbidden in @(
                    ...
                    if ($disassembly -match $forbidden) {
                        throw "ARM_CM55_MVEF_MPU SVC/runtime generated forbidden mechanism: $forbidden / $regions/$($mode.Name)"
                    }
                }
            }
            ...
            & $Compiler @($baseArgs + @(
                ...
                "-o", $elf))
            if ($LASTEXITCODE -ne 0) {
                throw "ARM_CM55_MVEF_MPU SVC synthetic ELF failed link ($regions/$($mode.Name))"
            }
            $elfDefined = @(& $objectNm -g --defined-only $elf)
            foreach ($symbol in @($requiredSymbols + @(
                    "fiber_arm_cm55_mvef_mpu_svc_probe"))) {
                if (-not ($elfDefined -match
                        "\bT\s+$([regex]::Escape($symbol))$")) {
                    throw "ARM_CM55_MVEF_MPU SVC ELF lost symbol: $symbol / $regions/$($mode.Name)"
                }
            }
            foreach ($handler in @("SVC_Handler", "PendSV_Handler")) {
				...
				if ($handlerSymbols.Count -ne 1) {
					throw "ARM_CM55_MVEF_MPU runtime ELF must retain one strong ${handler}: $regions/$($mode.Name)"
				}
			}
            ...
            if ($LASTEXITCODE -ne 0) {
                throw "ARM_CM55_MVEF_MPU SVC vector extraction failed: $regions/$($mode.Name)"
            }
            $vector = [IO.File]::ReadAllBytes($vectorBinary)
            if ($vector.Length -lt (15 * 4)) {
                throw "ARM_CM55_MVEF_MPU SVC vector fixture is truncated: $regions/$($mode.Name)"
            }
            if ([BitConverter]::ToUInt32($vector, 11 * 4) -ne
                    ($svcAddress -bor [uint32]1)) {
                throw "ARM_CM55_MVEF_MPU SVC vector slot 11 lost strong handler: $regions/$($mode.Name)"
            }
            if ([BitConverter]::ToUInt32($vector, 14 * 4) -ne
                    ($pendSvAddress -bor [uint32]1)) {
                throw "ARM_CM55_MVEF_MPU runtime vector slot 14 lost strong handler: $regions/$($mode.Name)"
            }
            ...
            if (($regions -eq 8) -and ($mode.Name -eq "hard-o2")) {
                ...
                if (($duplicate.ExitCode -eq 0) -or
                    ...
                    throw "ARM_CM55_MVEF_MPU competing SVC handlers must fail link`n$($duplicate.Output)"
                }
            }
        }
    }
