function Test-ArmCm55fMpuSlice1Contract {
    param(
    ...
    $layoutFixture = Join-Path $RepositoryRoot `
        "tools\fixtures\arm_cm55f_mpu_layout_probe.c"
    $bootFixture = Join-Path $RepositoryRoot `
        "tools\fixtures\arm_cm55f_mpu_boot_probe.c"
    $fixtureLinker = Join-Path $RepositoryRoot `
        "tools\fixtures\arm_cm55f_mpu_boot.ld"
    ...
    if (Compare-Object -ReferenceObject $expectedFiles `
        ...
        throw "ARM_CM55F_MPU slice-1 file surface changed"
    }
    foreach ($required in @($bootSource, $linkerContract, $layoutFixture,
            $bootFixture, $fixtureLinker)) {
        if (-not (Test-Path -LiteralPath $required)) {
            throw "ARM_CM55F_MPU slice-1 is missing: $required"
        }
    }
    foreach ($forbidden in @(
        ...
        if (Test-Path -LiteralPath (Join-Path $profileDir $forbidden)) {
            throw "ARM_CM55F_MPU slice-1 exposed deferred artifact: $forbidden"
        }
    }
    ...
    foreach ($selector in @(
        ...
        if ((Get-Content -LiteralPath $selector -Raw).IndexOf(
            ...
            throw "ARM_CM55F_MPU slice-1 must not enter global selection: $selector"
        }
    }
    ...
    foreach ($forbiddenTypeDependency in @(
        ...
        if ([regex]::IsMatch($typeText, $includePattern)) {
            throw "ARM_CM55F_MPU public storage acquired CPU dependency: $forbiddenTypeDependency"
        }
    }

    $macro = Get-Content -LiteralPath (Join-Path $profileDir "fiber_portmacro.h") -Raw
    foreach ($token in @(
            "FIBER_PORT_CM55F_MPU_TOTAL_REGIONS",
            ...
            "FIBER_PORT_CONTEXT_ABI_PORT_ID 0x43353550u",
            "FIBER_PORT_CM55F_MPU_STACK_REQUIRED_BYTES")) {
        if ($macro.IndexOf($token, [System.StringComparison]::Ordinal) -lt 0) {
            throw "ARM_CM55F_MPU portmacro lost frozen trait: $token"
        }
    }
    ...
    foreach ($token in @(
        ...
        if ($bootText.IndexOf($token, [System.StringComparison]::Ordinal) -lt 0) {
            throw "ARM_CM55F_MPU construction lost required mechanism: $token"
        }
    }
    foreach ($forbidden in @(
            "void SVC_Handler(",
        ...
        if ($bootText.IndexOf($forbidden, [System.StringComparison]::Ordinal) -ge 0) {
            throw "ARM_CM55F_MPU construction acquired runtime ownership: $forbidden"
        }
    }
    ...
    foreach ($token in @(
        ...
        if ($parity.IndexOf($token, [System.StringComparison]::Ordinal) -lt 0) {
            throw "ARM_CM55F_MPU parity ledger lost evidence: $token"
        }
    }

    $probeDir = Join-Path $BuildRoot "arm-cm55f-mpu-slice1"
...
#endif
FiberContext fiber_arm_cm55f_mpu_type_only_object;
'@
...
#endif
FiberContext fiber_arm_cm55f_mpu_cpp_type_only_object;
'@
    ...
    foreach ($regions in @(8, 16)) {
        ...
        if ($LASTEXITCODE -ne 0) {
            throw "ARM_CM55F_MPU type-only C header failed ($regions regions)"
        }
        $cppObject = Join-Path $regionDir "type-only-cpp.o"
        & $Compiler -x c++ "-march=armv8.1-m.main+fp" -mthumb -mfloat-abi=hard `
            -std=c++17 -fno-exceptions -fno-rtti @cppWarningArgs `
            "-I$profileDir" "-DFIBER_PORT_CM55F_MPU_TOTAL_REGIONS=$regions" `
        ...
        foreach ($mode in $modes) {
            ...
            & $Compiler @($baseArgs + @("-c", $layoutFixture,
                "-o", $layoutObject))
            if ($LASTEXITCODE -ne 0) {
                throw "ARM_CM55F_MPU layout compile failed ($regions/$($mode.Name))"
            }
            ...
            if ($layoutCohorts.Count -ne 1) {
                throw "ARM_CM55F_MPU layout must define one exact cohort ($regions/$($mode.Name))"
            }
            foreach ($token in @(
                    "armv81m_mainline",
                    "p0x43353550u",
                ...
                if ($layoutCohorts[0].IndexOf($token,
                    ...
                    throw "ARM_CM55F_MPU exact cohort lost token ${token} ($regions/$($mode.Name)): $($layoutCohorts[0])"
                }
            }
            if (($null -ne $expectedCohort) -and
                    ($expectedCohort -ne $layoutCohorts[0])) {
                throw "ARM_CM55F_MPU cohort changed across ABI/optimization modes ($regions)"
            }
            ...
            & $Compiler @($baseArgs + @("-save-temps=obj", "-c", $bootSource,
                "-o", $bootObject))
            if ($LASTEXITCODE -ne 0) {
                throw "ARM_CM55F_MPU construction compile failed ($regions/$($mode.Name))"
            }
            $bootDefined = @(& $GccNm -g --defined-only $bootObject)
            foreach ($symbol in $requiredSymbols) {
                if (-not ($bootDefined -match
                        "\bT\s+$([regex]::Escape($symbol))$")) {
                    throw "ARM_CM55F_MPU construction lost strong symbol: $symbol / $regions/$($mode.Name)"
                }
            }
            ...
            if (($bootCohorts.Count -ne 1) -or
                    ($bootCohorts[0] -ne $expectedCohort)) {
                throw "ARM_CM55F_MPU construction must define the layout cohort ($regions/$($mode.Name))"
            }
            if ($bootDefined -match '\b(?:SVC_Handler|PendSV_Handler|fiber_port_runtime_)') {
                throw "ARM_CM55F_MPU construction object emitted runtime ownership ($regions/$($mode.Name))"
            }
            ...
            if (($allowedUndefined.Count -ne $actualUndefined.Count) -or
                ...
                throw "ARM_CM55F_MPU construction dependency surface changed ($regions/$($mode.Name))`nExpected: $($allowedUndefined -join ', ')`nActual: $($actualUndefined -join ', ')"
            }
            ...
            if (-not $mode.Lto) {
                ...
                foreach ($symbol in $requiredSymbols) {
                    ...
                    if ($sectionMatch.Count -ne 1) {
                        throw "ARM_CM55F_MPU construction escaped privileged section: $symbol / $regions/$($mode.Name)"
                    }
                }
                ...
                foreach ($forbiddenInstruction in @(
                    ...
                    if ($assembly -match $forbiddenInstruction) {
                        throw "ARM_CM55F_MPU construction generated forbidden transfer/state write: $forbiddenInstruction / $regions/$($mode.Name)"
                    }
                }
            }

            $fixtureObject = Join-Path $modeDir "probe.o"
            & $Compiler @($baseArgs + @("-c", $bootFixture,
                "-o", $fixtureObject))
            if ($LASTEXITCODE -ne 0) {
                throw "ARM_CM55F_MPU construction fixture compile failed ($regions/$($mode.Name))"
            }
            $elf = Join-Path $modeDir "construction.elf"
            & $Compiler @($baseArgs + @(
                "-nostdlib", "-Wl,--gc-sections", "-Wl,-T,$fixtureLinker",
                "-Wl,-T,$linkerContract", $bootObject, $fixtureObject,
                "-o", $elf))
            if ($LASTEXITCODE -ne 0) {
                throw "ARM_CM55F_MPU construction/linker proof failed ($regions/$($mode.Name))"
            }
            $elfDefined = @(& $GccNm -g --defined-only $elf)
            foreach ($symbol in @($requiredSymbols + @(
                    "fiber_arm_cm55f_mpu_boot_probe",
                    "fiber_port_unprivileged_task_return",
                    $expectedCohort))) {
                if (-not ($elfDefined -match
                        "\b[TR]\s+$([regex]::Escape($symbol))$")) {
                    throw "ARM_CM55F_MPU synthetic ELF lost symbol: $symbol / $regions/$($mode.Name)"
                }
            }
            if ($elfDefined -match '\b(?:SVC_Handler|PendSV_Handler)$') {
                throw "ARM_CM55F_MPU construction ELF unexpectedly owns handlers ($regions/$($mode.Name))"
            }
            ...
            foreach ($section in @(
                ...
                if ($sections.IndexOf($section,
                        [System.StringComparison]::Ordinal) -lt 0) {
                    throw "ARM_CM55F_MPU synthetic ELF lost required section: $section / $regions/$($mode.Name)"
                }
            }
            ...
            if ($mode.Name -eq "hard-o2") {
                ...
                if (($negative.ExitCode -eq 0) -or
                        ...
                        [System.StringComparison]::Ordinal) -lt 0)) {
                    throw "ARM_CM55F_MPU linker contract accepted a missing syscall boundary ($regions/$($mode.Name))`n$($negative.Output)"
                }
            }
        }
    }
    ...
    $invalidCases = @(
        [pscustomobject]@{ Name = "missing-fpu"; CpuArgs = @("-march=armv8.1-m.main+fp", "-mfloat-abi=hard"); Defines = @("-DFIBER_TEST_FPU_PRESENT=0"); Diagnostic = "requires CMSIS __FPU_PRESENT == 1" },
        [pscustomobject]@{ Name = "fpu-not-used"; CpuArgs = @("-march=armv8.1-m.main+fp", "-mfloat-abi=hard"); Defines = @("-DFIBER_TEST_FPU_USED=0"); Diagnostic = "requires CMSIS __FPU_USED == 1" },
        [pscustomobject]@{ Name = "no-fp"; CpuArgs = @("-march=armv8.1-m.main", "-mfloat-abi=soft"); Defines = @(); Diagnostic = "requires compiler FP code generation" },
        [pscustomobject]@{ Name = "mve"; CpuArgs = @("-march=armv8.1-m.main+mve.fp", "-mfloat-abi=hard"); Defines = @(); Diagnostic = "does not permit MVE" },
        [pscustomobject]@{ Name = "secure-cmse"; CpuArgs = @("-march=armv8.1-m.main+fp", "-mfloat-abi=hard", "-mcmse"); Defines = @("-DFIBER_TEST_SECURE_CMSE=3"); Diagnostic = "selected profile excludes Secure CMSE builds" },
        [pscustomobject]@{ Name = "wrong-core"; CpuArgs = @("-march=armv8.1-m.main+fp", "-mfloat-abi=hard"); Defines = @("-DFIBER_TEST_CORTEX_M=33"); Diagnostic = "CMSIS __CORTEX_M == 55" },
        [pscustomobject]@{ Name = "invalid-regions"; CpuArgs = @("-march=armv8.1-m.main+fp", "-mfloat-abi=hard"); Defines = @("-DFIBER_PORT_CM55F_MPU_TOTAL_REGIONS=12"); Diagnostic = "supports exactly 8 or 16 MPU regions" },
        [pscustomobject]@{ Name = "runtime-selectable"; CpuArgs = @("-march=armv8.1-m.main+fp", "-mfloat-abi=hard"); Defines = @("-DFIBER_PORT_RUNTIME_SELECTABLE=1"); Diagnostic = "runtime-selectable state must not be predefined" }
    )
    foreach ($case in $invalidCases) {
        ...
        if (($result.ExitCode -eq 0) -or
                ...
                [System.StringComparison]::Ordinal) -lt 0)) {
            throw "Invalid ARM_CM55F_MPU manifest failed for the wrong reason: $($case.Name)`n$($result.Output)"
        }
    }
}
