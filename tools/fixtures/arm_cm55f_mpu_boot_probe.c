...
FIBER_STATIC_ASSERT(fiber_portMPU_CONTEXT_STACK_INDEX == 0u,
		"ARM_CM55F_MPU stack pair must remain context index zero");
FIBER_STATIC_ASSERT(fiber_portMPU_STACK_REGION_NUMBER == 4u,
		"ARM_CM55F_MPU stack must program hardware region four");
FIBER_STATIC_ASSERT(fiber_portMPU_CURRENT_CONTEXT_REGION_NUMBER == 5u &&
		fiber_portMPU_CONTEXT_CURRENT_CONTEXT_INDEX == 1u,
		"ARM_CM55F_MPU current-slot aperture must occupy the second context pair");
FIBER_STATIC_ASSERT(fiber_portPROTECTED_CONTEXT_WORDS == 54u &&
		...
		"ARM_CM55F_MPU protected scalar-FP context geometry changed");
FIBER_STATIC_ASSERT(sizeof(FiberPortProtectedContext) == 216u,
		"ARM_CM55F_MPU protected scalar-FP image size changed");

fiber_portPRIVILEGED_DATA
static FiberContext fiber_arm_cm55f_mpu_probe_context;

__attribute__((section(".fiber_test_unprivileged_ram"), aligned(32)))
static unsigned char fiber_arm_cm55f_mpu_probe_stack[2048];

fiber_portUNPRIVILEGED_FUNCTION
static void fiber_arm_cm55f_mpu_probe_entry(void *arg)
{
	(void)arg;
}
...
fiber_portPRIVILEGED_FUNCTION
int fiber_arm_cm55f_mpu_boot_probe(void)
{
	...
	fiber_port_context_init(&fiber_arm_cm55f_mpu_probe_context,
			fiber_arm_cm55f_mpu_probe_stack,
			fiber_arm_cm55f_mpu_probe_stack +
				sizeof(fiber_arm_cm55f_mpu_probe_stack),
			fiber_arm_cm55f_mpu_probe_entry, (void *)0);
	fiber_port_context_validate_initial_restore(
			&fiber_arm_cm55f_mpu_probe_context);
	...
	return (int)(global_regions.regions[
			...
			pxn_region.rlar |
			fiber_arm_cm55f_mpu_probe_context.boot.hash |
			(uint32_t)(uintptr_t)
			fiber_internal_runtime_current_context_slot);
}
