...
static FiberContext fiber_arm_cm55f_mpu_pendsv_context;
...
static unsigned char fiber_arm_cm55f_mpu_pendsv_stack[2048];

fiber_portUNPRIVILEGED_FUNCTION
static void fiber_arm_cm55f_mpu_pendsv_entry(void *arg)
{
	(void)arg;
}
...
{
	return current != NULL ? current : &fiber_arm_cm55f_mpu_pendsv_context;
}
...
const uintptr_t fiber_arm_cm55f_mpu_pendsv_vectors[16] = {
...
FIBER_API_ATTR_SENSITIVE FIBER_GENERAL_REGS_ONLY
fiber_portPRIVILEGED_FUNCTION
int fiber_arm_cm55f_mpu_pendsv_probe(void)
{
	fiber_port_context_init(&fiber_arm_cm55f_mpu_pendsv_context,
			fiber_arm_cm55f_mpu_pendsv_stack,
			fiber_arm_cm55f_mpu_pendsv_stack +
				sizeof(fiber_arm_cm55f_mpu_pendsv_stack),
			fiber_arm_cm55f_mpu_pendsv_entry, (void *)0);
	fiber_internal_runtime_publish_current_context(
			&fiber_arm_cm55f_mpu_pendsv_context);
	fiber_port_prepare_first_start(&fiber_arm_cm55f_mpu_pendsv_context);
	return (int)(fiber_arm_cm55f_mpu_pendsv_context.mair0 |
