...
static FiberContext fiber_arm_cm55_mvef_mpu_svc_context;
...
static unsigned char fiber_arm_cm55_mvef_mpu_svc_stack[2048];

fiber_portUNPRIVILEGED_FUNCTION
static void fiber_arm_cm55_mvef_mpu_svc_entry(void *arg)
{
	(void)arg;
}
...
{
	return current != NULL ? current : &fiber_arm_cm55_mvef_mpu_svc_context;
}
...
const uintptr_t fiber_arm_cm55_mvef_mpu_svc_vectors[16] = {
...
FIBER_API_ATTR_SENSITIVE FIBER_GENERAL_REGS_ONLY
fiber_portPRIVILEGED_FUNCTION
int fiber_arm_cm55_mvef_mpu_svc_probe(void)
{
	fiber_port_context_init(&fiber_arm_cm55_mvef_mpu_svc_context,
			fiber_arm_cm55_mvef_mpu_svc_stack,
			fiber_arm_cm55_mvef_mpu_svc_stack +
				sizeof(fiber_arm_cm55_mvef_mpu_svc_stack),
			fiber_arm_cm55_mvef_mpu_svc_entry, (void *)0);
	fiber_internal_runtime_current_context_slot =
			&fiber_arm_cm55_mvef_mpu_svc_context;
	fiber_port_prepare_first_start(&fiber_arm_cm55_mvef_mpu_svc_context);
	return (int)(fiber_arm_cm55_mvef_mpu_svc_context.mair0 |
