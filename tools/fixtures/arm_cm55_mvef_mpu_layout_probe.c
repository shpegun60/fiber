...
FIBER_STATIC_ASSERT(FIBER_PORT_RUNTIME_SELECTABLE == 0,
		"[fiber]: ARM_CM55_MVEF_MPU construction slice must not expose runtime operations");
FIBER_STATIC_ASSERT(fiber_portPROTECTED_CONTEXT_WORDS == 54u,
		"[fiber]: ARM_CM55_MVEF_MPU protected context word count changed");
FIBER_STATIC_ASSERT(sizeof(FiberPortProtectedContext) == 216u,
		"[fiber]: ARM_CM55_MVEF_MPU protected context size changed");
FIBER_STATIC_ASSERT(sizeof(FiberPortBoot) == 88u,
		"[fiber]: ARM_CM55_MVEF_MPU boot record size changed");

/* FreeRTOS ARM_CM55_NTZ MPU/FPU/MVE uses two intentional views of one 54-word
...
FIBER_STATIC_ASSERT(offsetof(FiberPortProtectedBasicContext, r4) == 0u &&
		...
		"[fiber]: ARM_CM55_MVEF_MPU basic FreeRTOS cursor view changed");
FIBER_STATIC_ASSERT(offsetof(FiberPortProtectedExtendedContext,
		...
		"[fiber]: ARM_CM55_MVEF_MPU extended FreeRTOS cursor view changed");
FIBER_STATIC_ASSERT(fiber_portPROTECTED_BASIC_RESTORE_WORDS == 20u &&
		...
		"[fiber]: ARM_CM55_MVEF_MPU FreeRTOS FPU save geometry changed");
FIBER_STATIC_ASSERT(FIBER_PORT_SOFTWARE_FRAME_WORDS == 20u &&
		...
		"[fiber]: ARM_CM55_MVEF_MPU generic protected-frame geometry changed");

FIBER_STATIC_ASSERT(FIBER_PORT_HAS_FPU == 1 &&
		FIBER_PORT_HAS_EXTENDED_FP_CONTEXT == 1 &&
		FIBER_PORT_HAS_MVE == 1 && FIBER_PORT_HAS_SECURE_CONTEXT_SLOT == 0 &&
		FIBER_PORT_RUNS_NONSECURE == 1,
		"[fiber]: ARM_CM55_MVEF_MPU selected feature profile changed");
FIBER_STATIC_ASSERT(FIBER_PORT_CONTEXT_ABI_PORT_ID == 0x43353557u,
		"[fiber]: ARM_CM55_MVEF_MPU exact port identity changed");
...
FIBER_STATIC_ASSERT(FIBER_PORT_CM55_MVEF_MPU_PSP_INITIAL_FRAME_BYTES == 32u &&
		...
		"[fiber]: ARM_CM55_MVEF_MPU physical PSP admission changed");
FIBER_STATIC_ASSERT(fiber_portMPU_CURRENT_CONTEXT_REGION_NUMBER == 5u &&
		...
		"[fiber]: ARM_CM55_MVEF_MPU current-slot MPU policy changed");

#if FIBER_PORT_CM55_MVEF_MPU_TOTAL_REGIONS == 8
FIBER_STATIC_ASSERT(FIBER_PORT_CM55_MVEF_MPU_CONTEXT_REGION_COUNT == 4u &&
		...
		"[fiber]: ARM_CM55_MVEF_MPU 8-region context layout changed");
#else
FIBER_STATIC_ASSERT(FIBER_PORT_CM55_MVEF_MPU_CONTEXT_REGION_COUNT == 12u &&
		offsetof(FiberContext, protected_context_cursor) == 0u &&
		offsetof(FiberContext, mair0) == 4u &&
		offsetof(FiberContext, mpu_regions) == 8u &&
		...
		"[fiber]: ARM_CM55_MVEF_MPU 16-region context layout changed");
#endif

FIBER_STATIC_ASSERT(alignof(FiberContext) == 8u,
		"[fiber]: ARM_CM55_MVEF_MPU context alignment changed");

int fiber_arm_cm55_mvef_mpu_layout_probe(void)
{
