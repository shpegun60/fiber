function Test-ArmCm55MvefMpuPendSvContract {
    param(
    ...
    $fixture = Join-Path $RepositoryRoot "tools\fixtures\arm_cm55_mvef_mpu_pendsv_probe.c"
    $fixtureLinker = Join-Path $RepositoryRoot "tools\fixtures\arm_cm55_mvef_mpu_pendsv.ld"
    $competing = Join-Path $RepositoryRoot "tools\fixtures\arm_cm55_mvef_mpu_competing_pendsv.c"

    foreach ($required in @($bootSource, $svcSource, $pendSvSource,
            $privateHeader, $linkerContract, $fixture, $fixtureLinker,
            $competing)) {
        if (-not (Test-Path -LiteralPath $required)) {
            throw "ARM_CM55_MVEF_MPU protected PendSV slice is missing: $required"
        }
    }
    foreach ($selector in @(
        ...
        if ((Get-Content -LiteralPath $selector -Raw).IndexOf(
            ...
            throw "ARM_CM55_MVEF_MPU PendSV slice must remain explicitly build-selected: $selector"
        }
    }
    ...
    foreach ($requiredText in @(
        ...
        if ($pendSvText.IndexOf($requiredText,
            ...
            throw "ARM_CM55_MVEF_MPU protected PendSV source lost mechanism: $requiredText"
        }
    }
    foreach ($forbiddenText in @(
        ...
        if ($pendSvText.IndexOf($forbiddenText,
            ...
            throw "ARM_CM55_MVEF_MPU protected PendSV acquired deferred forward ABI ownership: $forbiddenText"
        }
    }
    ...
    if (($preflightIndex -lt 0) -or ($cursorLoadIndex -lt 0) -or
        ...
        throw "ARM_CM55_MVEF_MPU PendSV must validate current before loading its protected cursor"
    }

    $privateText = Get-Content -LiteralPath $privateHeader -Raw
    if ($privateText.IndexOf("fiber_runtime_port_abi.h",
            [System.StringComparison]::Ordinal) -lt 0) {
        throw "ARM_CM55_MVEF_MPU PendSV must consume frozen reverse ABI v1"
    }
    if ($privateText.IndexOf("fiber_port_runtime_abi.h",
            [System.StringComparison]::Ordinal) -lt 0) {
        throw "ARM_CM55_MVEF_MPU selected runtime must expose the frozen forward ABI to all private objects"
    }

    $fixtureText = Get-Content -LiteralPath $fixture -Raw
    if ($fixtureText.IndexOf("void fiber_schedule(void)",
            [System.StringComparison]::Ordinal) -lt 0 -or
            $fixtureText.IndexOf("fiber_port_runtime_schedule();",
            [System.StringComparison]::Ordinal) -lt 0) {
        throw "ARM_CM55_MVEF_MPU PendSV fixture must expose the selected public yield veneer"
    }

    $probeDir = Join-Path $BuildRoot "arm-cm55-mvef-mpu-pendsv"
    ...
    foreach ($regions in @(8, 16)) {
        ...
        foreach ($mode in $modes) {
            ...
            if ($LASTEXITCODE -ne 0) {
                throw "ARM_CM55_MVEF_MPU PendSV boot compile failed ($regions/$($mode.Name))"
            }
            & $Compiler @($baseArgs + @("-c", $svcSource, "-o", $svcObject))
            if ($LASTEXITCODE -ne 0) {
                throw "ARM_CM55_MVEF_MPU PendSV SVC support compile failed ($regions/$($mode.Name))"
            }
            $saveTemps = if ($mode.Lto) { @() } else { @("-save-temps=obj") }
            & $Compiler @($baseArgs + $saveTemps + @("-c", $pendSvSource,
                "-o", $pendSvObject))
            if ($LASTEXITCODE -ne 0) {
                throw "ARM_CM55_MVEF_MPU protected PendSV compile failed ($regions/$($mode.Name))"
            }
            & $Compiler @($baseArgs + @("-c", $fixture, "-o", $fixtureObject))
            if ($LASTEXITCODE -ne 0) {
                throw "ARM_CM55_MVEF_MPU PendSV fixture compile failed ($regions/$($mode.Name))"
            }
            ...
            foreach ($symbol in $requiredPendSvSymbols) {
                if (-not ($pendSvDefined -match
                    ...
                    throw "ARM_CM55_MVEF_MPU PendSV lost strong symbol: $symbol / $regions/$($mode.Name)"
                }
            }
            if ($pendSvDefined -match '\bfiber_port_runtime_') {
                throw "ARM_CM55_MVEF_MPU PendSV object emitted deferred forward runtime ownership: $regions/$($mode.Name)"
            }
            ...
            if (-not $mode.Lto) {
                ...
                foreach ($symbol in $requiredPendSvSymbols) {
                    if (-not ($symbols -match
                        ...
                        throw "ARM_CM55_MVEF_MPU PendSV escaped privileged flash: $symbol / $regions/$($mode.Name)"
                    }
                }
				...
				foreach ($requiredRelocation in @(
					...
					if (-not ($relocations -match [regex]::Escape($requiredRelocation))) {
						throw "ARM_CM55_MVEF_MPU PendSV lost retained link identity: $requiredRelocation / $regions/$($mode.Name)"
					}
				}
                ...
                foreach ($pattern in $requiredAssemblyPatterns) {
                    if ($disassembly -notmatch $pattern) {
                        throw "ARM_CM55_MVEF_MPU PendSV generated assembly lost $pattern / $regions/$($mode.Name)"
                    }
                }
                foreach ($forbidden in @(
                    ...
                    if ($disassembly -match $forbidden) {
                        throw "ARM_CM55_MVEF_MPU PendSV generated forbidden mechanism: $forbidden / $regions/$($mode.Name)"
                    }
                }
            }
            ...
            if ($LASTEXITCODE -ne 0) {
                throw "ARM_CM55_MVEF_MPU PendSV synthetic ELF failed link ($regions/$($mode.Name))"
            }
            $elfDefined = @(& $objectNm -g --defined-only $elf)
            foreach ($symbol in @($requiredPendSvSymbols + @(
                    "SVC_Handler", "fiber_arm_cm55_mvef_mpu_pendsv_probe"))) {
                if (-not ($elfDefined -match
                        "\bT\s+$([regex]::Escape($symbol))$")) {
                    throw "ARM_CM55_MVEF_MPU PendSV ELF lost symbol: $symbol / $regions/$($mode.Name)"
                }
            }
            foreach ($handler in @("SVC_Handler", "PendSV_Handler")) {
                ...
                if ($handlers.Count -ne 1) {
                    throw "ARM_CM55_MVEF_MPU PendSV ELF must retain one strong ${handler}: $regions/$($mode.Name)"
                }
            }
            ...
            if ($LASTEXITCODE -ne 0) {
                throw "ARM_CM55_MVEF_MPU PendSV vector extraction failed: $regions/$($mode.Name)"
            }
            $vector = [IO.File]::ReadAllBytes($vectorBinary)
            if ($vector.Length -lt (15 * 4)) {
                throw "ARM_CM55_MVEF_MPU PendSV vector fixture is truncated: $regions/$($mode.Name)"
            }
            foreach ($vectorExpectation in @(
                ...
                if ([BitConverter]::ToUInt32($vector,
                    ...
                    throw "ARM_CM55_MVEF_MPU PendSV vector slot $($vectorExpectation.Index) lost $($vectorExpectation.Symbol): $regions/$($mode.Name)"
                }
            }
            ...
            if (($regions -eq 8) -and ($mode.Name -eq "hard-o2")) {
                ...
                & $Compiler @($baseArgs + @("-c", $competing,
                    "-o", $competingObject))
                if ($LASTEXITCODE -ne 0) {
                    throw "ARM_CM55_MVEF_MPU competing PendSV fixture failed compile"
                }
                ...
                if (($duplicate.ExitCode -eq 0) -or
                    ...
                    throw "ARM_CM55_MVEF_MPU competing PendSV handlers must fail link`n$($duplicate.Output)"
                }
            }
        }
    }
