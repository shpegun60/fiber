...
function Test-ContextPortBoundary {
    ...
    $slotOwnerSources = @($portSources) + @(
        ...
        "fiber\port\ARM_CM55_NTZ\non_secure\fiber_port.c",
        "fiber\port\ARM_CM33_TFM\non_secure\fiber_port.c",
		...
		"fiber\port\ARM_CM55_MVEF_NTZ\non_secure\fiber_port.c",
		"fiber\port\ARM_CM55F_MPU\non_secure\fiber_port_svc.c",
		"fiber\port\ARM_CM55F_MPU\non_secure\fiber_port_pendsv.c",
...
function Test-ArmCm33MpuLayoutContract {
    ...
    foreach ($forbiddenArtifact in @(
        ...
        if (Test-Path -LiteralPath (Join-Path $profileDir $forbiddenArtifact)) {
            throw "ARM_CM33_MPU selected layout exposed forbidden artifact: $forbiddenArtifact"
        }
    }
    ...
    foreach ($selector in @(
        ...
        if ((Get-Content -LiteralPath $selector -Raw).IndexOf(
            ...
            throw "ARM_CM33_MPU selected layout must not enter global selection: $selector"
        }
    }
    ...
    foreach ($requiredParity in @(
            ...
            "the protected SVC/PendSV runtime compiles for 8 and 16 regions",
            "The original SVC")) {
        if ($parity.IndexOf($requiredParity,
                [System.StringComparison]::Ordinal) -lt 0) {
...
'@
    ...
    foreach ($regions in @(8, 16)) {
        ...
        foreach ($optimization in @("-O2", "-Os")) {
            ...
            throw "ARM_CM33_MPU type/layout probe emitted runtime symbols ($regions regions/$mode): $($runtimeDefinitions -join ', ')"
            }
...
function Test-ArmCm33MpuProtectedRuntimeContract {
    param(
        [string]$RepositoryRoot,
        [string]$Compiler,
        [string]$Nm,
    ...
    $competingPendSv = Join-Path $RepositoryRoot `
            "tools\fixtures\arm_cm33_mpu_competing_pendsv.c"
    ...
    foreach ($requiredText in @(
        ...
        if ($runtimeText.IndexOf($requiredText,
                [System.StringComparison]::Ordinal) -lt 0) {
            throw "ARM_CM33_MPU protected runtime source lost required mechanism: $requiredText"
        }
    }
    $privateText = Get-Content -LiteralPath $privateHeader -Raw
    if ($privateText.IndexOf("fiber_port_runtime_abi.h",
            [System.StringComparison]::Ordinal) -lt 0) {
        throw "ARM_CM33_MPU selected runtime must consume the frozen forward ABI"
    }
    if ($privateText.IndexOf("fiber_runtime_port_abi.h",
            [System.StringComparison]::Ordinal) -lt 0) {
        throw "ARM_CM33_MPU protected PendSV must consume frozen reverse ABI v1"
    }
    if ($privateText.IndexOf("fiber_api_decl.h",
            [System.StringComparison]::Ordinal) -lt 0) {
        throw "ARM_CM33_MPU runtime linker audit must consume public Thread API declarations"
    }
    ...
    foreach ($selector in @(
        ...
        if ((Get-Content -LiteralPath $selector -Raw).IndexOf(
                "ARM_CM33_MPU", [System.StringComparison]::Ordinal) -ge 0) {
            throw "ARM_CM33_MPU build-selected runtime entered global selection: $selector"
        }
    }
...
"@
    ...
    foreach ($regions in @(8, 16)) {
        ...
        foreach ($modeSpec in @(
            ...
            if ($LASTEXITCODE -ne 0) {
                throw "ARM_CM33_MPU protected runtime source failed compile ($regions/$mode)"
            }
            & $Compiler @($baseArgs + @("-c", $bootSource, "-o", $bootObject))
            if ($LASTEXITCODE -ne 0) {
                throw "ARM_CM33_MPU protected runtime boot source failed compile ($regions/$mode)"
            }
            & $Compiler @($baseArgs + @("-c", $fixture, "-o", $fixtureObject))
            if ($LASTEXITCODE -ne 0) {
                throw "ARM_CM33_MPU protected runtime fixture failed compile ($regions/$mode)"
            }
            ...
            foreach ($symbol in $requiredRuntimeSymbols) {
                if (-not ($runtimeDefined -match
                    ...
                    throw "ARM_CM33_MPU protected runtime object lost strong symbol: $symbol / $regions/$mode"
                }
            }
            if ($modeSpec.Lto -eq 0) {
                ...
                foreach ($symbol in @(
                    ...
                    if ($runtimeSymbols -notmatch
                        ...
                        throw "ARM_CM33_MPU protected runtime escaped privileged section: $symbol / $regions/$mode"
                    }
                }
                foreach ($symbol in @("fiber_port_runtime_memory_barrier")) {
                    if ($runtimeSymbols -notmatch
                        ...
                        throw "ARM_CM33_MPU Thread-mode helper escaped unprivileged flash: $symbol / $regions/$mode"
                    }
                }
                foreach ($symbol in @(
                        "fiber_port_runtime_schedule",
                    ...
                    if ($runtimeSymbols -notmatch
                        ...
                        throw "ARM_CM33_MPU syscall veneer escaped syscall flash: $symbol / $regions/$mode"
                    }
                }
                ...
                foreach ($requiredPattern in @(
                    ...
                    if ($runtimeDisassembly -notmatch $requiredPattern) {
                        throw "ARM_CM33_MPU protected runtime generated assembly lost $requiredPattern / $regions/$mode"
                    }
                }
                $pendSvMatch = [regex]::Match($runtimeDisassembly,
                    '(?ms)^[0-9a-fA-F]+\s+<PendSV_Handler>:\r?\n(?<body>.*?)(?=^[0-9a-fA-F]+\s+<[^>]+>:\r?\n|\z)')
                if (-not $pendSvMatch.Success) {
                    throw "ARM_CM33_MPU generated PendSV body is missing: $regions/$mode"
                }
                ...
                if (($preflightIndex -lt 0) -or ($firstCursorLoad -lt 0) -or
                    ...
                    throw "ARM_CM33_MPU PendSV must preflight current before its cursor load: $regions/$mode"
                }
            }
            ...
            if ($LASTEXITCODE -ne 0) {
                throw "ARM_CM33_MPU protected runtime synthetic ELF failed link ($regions/$mode)"
            }
            ...
            foreach ($symbol in @($requiredRuntimeSymbols + @(
                ...
                if (-not ($elfDefined -match
                        "\bT\s+$([regex]::Escape($symbol))$")) {
                    throw "ARM_CM33_MPU protected runtime ELF lost symbol: $symbol / $regions/$mode"
                }
            }
            foreach ($symbol in @("fiber_current", "fiber_schedule")) {
                ...
                if ($threadSymbols.Count -ne 1) {
                    throw "ARM_CM33_MPU protected runtime ELF lost Thread API symbol: $symbol / $regions/$mode"
                }
                ...
                if (($threadAddress -lt 0x08010000) -or
                    ...
                    throw "ARM_CM33_MPU protected runtime Thread API escaped unprivileged flash: $symbol / $regions/$mode"
                }
            }
            ...
            if ($svcSymbols.Count -ne 1) {
                throw "ARM_CM33_MPU protected runtime ELF must retain one strong SVC_Handler: $regions/$mode"
            }
            ...
            if ($pendsvSymbols.Count -ne 1) {
                throw "ARM_CM33_MPU protected runtime ELF must retain one strong PendSV_Handler: $regions/$mode"
            }
            ...
            if ($LASTEXITCODE -ne 0) {
                throw "ARM_CM33_MPU protected runtime vector extraction failed: $regions/$mode"
            }
            $vectorBytes = [IO.File]::ReadAllBytes($vectorBinary)
            if ($vectorBytes.Length -lt (16 * 4)) {
                throw "ARM_CM33_MPU protected runtime vector fixture is truncated: $regions/$mode"
            }
            if ([BitConverter]::ToUInt32($vectorBytes, 11 * 4) -ne
                    ($svcAddress -bor [uint32]1)) {
                throw "ARM_CM33_MPU protected runtime vector slot 11 lost SVC_Handler: $regions/$mode"
            }
            if ([BitConverter]::ToUInt32($vectorBytes, 14 * 4) -ne
                    ($pendsvAddress -bor [uint32]1)) {
                throw "ARM_CM33_MPU protected runtime vector slot 14 lost PendSV_Handler: $regions/$mode"
            }
            ...
            if ($LASTEXITCODE -ne 0) {
                throw "ARM_CM33_MPU competing-PendSV fixture failed compile ($regions/$mode)"
            }
            ...
            if (($duplicatePendSv.ExitCode -eq 0) -or
                ...
                throw "ARM_CM33_MPU competing strong PendSV handlers must fail link ($regions/$mode)`n$($duplicatePendSv.Output)"
            }
        }
    }
}

function Test-ArmCm33MpuRuntimeIntegration {
    param(
        [string]$RepositoryRoot,
        [string]$Compiler,
        [string]$Nm,
    ...
    $profileDir = Join-Path $RepositoryRoot `
        "fiber\port\ARM_CM33_MPU\non_secure"
    $startupSource = Join-Path $RepositoryRoot `
        "tools\fixtures\arm_cm33_mpu_runtime_startup.c"
    $portableSource = Join-Path $RepositoryRoot `
        "tools\fixtures\portable_application.c"
    $expectationSource = Join-Path $RepositoryRoot `
        "fiber\port\fiber_port_context_cohort_expectation.c"
    $linkerScript = Join-Path $RepositoryRoot `
        "tools\fixtures\arm_cm33_mpu_runtime.ld"
    $linkerContract = Join-Path $profileDir "fiber_port_linker_contract.ld"
    ...
    foreach ($requiredPath in @(
            ...
            $competingPendSv)) {
        if (-not (Test-Path -LiteralPath $requiredPath)) {
            throw "ARM_CM33_MPU runtime integration fixture is missing: $requiredPath"
        }
    }
    ...
    foreach ($regions in @(8, 16)) {
        ...
        foreach ($useLto in @($false, $true)) {
            ...
            $probeDir = Join-Path $BuildRoot `
                "arm-cm33-mpu-runtime-r$regions-$mode"
            New-Item -ItemType Directory -Path $probeDir | Out-Null
            Set-Content -LiteralPath (Join-Path $probeDir "main.h") `
                -Value $mainHeader -Encoding ASCII
            ...
            foreach ($source in ($commonSources + $portSources)) {
                ...
                & $Compiler @($baseArgs + $extraFlags + @(
                    "-c", (Join-Path $RepositoryRoot $source), "-o", $object))
                if ($LASTEXITCODE -ne 0) {
                    throw "ARM_CM33_MPU runtime archive compile failed (r$regions/$mode): $source"
                }
                $archiveObjects += $object
            }
            ...
            foreach ($compile in @(
                ...
                & $Compiler @($fixtureArgs + $counterFlags + @(
                    "-c", $compile.Source, "-o", $compile.Object))
                if ($LASTEXITCODE -ne 0) {
                    throw "ARM_CM33_MPU $($compile.Name) compile failed (r$regions/$mode)"
                }
            }
            ...
            $expectationUndefined = @(& $objectNm --undefined-only $expectationObject)
            if ($LASTEXITCODE -ne 0) {
                throw "ARM_CM33_MPU expectation nm failed (r$regions/$mode)"
            }
            ...
            if ($expectedCohortLines.Count -ne 1) {
                throw "ARM_CM33_MPU expectation must retain one exact cohort relocation (r$regions/$mode)"
            }
            $null = $expectedCohortLines[0] -match
                '\bU\s+(fiber_port_context_cohort_\S+)$'
            $expectedCohort = $Matches[1]

            $archivePath = Join-Path $probeDir "libfiber-arm-cm33-mpu-r$regions.a"
            & $Ar rcs $archivePath @archiveObjects
            if ($LASTEXITCODE -ne 0) {
                throw "ARM_CM33_MPU runtime archive creation failed (r$regions/$mode)"
            }

            $elfPath = Join-Path $probeDir "fiber-arm-cm33-mpu-r$regions.elf"
            $mapPath = Join-Path $probeDir "fiber-arm-cm33-mpu-r$regions.map"
            $linkArgs = @(
                "-mcpu=cortex-m33",
                "-mthumb",
                "-mfloat-abi=soft"
            ...
            & $Compiler @linkArgs
            if ($LASTEXITCODE -ne 0) {
                throw "ARM_CM33_MPU portable archive/link integration failed (r$regions/$mode)"
            }

            $defined = @(& $Nm -a --defined-only $elfPath)
            if ($LASTEXITCODE -ne 0) {
                throw "ARM_CM33_MPU final ELF nm failed (r$regions/$mode)"
            }
            foreach ($symbol in $forwardAbiSymbols) {
                $definitions = @($defined | Where-Object {
                ...
                if ($definitions.Count -ne 1) {
                    throw "ARM_CM33_MPU final ELF must define one strong forward ABI symbol (r$regions/$mode): $symbol"
                }
            }
            ...
            if ($cohortDefinitions.Count -ne 1) {
                throw "ARM_CM33_MPU final ELF does not satisfy its exact cohort expectation (r$regions/$mode)"
            }
            ...
            if (($mapText.IndexOf(".fiber_port_context_cohort_expectation",
                    ...
                    [System.StringComparison]::Ordinal) -lt 0)) {
                throw "ARM_CM33_MPU final link lost the external cohort expectation (r$regions/$mode)"
            }
            ...
            foreach ($range in $symbolRanges) {
                foreach ($symbol in $range.Symbols) {
                    ...
                    if ($lines.Count -ne 1) {
                        throw "ARM_CM33_MPU ELF lost range-audited symbol (r$regions/$mode): $symbol"
                    }
                    ...
                    if (($address -lt $range.Start) -or ($address -ge $range.End)) {
                        throw "ARM_CM33_MPU symbol escaped $($range.Name) (r$regions/$mode): $symbol at 0x$($address.ToString('X8'))"
                    }
                }
            }
            ...
            foreach ($stack in @("portable_fixture_stack_1", "portable_fixture_stack_2")) {
                ...
                if ($stackLines.Count -ne 1) {
                    throw "ARM_CM33_MPU portable stack symbol is missing (r$regions/$mode): $stack"
                }
                ...
                if (($stackAddress -lt 0x24000000) -or
                    ...
                    throw "ARM_CM33_MPU portable stack escaped exact unprivileged RAM geometry (r$regions/$mode): $stack"
                }
            }
            foreach ($privilegedObject in @(
                ...
                if ($objectLines.Count -ne 1) {
                    throw "ARM_CM33_MPU privileged application object is missing (r$regions/$mode): $privilegedObject"
                }
                ...
                if (($objectAddress -lt 0x30000020) -or
                    ...
                    throw "ARM_CM33_MPU application object escaped privileged data (r$regions/$mode): $privilegedObject"
                }
            }
            ...
            if ($slotLines.Count -ne 1) {
                throw "ARM_CM33_MPU current slot escaped its exact 32-byte aperture (r$regions/$mode)"
            }

            $sections = (& $Objdump -h $elfPath) -join "`n"
            if ($LASTEXITCODE -ne 0) {
                throw "ARM_CM33_MPU final ELF section audit failed (r$regions/$mode)"
            }
            foreach ($requiredSection in @(
                ...
                if ($sections -notmatch $requiredSection) {
                    throw "ARM_CM33_MPU final ELF lost exact MPU output section (r$regions/$mode): $requiredSection"
                }
            }
            ...
            & $Objcopy -O binary --only-section=.isr_vector $elfPath $vectorPath
            if (($LASTEXITCODE -ne 0) -or (-not (Test-Path $vectorPath))) {
                throw "ARM_CM33_MPU runtime vector extraction failed (r$regions/$mode)"
            }
            $vectorBytes = [IO.File]::ReadAllBytes($vectorPath)
            if ($vectorBytes.Length -ne 64) {
                throw "ARM_CM33_MPU runtime vector fixture changed size (r$regions/$mode)"
            }
            if ([BitConverter]::ToUInt32($vectorBytes, 11 * 4) -ne
                    ($svcAddress -bor 1)) {
                throw "ARM_CM33_MPU vector slot 11 lost strong SVC (r$regions/$mode)"
            }
            if ([BitConverter]::ToUInt32($vectorBytes, 14 * 4) -ne
                    ($pendsvAddress -bor 1)) {
                throw "ARM_CM33_MPU vector slot 14 lost strong PendSV (r$regions/$mode)"
            }
            ...
            foreach ($competing in @(
                ...
                if ($LASTEXITCODE -ne 0) {
                    throw "ARM_CM33_MPU competing $($competing.Symbol) fixture failed compile (r$regions/$mode)"
                }
                ...
                if (($duplicate.ExitCode -eq 0) -or
                    ...
                    throw "ARM_CM33_MPU competing strong $($competing.Symbol) must fail link (r$regions/$mode)`n$($duplicate.Output)"
                }
            }
            ...
            $staleBaseArgs = @($baseArgs | Where-Object {
                $_ -notmatch '^\-DFIBER_PORT_CM33_MPU_TOTAL_REGIONS='
            }) + @("-DFIBER_PORT_CM33_MPU_TOTAL_REGIONS=$otherRegions")
            $staleObjects = @()
            foreach ($source in ($commonSources + $portSources)) {
                $object = Join-Path $probeDir `
                ...
                & $Compiler @($staleBaseArgs + $extraFlags + @(
                    "-c", (Join-Path $RepositoryRoot $source), "-o", $object))
                if ($LASTEXITCODE -ne 0) {
                    throw "ARM_CM33_MPU stale archive compile failed (r$regions/$mode): $source"
                }
                $staleObjects += $object
            }
            $staleArchive = Join-Path $probeDir `
                "libfiber-arm-cm33-mpu-r$otherRegions-stale.a"
            & $Ar rcs $staleArchive @staleObjects
            if ($LASTEXITCODE -ne 0) {
                throw "ARM_CM33_MPU stale archive creation failed (r$regions/$mode)"
            }
            ...
            if (($stale.ExitCode -eq 0) -or
                ...
                throw "ARM_CM33_MPU stale complete archive must fail the exact cohort expectation (r$regions/$mode)`n$($stale.Output)"
            }
            ...
            if (($positiveMapAfterNegativeLinks.IndexOf(
                    ...
                    [System.StringComparison]::Ordinal) -ge 0)) {
                throw "ARM_CM33_MPU retained positive map was overwritten by a negative link (r$regions/$mode)"
            }
        }
    }
}
...
function Test-ArmCm55fMpuRuntimeIntegration {
    param(
        [string]$RepositoryRoot,
        [string]$Compiler,
        [string]$Nm,
    ...
    $profileDir = Join-Path $RepositoryRoot `
        "fiber\port\ARM_CM55F_MPU\non_secure"
    $startupSource = Join-Path $RepositoryRoot `
        "tools\fixtures\arm_cm55f_mpu_runtime_startup.c"
    $portableSource = Join-Path $RepositoryRoot `
        "tools\fixtures\portable_application.c"
    $expectationSource = Join-Path $RepositoryRoot `
        "fiber\port\fiber_port_context_cohort_expectation.c"
    $linkerScript = Join-Path $RepositoryRoot `
        "tools\fixtures\arm_cm55f_mpu_runtime.ld"
    $linkerContract = Join-Path $profileDir "fiber_port_linker_contract.ld"
    $competingSvc = Join-Path $RepositoryRoot `
        "tools\fixtures\arm_cm55f_mpu_competing_svc.c"
    $competingPendSv = Join-Path $RepositoryRoot `
        "tools\fixtures\arm_cm55f_mpu_competing_pendsv.c"
    ...
    foreach ($required in @(
            ...
            $competingPendSv)) {
        if (-not (Test-Path -LiteralPath $required)) {
            throw "ARM_CM55F_MPU runtime integration fixture is missing: $required"
        }
    }
...
#endif
"@
    # Source-level SVC/PendSV checks exercise every hard/softfp -O2/-Os
    # combination. The archive proof samples both ABI families and one LTO
    # configuration without multiplying every stale/duplicate negative link.
    $modes = @(
        [pscustomobject]@{ Name = "hard-o2"; CpuArgs = @("-march=armv8.1-m.main+fp", "-mfloat-abi=hard"); Lto = $false },
        [pscustomobject]@{ Name = "softfp-os"; CpuArgs = @("-march=armv8.1-m.main+fp", "-mfloat-abi=softfp"); Lto = $false },
        [pscustomobject]@{ Name = "hard-o2-lto"; CpuArgs = @("-march=armv8.1-m.main+fp", "-mfloat-abi=hard"); Lto = $true }
    )
    $warningArgs = @(
        "-ffreestanding", "-fno-builtin", "-fno-common",
    ...
    foreach ($regions in @(8, 16)) {
        ...
        foreach ($mode in $modes) {
            $probeDir = Join-Path $BuildRoot `
                "arm-cm55f-mpu-runtime-r$regions-$($mode.Name)"
            New-Item -ItemType Directory -Path $probeDir | Out-Null
            Set-Content -LiteralPath (Join-Path $probeDir "main.h") `
                -Value $mainHeader -Encoding ASCII
            ...
            $baseArgs = @($mode.CpuArgs) + @(
                "-mthumb", "-O2", "-std=gnu11") + $warningArgs +
                $manifestDefines + $ltoArgs + @(
                "-I$profileDir",
                "-I$(Join-Path $RepositoryRoot 'fiber\port')",
                "-I$(Join-Path $RepositoryRoot 'fiber')",
            ...
            # Archive all mandatory common and selected-port objects. The
            # expectation object stays outside this archive exactly as it must
            # in a board build to reject a coherent but stale whole port.
            $archiveObjects = @()
            foreach ($source in ($commonSources + $portSources)) {
                $object = Join-Path $probeDir `
                    (($source -replace '[\\/]', '_') + ".o")
                & $Compiler @($baseArgs + @(
                    "-c", (Join-Path $RepositoryRoot $source), "-o", $object))
                if ($LASTEXITCODE -ne 0) {
                    throw "ARM_CM55F_MPU runtime archive compile failed (r$regions/$($mode.Name)): $source"
                }
                $archiveObjects += $object
            }
            $archivePath = Join-Path $probeDir `
                "libfiber-arm-cm55f-mpu-r$regions.a"
            & $Ar rcs $archivePath @archiveObjects
            if ($LASTEXITCODE -ne 0) {
                throw "ARM_CM55F_MPU runtime archive creation failed (r$regions/$($mode.Name))"
            }
            ...
            foreach ($compile in @(
                    ...
                    [pscustomobject]@{ Source = $expectationSource; Object = $expectationObject; Name = "cohort expectation" })) {
                $fixtureArgs = $baseArgs
                if ($mode.Lto -and ($compile.Name -eq "portable application")) {
                    $fixtureArgs = @($baseArgs | Where-Object { $_ -ne "-flto" })
                    $fixtureArgs += "-fno-lto"
                }
                & $Compiler @($fixtureArgs + @(
                    "-c", $compile.Source, "-o", $compile.Object))
                if ($LASTEXITCODE -ne 0) {
                    throw "ARM_CM55F_MPU $($compile.Name) compile failed (r$regions/$($mode.Name))"
                }
            }
            ...
            $expectationUndefined = @(& $objectNm --undefined-only $expectationObject)
            if ($LASTEXITCODE -ne 0) {
                throw "ARM_CM55F_MPU expectation nm failed (r$regions/$($mode.Name))"
            }
            $expectedCohortLines = @($expectationUndefined | Where-Object {
                $_ -match '\bU\s+(fiber_port_context_cohort_\S+)$'
            })
            if ($expectedCohortLines.Count -ne 1) {
                throw "ARM_CM55F_MPU expectation must retain one exact cohort relocation (r$regions/$($mode.Name))"
            }
            $null = $expectedCohortLines[0] -match
                '\bU\s+(fiber_port_context_cohort_\S+)$'
            $expectedCohort = $Matches[1]

            $elfPath = Join-Path $probeDir "fiber-arm-cm55f-mpu-r$regions.elf"
            $mapPath = Join-Path $probeDir "fiber-arm-cm55f-mpu-r$regions.map"
            $linkArgs = @($mode.CpuArgs) + @("-mthumb") + $ltoArgs + @(
                "-nostdlib", "-Wl,--gc-sections", "-Wl,-Map,$mapPath",
                "-Wl,-T,$linkerScript", "-Wl,-T,$linkerContract",
                $startupObject, $portableObject, $expectationObject,
                $archivePath, "-o", $elfPath
            )
            & $Compiler @linkArgs
            if ($LASTEXITCODE -ne 0) {
                throw "ARM_CM55F_MPU portable archive/link integration failed (r$regions/$($mode.Name))"
            }

            $defined = @(& $Nm -a --defined-only $elfPath)
            if ($LASTEXITCODE -ne 0) {
                throw "ARM_CM55F_MPU final ELF nm failed (r$regions/$($mode.Name))"
            }
            foreach ($symbol in $forwardAbiSymbols) {
                $definitions = @($defined | Where-Object {
                    $_ -match ("\bT\s+" + [regex]::Escape($symbol) + "$")
                ...
                if ($definitions.Count -ne 1) {
                    throw "ARM_CM55F_MPU final ELF must define one strong forward ABI symbol (r$regions/$($mode.Name)): $symbol"
                }
            }
            foreach ($symbol in @(
                    "fiber_port_handler_bundle_v1_anchor",
                    "fiber_port_arm_cm55f_mpu_pendsv_handler_component_v1_anchor")) {
                if (-not ($defined -match ("\bT\s+" +
                        [regex]::Escape($symbol) + "$"))) {
                    throw "ARM_CM55F_MPU final ELF lost handler-bundle mechanism (r$regions/$($mode.Name)): $symbol"
                }
            }
            $cohortDefinitions = @($defined | Where-Object {
                $_ -match ('\b[TR]\s+' + [regex]::Escape($expectedCohort) + '$')
            })
            if ($cohortDefinitions.Count -ne 1) {
                throw "ARM_CM55F_MPU final ELF does not satisfy its exact cohort expectation (r$regions/$($mode.Name))"
            }
            ...
            if (($mapText.IndexOf(".fiber_port_context_cohort_expectation",
                    ...
                    ($mapText.IndexOf($expectedCohort,
                    [System.StringComparison]::Ordinal) -lt 0)) {
                throw "ARM_CM55F_MPU final link lost the external cohort expectation (r$regions/$($mode.Name))"
            }
            if (-not $mode.Lto) {
                foreach ($archiveMember in @(
                        "fiber_port_boot.c.o", "fiber_port_svc.c.o",
                        "fiber_port_pendsv.c.o")) {
                    if ($mapText.IndexOf($archiveMember,
                            [System.StringComparison]::Ordinal) -lt 0) {
                        throw "ARM_CM55F_MPU archive failed to extract mandatory member (r$regions/$($mode.Name)): $archiveMember"
                    }
                }
            }
            ...
            $ranges = @(
                [pscustomobject]@{
                    Start = [uint32]0x08010000; End = [uint32]0x08020000;
                    Name = "unprivileged flash";
                    Symbols = @("fiber_current", "fiber_schedule",
                        "fiber_port_runtime_memory_barrier")
                },
                [pscustomobject]@{
                    Start = [uint32]0x08020000; End = [uint32]0x08021000;
                    Name = "syscall flash";
                    Symbols = @("fiber_port_runtime_schedule",
                        "fiber_port_unprivileged_task_return")
                },
                [pscustomobject]@{
                    Start = [uint32]0x08000000; End = [uint32]0x08010000;
                    Name = "privileged flash";
                    Symbols = @("fiber_port_context_init", "fiber_port_panic_wait",
                        "fiber_port_require_scheduler_configuration_environment",
                        "fiber_port_runtime_prepare_start",
                        "fiber_port_runtime_select_first",
                        "fiber_port_runtime_start_first",
            ...
            foreach ($range in $ranges) {
                foreach ($symbol in $range.Symbols) {
                    $line = @($defined | Where-Object {
                        $_ -match ("^\s*(?<address>[0-9a-fA-F]+)\s+[TtWw]\s+" +
                            [regex]::Escape($symbol) + "$")
                    })
                    if ($line.Count -ne 1) {
                        throw "ARM_CM55F_MPU range audit lost symbol (r$regions/$($mode.Name)): $symbol"
                    }
                    $null = $line[0] -match '^\s*(?<address>[0-9a-fA-F]+)'
                    $address = [Convert]::ToUInt32($Matches['address'], 16)
                    if (($address -lt $range.Start) -or ($address -ge $range.End)) {
                        throw "ARM_CM55F_MPU symbol escaped $($range.Name) (r$regions/$($mode.Name)): $symbol"
                    }
                }
            }
            ...
            foreach ($requiredSection in @(
                    ...
                    '\.fiber_current_context_slot\s+00000020\s+30000000\s+')) {
                if ($sections -notmatch $requiredSection) {
                    throw "ARM_CM55F_MPU final ELF lost exact MPU output section (r$regions/$($mode.Name)): $requiredSection"
                }
            }
            $vectorPath = Join-Path $probeDir "vectors.bin"
            & $Objcopy -O binary --only-section=.isr_vector $elfPath $vectorPath
            if (($LASTEXITCODE -ne 0) -or (-not (Test-Path $vectorPath))) {
                throw "ARM_CM55F_MPU runtime vector extraction failed (r$regions/$($mode.Name))"
            }
            $vector = [IO.File]::ReadAllBytes($vectorPath)
            if (($vector.Length -ne 64) -or
                    ([BitConverter]::ToUInt32($vector, 11 * 4) -ne
                        ($svcAddress -bor 1)) -or
                    ([BitConverter]::ToUInt32($vector, 14 * 4) -ne
                        ($pendsvAddress -bor 1))) {
                throw "ARM_CM55F_MPU vector slots 11/14 lost selected strong handlers (r$regions/$($mode.Name))"
            }
            ...
            foreach ($competing in @(
                ...
                & $Compiler @($baseArgs + @(
                    "-c", $competing.Source, "-o", $competingObject))
                if ($LASTEXITCODE -ne 0) {
                    throw "ARM_CM55F_MPU competing $($competing.Symbol) fixture failed compile (r$regions/$($mode.Name))"
                }
                $duplicateLog = Join-Path $probeDir `
                    ("duplicate-" + $competing.Symbol + ".log")
                $duplicateMapPath = Join-Path $probeDir `
                ...
                if (($duplicate.ExitCode -eq 0) -or
                        ($duplicate.Output -notmatch
                            ("(?s)multiple definition.*" + $competing.Symbol))) {
                    throw "ARM_CM55F_MPU competing strong $($competing.Symbol) must fail link (r$regions/$($mode.Name))`n$($duplicate.Output)"
                }
            }

            # A coherent archive from the other MPU region geometry is still
            # invalid for this application-owned expectation. Test both 8->16
            # and 16->8, including LTO, without relying on object names.
            $staleBaseArgs = @($baseArgs | Where-Object {
                $_ -notmatch '^-DFIBER_PORT_CM55F_MPU_TOTAL_REGIONS='
            }) + @("-DFIBER_PORT_CM55F_MPU_TOTAL_REGIONS=$otherRegions")
            $staleObjects = @()
            foreach ($source in ($commonSources + $portSources)) {
                $object = Join-Path $probeDir `
                    ((($source -replace '[\\/]', '_') -replace '\.c$', '') + "-stale.o")
                & $Compiler @($staleBaseArgs + @(
                    "-c", (Join-Path $RepositoryRoot $source), "-o", $object))
                if ($LASTEXITCODE -ne 0) {
                    throw "ARM_CM55F_MPU stale archive compile failed (r$regions/$($mode.Name)): $source"
                }
                $staleObjects += $object
            }
            $staleArchive = Join-Path $probeDir `
                "libfiber-arm-cm55f-mpu-r$otherRegions-stale.a"
            & $Ar rcs $staleArchive @staleObjects
            if ($LASTEXITCODE -ne 0) {
                throw "ARM_CM55F_MPU stale archive creation failed (r$regions/$($mode.Name))"
            }
            $staleLog = Join-Path $probeDir "stale-archive.log"
            $staleMapPath = Join-Path $probeDir "stale-archive.map"
            $stalePrefix = @($linkArgs[0..($linkArgs.Count - 4)] |
            ...
            $staleArgs = @($stalePrefix + @(
                "-Wl,-Map,$staleMapPath", $staleArchive, "-o",
                (Join-Path $probeDir "stale.elf")))
            $stale = Invoke-CompilerProbe -Compiler $Compiler -Arguments $staleArgs `
                -LogPath $staleLog
            if (($stale.ExitCode -eq 0) -or
                    (($stale.Output -replace '\s+', '') -notmatch
                        [regex]::Escape($expectedCohort))) {
                throw "ARM_CM55F_MPU stale complete archive must fail the exact cohort expectation (r$regions/$($mode.Name))`n$($stale.Output)"
            }
        }
    }
}

function Test-ArmCm55MvefMpuRuntimeIntegration {
    param(
        [string]$RepositoryRoot,
        [string]$Compiler,
        [string]$Nm,
    ...
    $profileDir = Join-Path $RepositoryRoot `
        "fiber\port\ARM_CM55_MVEF_MPU\non_secure"
    $startupSource = Join-Path $RepositoryRoot `
        "tools\fixtures\arm_cm55_mvef_mpu_runtime_startup.c"
    $portableSource = Join-Path $RepositoryRoot `
        "tools\fixtures\portable_application.c"
    $expectationSource = Join-Path $RepositoryRoot `
        "fiber\port\fiber_port_context_cohort_expectation.c"
    $linkerScript = Join-Path $RepositoryRoot `
        "tools\fixtures\arm_cm55_mvef_mpu_runtime.ld"
    $linkerContract = Join-Path $profileDir "fiber_port_linker_contract.ld"
    $competingSvc = Join-Path $RepositoryRoot `
        "tools\fixtures\arm_cm55_mvef_mpu_competing_svc.c"
    $competingPendSv = Join-Path $RepositoryRoot `
        "tools\fixtures\arm_cm55_mvef_mpu_competing_pendsv.c"
    ...
    foreach ($required in @(
            ...
            $competingPendSv)) {
        if (-not (Test-Path -LiteralPath $required)) {
            throw "ARM_CM55_MVEF_MPU runtime integration fixture is missing: $required"
        }
    }
...
"@
    ...
    # configuration without multiplying every stale/duplicate negative link.
    $modes = @(
        [pscustomobject]@{ Name = "hard-o2"; CpuArgs = @("-march=armv8.1-m.main+mve.fp", "-mfloat-abi=hard"); Lto = $false },
        [pscustomobject]@{ Name = "softfp-os"; CpuArgs = @("-march=armv8.1-m.main+mve.fp", "-mfloat-abi=softfp"); Lto = $false },
        [pscustomobject]@{ Name = "hard-o2-lto"; CpuArgs = @("-march=armv8.1-m.main+mve.fp", "-mfloat-abi=hard"); Lto = $true }
    )
    $warningArgs = @(
        "-ffreestanding", "-fno-builtin", "-fno-common",
        "-ffunction-sections", "-fdata-sections",
    ...
    foreach ($regions in @(8, 16)) {
        ...
        foreach ($mode in $modes) {
            $probeDir = Join-Path $BuildRoot `
                "arm-cm55-mvef-mpu-runtime-r$regions-$($mode.Name)"
            New-Item -ItemType Directory -Path $probeDir | Out-Null
            Set-Content -LiteralPath (Join-Path $probeDir "main.h") `
                -Value $mainHeader -Encoding ASCII
            ...
            foreach ($source in ($commonSources + $portSources)) {
                ...
                & $Compiler @($baseArgs + @(
                    "-c", (Join-Path $RepositoryRoot $source), "-o", $object))
                if ($LASTEXITCODE -ne 0) {
                    throw "ARM_CM55_MVEF_MPU runtime archive compile failed (r$regions/$($mode.Name)): $source"
                }
                $archiveObjects += $object
            }
            $archivePath = Join-Path $probeDir `
                "libfiber-arm-cm55-mvef-mpu-r$regions.a"
            & $Ar rcs $archivePath @archiveObjects
            if ($LASTEXITCODE -ne 0) {
                throw "ARM_CM55_MVEF_MPU runtime archive creation failed (r$regions/$($mode.Name))"
            }
            ...
            foreach ($compile in @(
                ...
                & $Compiler @($fixtureArgs + @(
                    "-c", $compile.Source, "-o", $compile.Object))
                if ($LASTEXITCODE -ne 0) {
                    throw "ARM_CM55_MVEF_MPU $($compile.Name) compile failed (r$regions/$($mode.Name))"
                }
            }
            ...
            $expectationUndefined = @(& $objectNm --undefined-only $expectationObject)
            if ($LASTEXITCODE -ne 0) {
                throw "ARM_CM55_MVEF_MPU expectation nm failed (r$regions/$($mode.Name))"
            }
            $expectedCohortLines = @($expectationUndefined | Where-Object {
                $_ -match '\bU\s+(fiber_port_context_cohort_\S+)$'
            })
            if ($expectedCohortLines.Count -ne 1) {
                throw "ARM_CM55_MVEF_MPU expectation must retain one exact cohort relocation (r$regions/$($mode.Name))"
            }
            $null = $expectedCohortLines[0] -match
                '\bU\s+(fiber_port_context_cohort_\S+)$'
            $expectedCohort = $Matches[1]

            $elfPath = Join-Path $probeDir "fiber-arm-cm55-mvef-mpu-r$regions.elf"
            $mapPath = Join-Path $probeDir "fiber-arm-cm55-mvef-mpu-r$regions.map"
            $linkArgs = @($mode.CpuArgs) + @("-mthumb") + $ltoArgs + @(
                "-nostdlib", "-Wl,--gc-sections", "-Wl,-Map,$mapPath",
                "-Wl,-T,$linkerScript", "-Wl,-T,$linkerContract",
                $startupObject, $portableObject, $expectationObject,
            ...
            & $Compiler @linkArgs
            if ($LASTEXITCODE -ne 0) {
                throw "ARM_CM55_MVEF_MPU portable archive/link integration failed (r$regions/$($mode.Name))"
            }

            $defined = @(& $Nm -a --defined-only $elfPath)
            if ($LASTEXITCODE -ne 0) {
                throw "ARM_CM55_MVEF_MPU final ELF nm failed (r$regions/$($mode.Name))"
            }
            foreach ($symbol in $forwardAbiSymbols) {
                $definitions = @($defined | Where-Object {
                    $_ -match ("\bT\s+" + [regex]::Escape($symbol) + "$")
                ...
                if ($definitions.Count -ne 1) {
                    throw "ARM_CM55_MVEF_MPU final ELF must define one strong forward ABI symbol (r$regions/$($mode.Name)): $symbol"
                }
            }
            foreach ($symbol in @(
                    "fiber_port_handler_bundle_v1_anchor",
                    "fiber_port_arm_cm55_mvef_mpu_pendsv_handler_component_v1_anchor")) {
                if (-not ($defined -match ("\bT\s+" +
                        [regex]::Escape($symbol) + "$"))) {
                    throw "ARM_CM55_MVEF_MPU final ELF lost handler-bundle mechanism (r$regions/$($mode.Name)): $symbol"
                }
            }
            $cohortDefinitions = @($defined | Where-Object {
                $_ -match ('\b[TR]\s+' + [regex]::Escape($expectedCohort) + '$')
            ...
            if ($cohortDefinitions.Count -ne 1) {
                throw "ARM_CM55_MVEF_MPU final ELF does not satisfy its exact cohort expectation (r$regions/$($mode.Name))"
            }
            ...
            if (($mapText.IndexOf(".fiber_port_context_cohort_expectation",
                    ...
                    ($mapText.IndexOf($expectedCohort,
                    [System.StringComparison]::Ordinal) -lt 0)) {
                throw "ARM_CM55_MVEF_MPU final link lost the external cohort expectation (r$regions/$($mode.Name))"
            }
            if (-not $mode.Lto) {
                foreach ($archiveMember in @(
                        "fiber_port_boot.c.o", "fiber_port_svc.c.o",
                    ...
                    if ($mapText.IndexOf($archiveMember,
                            [System.StringComparison]::Ordinal) -lt 0) {
                        throw "ARM_CM55_MVEF_MPU archive failed to extract mandatory member (r$regions/$($mode.Name)): $archiveMember"
                    }
                }
            }
            ...
            foreach ($range in $ranges) {
                foreach ($symbol in $range.Symbols) {
                    ...
                    if ($line.Count -ne 1) {
                        throw "ARM_CM55_MVEF_MPU range audit lost symbol (r$regions/$($mode.Name)): $symbol"
                    }
                    $null = $line[0] -match '^\s*(?<address>[0-9a-fA-F]+)'
                    $address = [Convert]::ToUInt32($Matches['address'], 16)
                    if (($address -lt $range.Start) -or ($address -ge $range.End)) {
                        throw "ARM_CM55_MVEF_MPU symbol escaped $($range.Name) (r$regions/$($mode.Name)): $symbol"
                    }
                }
            }
            ...
            foreach ($requiredSection in @(
                    ...
                    '\.fiber_current_context_slot\s+00000020\s+30000000\s+')) {
                if ($sections -notmatch $requiredSection) {
                    throw "ARM_CM55_MVEF_MPU final ELF lost exact MPU output section (r$regions/$($mode.Name)): $requiredSection"
                }
            }
            $vectorPath = Join-Path $probeDir "vectors.bin"
            & $Objcopy -O binary --only-section=.isr_vector $elfPath $vectorPath
            if (($LASTEXITCODE -ne 0) -or (-not (Test-Path $vectorPath))) {
                throw "ARM_CM55_MVEF_MPU runtime vector extraction failed (r$regions/$($mode.Name))"
            }
            $vector = [IO.File]::ReadAllBytes($vectorPath)
            if (($vector.Length -ne 64) -or
                    ([BitConverter]::ToUInt32($vector, 11 * 4) -ne
                    ...
                    ([BitConverter]::ToUInt32($vector, 14 * 4) -ne
                        ($pendsvAddress -bor 1))) {
                throw "ARM_CM55_MVEF_MPU vector slots 11/14 lost selected strong handlers (r$regions/$($mode.Name))"
            }
            ...
            foreach ($competing in @(
                ...
                & $Compiler @($baseArgs + @(
                    "-c", $competing.Source, "-o", $competingObject))
                if ($LASTEXITCODE -ne 0) {
                    throw "ARM_CM55_MVEF_MPU competing $($competing.Symbol) fixture failed compile (r$regions/$($mode.Name))"
                }
                $duplicateLog = Join-Path $probeDir `
                    ("duplicate-" + $competing.Symbol + ".log")
                $duplicateMapPath = Join-Path $probeDir `
                ...
                if (($duplicate.ExitCode -eq 0) -or
                        ($duplicate.Output -notmatch
                            ("(?s)multiple definition.*" + $competing.Symbol))) {
                    throw "ARM_CM55_MVEF_MPU competing strong $($competing.Symbol) must fail link (r$regions/$($mode.Name))`n$($duplicate.Output)"
                }
            }
            ...
            # and 16->8, including LTO, without relying on object names.
            $staleBaseArgs = @($baseArgs | Where-Object {
                $_ -notmatch '^-DFIBER_PORT_CM55_MVEF_MPU_TOTAL_REGIONS='
            }) + @("-DFIBER_PORT_CM55_MVEF_MPU_TOTAL_REGIONS=$otherRegions")
            $staleObjects = @()
            foreach ($source in ($commonSources + $portSources)) {
                $object = Join-Path $probeDir `
                    ((($source -replace '[\\/]', '_') -replace '\.c$', '') + "-stale.o")
                & $Compiler @($staleBaseArgs + @(
                    "-c", (Join-Path $RepositoryRoot $source), "-o", $object))
                if ($LASTEXITCODE -ne 0) {
                    throw "ARM_CM55_MVEF_MPU stale archive compile failed (r$regions/$($mode.Name)): $source"
                }
                $staleObjects += $object
            }
            $staleArchive = Join-Path $probeDir `
                "libfiber-arm-cm55-mvef-mpu-r$otherRegions-stale.a"
            & $Ar rcs $staleArchive @staleObjects
            if ($LASTEXITCODE -ne 0) {
                throw "ARM_CM55_MVEF_MPU stale archive creation failed (r$regions/$($mode.Name))"
            }
            $staleLog = Join-Path $probeDir "stale-archive.log"
            $staleMapPath = Join-Path $probeDir "stale-archive.map"
            $stalePrefix = @($linkArgs[0..($linkArgs.Count - 4)] |
            ...
            if (($stale.ExitCode -eq 0) -or
                    (($stale.Output -replace '\s+', '') -notmatch
                        [regex]::Escape($expectedCohort))) {
                throw "ARM_CM55_MVEF_MPU stale complete archive must fail the exact cohort expectation (r$regions/$($mode.Name))`n$($stale.Output)"
            }
        }
    }
}
...
function Test-ArmCm55NtzRuntimeContract {
    ...
    $profileDir = Join-Path $RepositoryRoot `
        "fiber\port\ARM_CM55_NTZ\non_secure"
    $fixture = Join-Path $RepositoryRoot `
        "tools\fixtures\arm_cm55_ntz_layout_probe.c"
    foreach ($required in @(
            (Join-Path $profileDir "fiber_port_types.h"),
            (Join-Path $profileDir "fiber_port_boot_types.h"),
            (Join-Path $profileDir "fiber_portmacro.h"),
            ...
            $fixture)) {
        if (-not (Test-Path -LiteralPath $required)) {
            throw "ARM_CM55_NTZ runtime slice is missing: $required"
        }
    }
    ...
    foreach ($forbiddenRuntime in @(
            ...
            "fiber_port_secure_context.c")) {
        if (Test-Path -LiteralPath (Join-Path $profileDir $forbiddenRuntime)) {
            throw "ARM_CM55_NTZ runtime slice must not expose unsupported artifact: $forbiddenRuntime"
        }
    }
    ...
    foreach ($selector in $selectors) {
        if ((Get-Content -LiteralPath $selector -Raw).IndexOf(
                "ARM_CM55_NTZ", [System.StringComparison]::Ordinal) -ge 0) {
            throw "Staged ARM_CM55_NTZ profile entered global selection: $selector"
        }
    }
    ...
    foreach ($forbiddenTypeDependency in @(
        ...
        $includePattern = '(?m)^\s*#\s*include\s*["<][^">]*' +
            [regex]::Escape($forbiddenTypeDependency) + '[">]'
        if ([regex]::IsMatch($typeText, $includePattern)) {
            throw "ARM_CM55_NTZ public storage acquired CPU dependency: $forbiddenTypeDependency"
        }
    }
    ...
    foreach ($requiredParity in @(
            ...
            "fiber_port_init_context_frame()",
            "0xFFFFFFB8",
            "FAP-M55-CONCRETE-MVE",
            "FAP-M55-SINGLE-CORE",
            "FAP-M55-FULL-FIRST-RESTORE",
            "FAP-M55-PSPLIM-READBACK")) {
        if ($parity.IndexOf($requiredParity,
                [System.StringComparison]::Ordinal) -lt 0) {
            throw "ARM_CM55_NTZ parity ledger lost reference evidence: $requiredParity"
        }
    }

    $probeDir = Join-Path $BuildRoot "arm-cm55-ntz-layout"
    New-Item -ItemType Directory -Path $probeDir | Out-Null
    $mainHeader = @"
#ifndef MAIN_H_
#define MAIN_H_
...
_Static_assert(sizeof(FiberContext) == 76u, "context size");
_Static_assert(_Alignof(FiberContext) == 4u, "context alignment");
FiberContext fiber_cm55_ntz_type_only_object;
'@
    $typeObject = Join-Path $probeDir "type-only.o"
    $typeArgs = @(
        "-mcpu=cortex-m55", "-mthumb", "-mfloat-abi=soft", "-std=c11",
    ...
    & $Compiler @typeArgs
    if ($LASTEXITCODE -ne 0) {
        throw "ARM_CM55_NTZ type-only C header failed without CMSIS"
    }
...
static_assert(sizeof(FiberContext) == 76u, "context size");
static_assert(alignof(FiberContext) == 4u, "context alignment");
FiberContext fiber_cm55_ntz_cpp_type_only_object;
'@
    $cppObject = Join-Path $probeDir "type-only-cpp.o"
    & $Compiler -x c++ -mcpu=cortex-m55 -mthumb -mfloat-abi=soft -std=c++17 `
        -ffreestanding -fno-builtin -Wall -Wextra -Werror `
    ...
    & $Compiler @manifestArgs -c $fixture -o $layoutObject
    if ($LASTEXITCODE -ne 0) {
        throw "ARM_CM55_NTZ exact layout manifest failed compile"
    }

    $defined = @(& $Nm -g --defined-only $layoutObject)
    if ($LASTEXITCODE -ne 0) {
        throw "nm failed for ARM_CM55_NTZ layout probe"
    }
    $cohorts = @($defined | ForEach-Object {
        if ($_ -match '\b(fiber_port_context_cohort_\S+)$') {
            $Matches[1]
    ...
    if ($cohorts.Count -ne 1) {
        throw "ARM_CM55_NTZ layout must define one exact cohort"
    }
    foreach ($token in @(
            "armv81m_mainline",
            "p0x4335354Eu",
        ...
        if ($cohorts[0].IndexOf($token,
                [System.StringComparison]::Ordinal) -lt 0) {
            throw "ARM_CM55_NTZ exact cohort lost token ${token}: $($cohorts[0])"
        }
    }
    ...
    foreach ($requiredRuntimeToken in @(
        ...
        if (($frameSource.IndexOf($requiredRuntimeToken,
                ...
                ($macroHeader.IndexOf($requiredRuntimeToken,
                    [System.StringComparison]::Ordinal) -lt 0)) {
            throw "ARM_CM55_NTZ runtime source lost contract: $requiredRuntimeToken"
        }
    }
    ...
    foreach ($assignment in @(
        ...
        if ($frameBody.IndexOf($assignment,
                [System.StringComparison]::Ordinal) -lt 0) {
            throw "ARM_CM55_NTZ initial frame lost required assignment: $assignment"
        }
    }
    $lastAssignment = -1
    foreach ($assignment in @(
        ...
        $assignmentIndex = $frameBody.IndexOf($assignment,
            [System.StringComparison]::Ordinal)
        if ($assignmentIndex -le $lastAssignment) {
            throw "ARM_CM55_NTZ initial frame lost exact assignment order: $assignment"
        }
        $lastAssignment = $assignmentIndex
    }
    if (([regex]::Matches($frameBody,
            'frame\[fiber_portFRAME_[A-Z0-9_]+\]\s*=')).Count -ne 18) {
        throw "ARM_CM55_NTZ constructor must assign exactly eighteen frame words"
    }
    foreach ($requiredConstructionToken in @(
            "FIBER_PORT_CONTEXT_COHORT_DEFINE();",
            "fiber_port_boot_record_check(&ctx->boot);",
        ...
        if (($frameSource.IndexOf($requiredConstructionToken,
                ...
                ($bootSource.IndexOf($requiredConstructionToken,
                    [System.StringComparison]::Ordinal) -lt 0)) {
            throw "ARM_CM55_NTZ construction source lost contract: $requiredConstructionToken"
        }
    }
    ...
    & $Compiler @constructionArgs -c $frameSourcePath -o $frameObject
    if ($LASTEXITCODE -ne 0) {
        throw "ARM_CM55_NTZ frame construction source failed compile"
    }
    & $Compiler @constructionArgs -c $bootSourcePath -o $bootObject
    if ($LASTEXITCODE -ne 0) {
        throw "ARM_CM55_NTZ boot construction source failed compile"
    }
    $constructionGroup = Join-Path $probeDir "construction-group.o"
    & $Compiler -r $frameObject $bootObject -o $constructionGroup
    if ($LASTEXITCODE -ne 0) {
        throw "ARM_CM55_NTZ construction object group failed relocatable link"
    }
    $constructionDefined = @(& $Nm -g --defined-only $constructionGroup)
    if ($LASTEXITCODE -ne 0) {
        throw "nm failed for ARM_CM55_NTZ construction object group"
    }
    $expectedForwardDefinitions = @(
        "fiber_port_context_init",
        "fiber_port_runtime_memory_barrier",
    ...
    foreach ($expectedForwardDefinition in $expectedForwardDefinitions) {
        ...
        if ($definitions.Count -ne 1) {
            throw "ARM_CM55_NTZ runtime group must define one $expectedForwardDefinition"
        }
    }
    $constructionCohorts = @($constructionDefined | ForEach-Object {
        if ($_ -match '\b(fiber_port_context_cohort_\S+)$') {
    ...
    if (($constructionCohorts.Count -ne 1) -or
            ($constructionCohorts[0] -ne $cohorts[0])) {
        throw "ARM_CM55_NTZ construction group disagrees with frozen layout cohort"
    }
    $svcDefinitions = @($constructionDefined | Where-Object {
        $_ -match '\bT\s+SVC_Handler$'
    })
    if ($svcDefinitions.Count -ne 1) {
        throw "ARM_CM55_NTZ runtime group must define one strong SVC_Handler"
    }
    $pendSvDefinitions = @($constructionDefined | Where-Object {
        $_ -match '\bT\s+PendSV_Handler$'
    })
    if ($pendSvDefinitions.Count -ne 1) {
        throw "ARM_CM55_NTZ runtime group must define one strong PendSV_Handler"
    }
    $constructionUndefined = @(& $Nm -u $constructionGroup | ForEach-Object {
        if ($_ -match '^\s*U\s+(\S+)$') {
            $Matches[1]
    ...
    $constructionUndefinedDelta = @(Compare-Object `
        ...
        -DifferenceObject $constructionUndefined)
    if ($constructionUndefinedDelta.Count -ne 0) {
        throw "ARM_CM55_NTZ runtime group must retain the exact reverse/integration dependency set: $($constructionUndefined -join ', ')"
    }
    ...
    foreach ($variant in $sourceVariants) {
        ...
        & $Compiler @variantArgs -c $frameSourcePath -o $variantRuntime
        if ($LASTEXITCODE -ne 0) {
            throw "ARM_CM55_NTZ $($variant.Name) runtime variant failed compile"
        }
        & $Compiler @variantArgs -c $bootSourcePath -o $variantBoot
        if ($LASTEXITCODE -ne 0) {
            throw "ARM_CM55_NTZ $($variant.Name) boot variant failed compile"
        }
        & $Compiler -r $variantRuntime $variantBoot -o $variantGroup
        if ($LASTEXITCODE -ne 0) {
            throw "ARM_CM55_NTZ $($variant.Name) object group failed link"
        }
    }
    ...
    $runtimeDisassembly = (& $Objdump -dr $frameObject) -join "`n"
    if ($LASTEXITCODE -ne 0) {
        throw "objdump failed for ARM_CM55_NTZ runtime object"
    }
    $svcBody = Get-DisassemblyFunctionBody -Disassembly $runtimeDisassembly `
        -Symbol "SVC_Handler" -Path $frameObject
    $startBody = Get-DisassemblyFunctionBody -Disassembly $runtimeDisassembly `
    ...
    foreach ($instructionPattern in $svcInstructionOrder) {
        $instructionMatch = [regex]::Match($remainingSvcBody,
            $instructionPattern, [Text.RegularExpressions.RegexOptions]::IgnoreCase)
        if (-not $instructionMatch.Success) {
            throw "ARM_CM55_NTZ generated SVC lost FreeRTOS-parity instruction order: $instructionPattern`n$svcBody"
        }
        $remainingSvcBody = $remainingSvcBody.Substring(
            $instructionMatch.Index + $instructionMatch.Length)
    }
    ...
    foreach ($instructionPattern in $startInstructionOrder) {
        $instructionMatch = [regex]::Match($remainingStartBody,
            $instructionPattern, [Text.RegularExpressions.RegexOptions]::IgnoreCase)
        if (-not $instructionMatch.Success) {
            throw "ARM_CM55_NTZ generated first-start helper lost FreeRTOS-parity instruction order: $instructionPattern`n$startBody"
        }
        $remainingStartBody = $remainingStartBody.Substring(
            $instructionMatch.Index + $instructionMatch.Length)
    }
    ...
    foreach ($instructionPattern in $pendSvInstructionOrder) {
        $instructionMatch = [regex]::Match($remainingPendSvBody,
            $instructionPattern, [Text.RegularExpressions.RegexOptions]::IgnoreCase)
        if (-not $instructionMatch.Success) {
            throw "ARM_CM55_NTZ generated PendSV lost FreeRTOS-parity instruction order: $instructionPattern`n$pendSvBody"
        }
        $remainingPendSvBody = $remainingPendSvBody.Substring(
            $instructionMatch.Index + $instructionMatch.Length)
    }
    ...
    foreach ($forbiddenGeneratedSymbol in @(
        ...
        if ($runtimeDisassembly.IndexOf($forbiddenGeneratedSymbol,
                [System.StringComparison]::Ordinal) -ge 0) {
            throw "ARM_CM55_NTZ generated runtime contains compiler helper: $forbiddenGeneratedSymbol"
        }
    }
    ...
    & $Compiler @($manifestArgs + @(
        ...
        "-c", $frameSourcePath, "-o", $adversarialObject))
    if ($LASTEXITCODE -ne 0) {
        throw "ARM_CM55_NTZ adversarial runtime source failed compile"
    }
    $adversarialDisassembly = (& $Objdump -dr $adversarialObject) -join "`n"
    if ($LASTEXITCODE -ne 0) {
        throw "objdump failed for adversarial ARM_CM55_NTZ runtime object"
    }
    Assert-SensitiveFunctionClean -Disassembly $adversarialDisassembly `
        -Symbol "SVC_Handler" -Path $adversarialObject
    Assert-SensitiveFunctionClean -Disassembly $adversarialDisassembly `
...
'@
    ...
    foreach ($archiveMode in @(
        ...
        $archiveRuntimeObject = Join-Path $modeDir "runtime.o"
        $archiveBootObject = Join-Path $modeDir "boot.o"
        $archivePath = Join-Path $modeDir "libfiber_cm55_ntz_runtime.a"
        $elfFixtureObject = Join-Path $modeDir "fixture.o"
        $competingObject = Join-Path $modeDir "competing.o"
        $elfPath = Join-Path $modeDir "runtime.elf"
        $vectorBinary = Join-Path $modeDir "vectors.bin"
        ...
        & $Compiler @archiveArgs -c $frameSourcePath -o $archiveRuntimeObject
        if ($LASTEXITCODE -ne 0) {
            throw "ARM_CM55_NTZ runtime archive source failed compile ($($archiveMode.Name))"
        }
        & $Compiler @archiveArgs -c $bootSourcePath -o $archiveBootObject
        if ($LASTEXITCODE -ne 0) {
            throw "ARM_CM55_NTZ boot archive source failed compile ($($archiveMode.Name))"
        }
        & $Ar rcs $archivePath $archiveRuntimeObject $archiveBootObject
        if ($LASTEXITCODE -ne 0) {
            throw "ARM_CM55_NTZ runtime archive creation failed ($($archiveMode.Name))"
        }
        & $Compiler @archiveArgs -c $elfFixturePath -o $elfFixtureObject
        if ($LASTEXITCODE -ne 0) {
            throw "ARM_CM55_NTZ runtime ELF fixture failed compile ($($archiveMode.Name))"
        }
        & $Compiler @archiveArgs -c $competingPath -o $competingObject
        if ($LASTEXITCODE -ne 0) {
            throw "ARM_CM55_NTZ competing-handler fixture failed compile ($($archiveMode.Name))"
        }

        $linkArgs = @(
            "-mcpu=cortex-m55", "-mthumb", "-mfloat-abi=soft", "-nostdlib") +
            $archiveMode.Flags + @(
            "-Wl,--gc-sections", "-T", $linkerPath,
            $elfFixtureObject, $archivePath, "-o", $elfPath)
        & $Compiler @linkArgs
        if ($LASTEXITCODE -ne 0) {
            throw "ARM_CM55_NTZ runtime archive/ELF proof failed link ($($archiveMode.Name))"
        }
        $objectNm = if ($archiveMode.Name -eq "lto") { $GccNm } else { $Nm }
        $elfDefined = @(& $objectNm -g --defined-only $elfPath)
        if ($LASTEXITCODE -ne 0) {
            throw "nm failed for ARM_CM55_NTZ runtime ELF ($($archiveMode.Name))"
        }
        $svcSymbols = @($elfDefined | Where-Object {
            $_ -match '^([0-9a-fA-F]+)\s+T\s+SVC_Handler$'
        })
        if ($svcSymbols.Count -ne 1) {
            throw "ARM_CM55_NTZ runtime ELF must contain one strong SVC_Handler ($($archiveMode.Name))"
        }
        $pendSvSymbols = @($elfDefined | Where-Object {
            $_ -match '^([0-9a-fA-F]+)\s+T\s+PendSV_Handler$'
        })
        if ($pendSvSymbols.Count -ne 1) {
            throw "ARM_CM55_NTZ runtime ELF must contain one strong PendSV_Handler ($($archiveMode.Name))"
        }
        [void]($svcSymbols[0] -match '^([0-9a-fA-F]+)')
        $svcAddress = [Convert]::ToUInt32($Matches[1], 16)
        [void]($pendSvSymbols[0] -match '^([0-9a-fA-F]+)')
        ...
        & $Objcopy -O binary --only-section=.isr_vector $elfPath $vectorBinary
        if ($LASTEXITCODE -ne 0) {
            throw "objcopy failed for ARM_CM55_NTZ runtime vector proof ($($archiveMode.Name))"
        }
        $vectorBytes = [IO.File]::ReadAllBytes($vectorBinary)
        if ($vectorBytes.Length -lt (16 * 4)) {
            throw "ARM_CM55_NTZ synthetic vector table is truncated ($($archiveMode.Name))"
        }
        $svcVector = [BitConverter]::ToUInt32($vectorBytes, 11 * 4)
        $pendSvVector = [BitConverter]::ToUInt32($vectorBytes, 14 * 4)
        if ($svcVector -ne ($svcAddress -bor [uint32]1)) {
            throw "ARM_CM55_NTZ vector slot 11 lost strong SVC_Handler ($($archiveMode.Name))"
        }
        if ($pendSvVector -ne ($pendSvAddress -bor [uint32]1)) {
            throw "ARM_CM55_NTZ vector slot 14 lost strong PendSV_Handler ($($archiveMode.Name))"
        }
        ...
        if (($duplicate.ExitCode -eq 0) -or
                ($duplicate.Output -notmatch 'multiple definition.*(?:SVC_Handler|PendSV_Handler)')) {
            throw "ARM_CM55_NTZ competing strong handlers must fail link ($($archiveMode.Name))`n$($duplicate.Output)"
        }
    }
    ...
    $negativeCases = @(
        [pscustomobject]@{
            ...
            CompilerArgs = @("-mcpu=cortex-m55", "-mthumb", "-mfloat-abi=soft")
            Defines = @("-DFIBER_PORT_PROFILE=5")
            Diagnostic = "ARM_CM55_NTZ is build-selected only"
            Main = $mainHeader
        },
        [pscustomobject]@{
            Name = "secure-cmse"
            CompilerArgs = @("-mcpu=cortex-m55", "-mthumb", "-mfloat-abi=soft", "-mcmse")
            Defines = @("-DFIBER_PORT_BUILD_SELECTED=1", "-DFIBER_PORT_ARMV81M_MAINLINE=1")
            Diagnostic = "does not accept a Secure CMSE build"
            Main = $mainHeader
        },
        [pscustomobject]@{
    ...
    foreach ($case in $negativeCases) {
        ...
        if (($result.ExitCode -eq 0) -or
                ($result.Output.IndexOf($case.Diagnostic,
                [System.StringComparison]::Ordinal) -lt 0)) {
            throw "Invalid ARM_CM55_NTZ manifest failed for the wrong reason: $($case.Name)`n$($result.Output)"
        }
    }
}


function Test-ArmCm55FNtzRuntimeContract {
    param(
        [string]$RepositoryRoot,
        [string]$Compiler,
        [string]$Nm,
    ...
    $profileDir = Join-Path $RepositoryRoot `
        "fiber\port\ARM_CM55F_NTZ\non_secure"
    $fixture = Join-Path $RepositoryRoot `
        "tools\fixtures\arm_cm55f_ntz_layout_probe.c"
    $referenceFixture = Join-Path $RepositoryRoot `
        "tools\fixtures\arm_cm55f_ntz_freertos_frame_reference.c"
    $svcReferenceFixture = Join-Path $RepositoryRoot `
        "tools\fixtures\arm_cm55f_ntz_freertos_svc_reference.c"
    $pendsvReferenceFixture = Join-Path $RepositoryRoot `
        "tools\fixtures\arm_cm55f_ntz_freertos_pendsv_reference.c"
    $expectedFiles = @(
        "FREERTOS_PARITY.md",
        "fiber_port.c",
        "fiber_port_boot.c",
    ...
    foreach ($file in $expectedFiles) {
        ...
        if (-not (Test-Path -LiteralPath $path)) {
            throw "ARM_CM55F_NTZ slice-4 file is missing: $path"
        }
    }
    foreach ($requiredFixture in @($fixture, $referenceFixture,
        ...
        if (-not (Test-Path -LiteralPath $requiredFixture)) {
            throw "ARM_CM55F_NTZ slice-4 fixture is missing: $requiredFixture"
        }
    }
    ...
    if (($unexpectedFiles.Count -ne 0) -or ($missingFiles.Count -ne 0)) {
        throw "ARM_CM55F_NTZ slice-4 file surface changed; expected complete scalar-FP runtime artifacts"
    }
    ...
    foreach ($selector in @(
            ...
            (Join-Path $RepositoryRoot "fiber\port\fiber_port_selected.h"))) {
        if ((Get-Content -LiteralPath $selector -Raw).IndexOf(
                "ARM_CM55F_NTZ", [System.StringComparison]::Ordinal) -ge 0) {
            throw "Staged ARM_CM55F_NTZ profile entered global selection: $selector"
        }
    }
    ...
    foreach ($forbiddenTypeDependency in @(
        ...
        $includePattern = '(?m)^\s*#\s*include\s*["<][^">]*' +
            [regex]::Escape($forbiddenTypeDependency) + '[">]'
        if ([regex]::IsMatch($typeText, $includePattern)) {
            throw "ARM_CM55F_NTZ public storage acquired CPU dependency: $forbiddenTypeDependency"
        }
    }
    ...
    foreach ($requiredMacroToken in @(
            ...
            "FIBER_PORT_HAS_MVE 0",
            "FIBER_PORT_CONTEXT_ABI_PORT_ID 0x43353546u",
            "FIBER_PORT_CONTEXT_ABI_FEATURE_MASK == 0x83u",
        ...
        if ($macro.IndexOf($requiredMacroToken,
                [System.StringComparison]::Ordinal) -lt 0) {
            throw "ARM_CM55F_NTZ portmacro lost frozen trait: $requiredMacroToken"
        }
    }
    ...
    foreach ($requiredParity in @(
        ...
        if ($parity.IndexOf($requiredParity,
                [System.StringComparison]::Ordinal) -lt 0) {
            throw "ARM_CM55F_NTZ parity ledger lost staged evidence: $requiredParity"
        }
    }

    $probeDir = Join-Path $BuildRoot "arm-cm55f-ntz-layout"
    New-Item -ItemType Directory -Path $probeDir | Out-Null
    $mainHeader = @"
#ifndef MAIN_H_
#define MAIN_H_
...
_Static_assert(sizeof(FiberContext) == 76u, "context size");
_Static_assert(_Alignof(FiberContext) == 4u, "context alignment");
FiberContext fiber_cm55f_ntz_type_only_object;
'@
    $typeObject = Join-Path $probeDir "type-only.o"
    & $Compiler -x c "-march=armv8.1-m.main+fp" -mthumb -mfloat-abi=hard `
        -std=c11 -ffreestanding -fno-builtin -Wall -Wextra -Werror `
        "-I$profileDir" -c $typeProbe -o $typeObject
    if ($LASTEXITCODE -ne 0) {
        throw "ARM_CM55F_NTZ type-only C header failed without CMSIS"
    }
...
static_assert(sizeof(FiberContext) == 76u, "context size");
static_assert(alignof(FiberContext) == 4u, "context alignment");
FiberContext fiber_cm55f_ntz_cpp_type_only_object;
'@
    $cppObject = Join-Path $probeDir "type-only-cpp.o"
    & $Compiler -x c++ "-march=armv8.1-m.main+fp" -mthumb -mfloat-abi=hard `
    ...
    foreach ($mode in $modes) {
        ...
        if ($LASTEXITCODE -ne 0) {
            throw "ARM_CM55F_NTZ layout compile failed: $($mode.Name)"
        }
        $defined = @(& $Nm -g --defined-only $object)
        if ($LASTEXITCODE -ne 0) {
            throw "nm failed for ARM_CM55F_NTZ layout: $($mode.Name)"
        }
        $cohorts = @($defined | ForEach-Object {
            if ($_ -match '\b(fiber_port_context_cohort_\S+)$') {
                $Matches[1]
        ...
        if ($cohorts.Count -ne 1) {
            throw "ARM_CM55F_NTZ layout must define one exact cohort: $($mode.Name)"
        }
        foreach ($token in @(
                "armv81m_mainline",
                "p0x43353546u",
            ...
            if ($cohorts[0].IndexOf($token,
                    [System.StringComparison]::Ordinal) -lt 0) {
                throw "ARM_CM55F_NTZ exact cohort lost token ${token}: $($cohorts[0])"
            }
        }
    }
    ...
    $invalidCases = @(
        ...
        [pscustomobject]@{
            Name = "no-fp-compiler-abi"
            CpuArgs = @("-march=armv8.1-m.main", "-mfloat-abi=soft")
            Defines = @()
        ...
        [pscustomobject]@{
            Name = "m55-cpu-hard-fp"
            CpuArgs = @("-mcpu=cortex-m55", "-mfpu=fpv5-sp-d16", "-mfloat-abi=hard")
            Defines = @()
            Diagnostic = "does not permit MVE"
        },
        [pscustomobject]@{
            Name = "secure-cmse"
            CpuArgs = @("-march=armv8.1-m.main+fp", "-mfloat-abi=hard", "-mcmse")
    ...
    foreach ($case in $invalidCases) {
        ...
        if (($result.ExitCode -eq 0) -or
                ($result.Output.IndexOf($case.Diagnostic,
                    [System.StringComparison]::Ordinal) -lt 0)) {
            throw "Invalid ARM_CM55F_NTZ manifest failed for the wrong reason: $($case.Name)`n$($result.Output)"
        }
    }
    ...
    foreach ($assignment in @(
        ...
        $assignmentIndex = $frameBody.IndexOf($assignment,
            [System.StringComparison]::Ordinal)
        if ($assignmentIndex -le $lastAssignment) {
            throw "ARM_CM55F_NTZ initial frame lost exact FreeRTOS assignment order: $assignment"
        }
        $lastAssignment = $assignmentIndex
    }
    if (([regex]::Matches($frameBody,
            'frame\[fiber_portFRAME_[A-Z0-9_]+\]\s*=')).Count -ne 18) {
        throw "ARM_CM55F_NTZ constructor must assign exactly eighteen basic-frame words"
    }
    ...
    foreach ($requiredConstructionToken in @(
        ...
        if (($frameSource.IndexOf($requiredConstructionToken,
                ...
                ($privateHeader.IndexOf($requiredConstructionToken,
                    [System.StringComparison]::Ordinal) -lt 0)) {
            throw "ARM_CM55F_NTZ construction source lost contract: $requiredConstructionToken"
        }
    }
    ...
    foreach ($forbiddenBootToken in @(
        ...
        if (($frameSource.IndexOf($forbiddenBootToken,
                ...
                ($bootSource.IndexOf($forbiddenBootToken,
                    [System.StringComparison]::Ordinal) -ge 0)) {
            throw "ARM_CM55F_NTZ boot object must not own SVC/runtime wrappers: $forbiddenBootToken"
        }
    }
    ...
    foreach ($requiredSvcToken in @(
        ...
        if (($svcSource.IndexOf($requiredSvcToken,
                ...
                ($privateHeader.IndexOf($requiredSvcToken,
                    [System.StringComparison]::Ordinal) -lt 0)) {
            throw "ARM_CM55F_NTZ strict SVC source lost contract: $requiredSvcToken"
        }
    }
    ...
    foreach ($forbiddenSvcDefinition in @(
        ...
        if ($svcSource -match $forbiddenSvcDefinition) {
            throw "ARM_CM55F_NTZ SVC object must not define PendSV runtime: $forbiddenSvcDefinition"
        }
    }
    ...
    foreach ($requiredRuntimeToken in @(
        ...
        if (($runtimeSource.IndexOf($requiredRuntimeToken,
                ...
                ($privateHeader.IndexOf($requiredRuntimeToken,
                [System.StringComparison]::Ordinal) -lt 0)) {
            throw "ARM_CM55F_NTZ scalar-FP runtime source lost contract: $requiredRuntimeToken"
        }
    }
    if ([regex]::Matches($runtimeSource,
            'FIBER_PORT_CONTEXT_COHORT_RETAIN\(\);').Count -ne 1) {
        throw "ARM_CM55F_NTZ must retain the exact cohort once from the one-shot first-selection bridge"
    }
    ...
    if ($pendSvBodySource.IndexOf("FIBER_PORT_CONTEXT_COHORT_RETAIN();",
            [System.StringComparison]::Ordinal) -ge 0) {
        throw "ARM_CM55F_NTZ PendSV must not add an exact-cohort read to the hot path"
    }
    ...
    foreach ($mode in $modes) {
        ...
        if ($LASTEXITCODE -ne 0) {
            throw "ARM_CM55F_NTZ frame construction failed compile: $($mode.Name)"
        }
        & $Compiler @compileArgs -c $svcSourcePath -o $svcObject
        if ($LASTEXITCODE -ne 0) {
            throw "ARM_CM55F_NTZ strict SVC path failed compile: $($mode.Name)"
        }
        & $Compiler @compileArgs -c $runtimeSourcePath -o $runtimeObject
        if ($LASTEXITCODE -ne 0) {
            throw "ARM_CM55F_NTZ scalar-FP PendSV runtime failed compile: $($mode.Name)"
        }
        ...
        if ($LASTEXITCODE -ne 0) {
            throw "Pinned FreeRTOS M55F frame fixture failed compile: $($mode.Name)"
        }
        & $Compiler @($mode.CpuArgs + @(
            "-std=c11", "-ffreestanding", "-fno-builtin", "-Wall",
            "-Wextra", "-Werror", $mode.Optimization,
        ...
        if ($LASTEXITCODE -ne 0) {
            throw "Pinned FreeRTOS M55F SVC fixture failed compile: $($mode.Name)"
        }
        & $Compiler @($mode.CpuArgs + @(
            "-std=c11", "-ffreestanding", "-fno-builtin", "-Wall",
            "-Wextra", "-Werror", $mode.Optimization,
        ...
        if ($LASTEXITCODE -ne 0) {
            throw "Pinned FreeRTOS M55F PendSV fixture failed compile: $($mode.Name)"
        }

        & $Compiler -r $frameObject $svcObject $runtimeObject -o $groupObject
        if ($LASTEXITCODE -ne 0) {
            throw "ARM_CM55F_NTZ complete runtime group failed link: $($mode.Name)"
        }

        $defined = @(& $Nm -g --defined-only $groupObject)
        if ($LASTEXITCODE -ne 0) {
            throw "nm failed for ARM_CM55F_NTZ complete runtime group: $($mode.Name)"
        }
        foreach ($requiredDefinition in $requiredRuntimeDefinitions) {
            if ((@($defined | Where-Object {
                ...
                throw "ARM_CM55F_NTZ complete runtime group must define one $requiredDefinition"
            }
        }
        ...
        if (($groupCohorts.Count -ne 1) -or
            ...
            throw "ARM_CM55F_NTZ complete runtime group disagrees with exact cohort"
        }
        ...
        Assert-ExactSymbolSet -Expected @(
            ...
            "ARM_CM55F_NTZ complete runtime reverse/integration surface ($($mode.Name))"

        $frameDisassembly = (& $Objdump -d $frameObject) -join "`n"
        if ($LASTEXITCODE -ne 0) {
            throw "objdump failed for ARM_CM55F_NTZ frame: $($mode.Name)"
        }
        $referenceDisassembly = (& $Objdump -d $referenceObject) -join "`n"
        if ($LASTEXITCODE -ne 0) {
            throw "objdump failed for FreeRTOS M55F frame fixture: $($mode.Name)"
        }
        ...
        $generatedReferenceBody = Get-DisassemblyFunctionBody `
            ...
            -Symbol "fiber_test_freertos_cm55f_initialise_stack" `
        ...
        foreach ($generatedBody in @($generatedFrameBody,
            ...
            if ($generatedBody -match
                ...
                throw "ARM_CM55F_NTZ construction/FPU policy emitted FP or MVE instruction: $($mode.Name)"
            }
        }
        foreach ($generatedBody in @($generatedFrameBody,
                $generatedReferenceBody)) {
            if ($generatedBody -notmatch '#-?(?:72|0x48)\b') {
                throw "ARM_CM55F_NTZ initial construction lost the 72-byte basic frame: $($mode.Name)"
            }
        }
        ...
        if ($LASTEXITCODE -ne 0) {
            throw "objdump failed for ARM_CM55F_NTZ SVC path: $($mode.Name)"
        }
        $svcReferenceDisassembly = (& $Objdump -d $svcReferenceObject) -join "`n"
        if ($LASTEXITCODE -ne 0) {
            throw "objdump failed for FreeRTOS M55F SVC fixture: $($mode.Name)"
        }
        $runtimeDisassembly = (& $Objdump -dr $runtimeObject) -join "`n"
        if ($LASTEXITCODE -ne 0) {
            throw "objdump failed for ARM_CM55F_NTZ PendSV runtime: $($mode.Name)"
        }
        $pendsvReferenceDisassembly = (& $Objdump -d $pendsvReferenceObject) -join "`n"
        if ($LASTEXITCODE -ne 0) {
            throw "objdump failed for FreeRTOS M55F PendSV fixture: $($mode.Name)"
        }
        ...
        $referenceStartBody = Get-DisassemblyFunctionBody `
            ...
            -Symbol "fiber_test_freertos_cm55f_vstart_first" `
            -Path $svcReferenceObject
        $referenceRestoreBody = Get-DisassemblyFunctionBody `
            -Disassembly $svcReferenceDisassembly `
            -Symbol "fiber_test_freertos_cm55f_restore_first" `
            -Path $svcReferenceObject
        $pendSvBody = Get-DisassemblyFunctionBody `
        ...
        $referencePendSvBody = Get-DisassemblyFunctionBody `
            ...
            -Symbol "fiber_test_freertos_cm55f_pendsv" `
        ...
        foreach ($generatedBody in @($startBody, $svcBody,
            ...
            if ($generatedBody -match
                    '(?im)^\s*[0-9a-f]+:\s+[0-9a-f ]+\s+v[a-z0-9.]+' ) {
                throw "ARM_CM55F_NTZ SVC first-start emitted FP or MVE instruction: $($mode.Name)"
            }
        }
        ...
        foreach ($requiredStartInstruction in @(
            ...
            if ($referenceStartBody -notmatch $requiredStartInstruction) {
                throw "Pinned FreeRTOS M55F SVC fixture lost transfer instruction: $requiredStartInstruction"
            }
            if ($startBody -notmatch $requiredStartInstruction) {
                throw "ARM_CM55F_NTZ first-start lost FreeRTOS transfer instruction: $requiredStartInstruction ($($mode.Name))"
            }
        }
        foreach ($fiberOnlyStartInstruction in @(
            ...
            if ($startBody -notmatch $fiberOnlyStartInstruction) {
                throw "ARM_CM55F_NTZ strict first-start hardening disappeared: $fiberOnlyStartInstruction ($($mode.Name))"
            }
        }
        foreach ($requiredRestoreInstruction in @(
            ...
            if ($referenceRestoreBody -notmatch $requiredRestoreInstruction) {
                throw "Pinned FreeRTOS M55F restore fixture lost instruction: $requiredRestoreInstruction"
            }
            if ($svcBody -notmatch $requiredRestoreInstruction) {
                throw "ARM_CM55F_NTZ SVC restore lost FreeRTOS instruction: $requiredRestoreInstruction ($($mode.Name))"
            }
        }
        foreach ($requiredSvcInstruction in @(
            ...
            if ($svcBody -notmatch $requiredSvcInstruction) {
                throw "ARM_CM55F_NTZ strict SVC provenance/restore instruction disappeared: $requiredSvcInstruction ($($mode.Name))"
            }
        }
        ...
        foreach ($requiredReferencePendSvInstruction in @(
            ...
            if ($referencePendSvBody -notmatch $requiredReferencePendSvInstruction) {
                throw "Pinned FreeRTOS M55F PendSV fixture lost instruction: $requiredReferencePendSvInstruction ($($mode.Name))"
            }
        }
        ...
        foreach ($instructionPattern in $pendSvInstructionOrder) {
            ...
            if (-not $instructionMatch.Success) {
                throw "ARM_CM55F_NTZ generated FP-aware PendSV lost FreeRTOS-parity instruction order: $instructionPattern`n$pendSvBody"
            }
            $remainingPendSvBody = $remainingPendSvBody.Substring(
                $instructionMatch.Index + $instructionMatch.Length)
        }
    ...
    # The scalar-FP frame geometry must remain valid with the production hot
    # path, the fully paranoid address/hash mode, and without MSP rewind.
    $hardCpuArgs = @("-march=armv8.1-m.main+fp", "-mfloat-abi=hard")
    $sourceVariants = @(
        [pscustomobject]@{
            Name = "production"
            Defines = @(
    ...
    foreach ($variant in $sourceVariants) {
        ...
        if ($LASTEXITCODE -ne 0) {
            throw "ARM_CM55F_NTZ $($variant.Name) boot variant failed compile"
        }
        & $Compiler @variantArgs -c $svcSourcePath -o $variantSvc
        if ($LASTEXITCODE -ne 0) {
            throw "ARM_CM55F_NTZ $($variant.Name) SVC variant failed compile"
        }
        & $Compiler @variantArgs -c $runtimeSourcePath -o $variantRuntime
        if ($LASTEXITCODE -ne 0) {
            throw "ARM_CM55F_NTZ $($variant.Name) PendSV variant failed compile"
        }
        & $Compiler -r $variantBoot $variantSvc $variantRuntime -o $variantGroup
        if ($LASTEXITCODE -ne 0) {
            throw "ARM_CM55F_NTZ $($variant.Name) runtime group failed link"
        }
    }
    ...
    if ($LASTEXITCODE -ne 0) {
        throw "ARM_CM55F_NTZ adversarial SVC source failed compile"
    }
    & $Compiler @adversarialArgs -c $runtimeSourcePath -o $adversarialRuntimeObject
    if ($LASTEXITCODE -ne 0) {
        throw "ARM_CM55F_NTZ adversarial PendSV source failed compile"
    }
...
'@
    ...
    foreach ($archiveMode in @(
        ...
        $archiveRuntimeObject = Join-Path $modeDir "runtime.o"
        $archivePath = Join-Path $modeDir "libfiber_cm55f_ntz_runtime.a"
        ...
        foreach ($compile in @(
            ...
            if ($LASTEXITCODE -ne 0) {
                throw "ARM_CM55F_NTZ runtime archive $($compile.Name) source failed compile ($($archiveMode.Name))"
            }
        }
        & $Ar rcs $archivePath $archiveBootObject $archiveSvcObject $archiveRuntimeObject
        if ($LASTEXITCODE -ne 0) {
            throw "ARM_CM55F_NTZ runtime archive creation failed ($($archiveMode.Name))"
        }
        & $Compiler @archiveArgs -c $elfFixturePath -o $fixtureObject
        if ($LASTEXITCODE -ne 0) {
            throw "ARM_CM55F_NTZ runtime ELF fixture failed compile ($($archiveMode.Name))"
        }
        & $Compiler @archiveArgs -c $expectationSource -o $expectationObject
        if ($LASTEXITCODE -ne 0) {
            throw "ARM_CM55F_NTZ external cohort expectation failed compile ($($archiveMode.Name))"
        }
        & $Compiler @archiveArgs -c $competingPath -o $competingObject
        if ($LASTEXITCODE -ne 0) {
            throw "ARM_CM55F_NTZ competing-handler fixture failed compile ($($archiveMode.Name))"
        }
        ...
        & $Compiler @linkArgs
        if ($LASTEXITCODE -ne 0) {
            throw "ARM_CM55F_NTZ runtime archive/ELF proof failed link ($($archiveMode.Name))"
        }

        $objectNm = if ($archiveMode.Name -eq "lto") { $GccNm } else { $Nm }
        $elfDefined = @(& $objectNm -g --defined-only $elfPath)
        if ($LASTEXITCODE -ne 0) {
            throw "nm failed for ARM_CM55F_NTZ runtime ELF ($($archiveMode.Name))"
        }
        $svcSymbols = @($elfDefined | Where-Object {
            $_ -match '^([0-9a-fA-F]+)\s+T\s+SVC_Handler$'
        })
        ...
        if (($svcSymbols.Count -ne 1) -or ($pendSvSymbols.Count -ne 1)) {
            throw "ARM_CM55F_NTZ runtime ELF must contain one strong SVC_Handler and PendSV_Handler ($($archiveMode.Name))"
        }
        ...
        if (($elfCohorts.Count -ne 1) -or ($elfCohorts[0] -ne $expectedCohort)) {
            throw "ARM_CM55F_NTZ runtime ELF lost the exact selected cohort ($($archiveMode.Name))"
        }
        $sections = (& $Objdump -h $elfPath) -join "`n"
        if ($LASTEXITCODE -ne 0) {
            throw "objdump failed for ARM_CM55F_NTZ cohort expectation section ($($archiveMode.Name))"
        }
        if ($sections -notmatch '\.fiber_port_context_cohort_expectation\s+00000004\s+[0-9a-fA-F]+\s+') {
            throw "ARM_CM55F_NTZ runtime ELF lost the retained four-byte external cohort expectation ($($archiveMode.Name))"
        }
        [void]($svcSymbols[0] -match '^([0-9a-fA-F]+)')
        $svcAddress = [Convert]::ToUInt32($Matches[1], 16)
        [void]($pendSvSymbols[0] -match '^([0-9a-fA-F]+)')
        ...
        & $Objcopy -O binary --only-section=.isr_vector $elfPath $vectorBinary
        if ($LASTEXITCODE -ne 0) {
            throw "objcopy failed for ARM_CM55F_NTZ runtime vector proof ($($archiveMode.Name))"
        }
        $vectorBytes = [IO.File]::ReadAllBytes($vectorBinary)
        if ($vectorBytes.Length -lt (16 * 4)) {
            throw "ARM_CM55F_NTZ runtime synthetic vector table is truncated ($($archiveMode.Name))"
        }
        if ([BitConverter]::ToUInt32($vectorBytes, 11 * 4) -ne
                ($svcAddress -bor [uint32]1)) {
            throw "ARM_CM55F_NTZ runtime vector slot 11 lost strong SVC_Handler ($($archiveMode.Name))"
        }
        if ([BitConverter]::ToUInt32($vectorBytes, 14 * 4) -ne
                ($pendSvAddress -bor [uint32]1)) {
            throw "ARM_CM55F_NTZ runtime vector slot 14 lost strong PendSV_Handler ($($archiveMode.Name))"
        }
        ...
        if (($duplicate.ExitCode -eq 0) -or
                ($duplicate.Output -notmatch 'multiple definition.*(?:SVC_Handler|PendSV_Handler)')) {
            throw "ARM_CM55F_NTZ competing strong handlers must fail link ($($archiveMode.Name)).`n$($duplicate.Output)"
        }
    }
}
...
function Test-ArmCm55MvefNtzConstructionContract {
    ...
    $macroPath = Join-Path $profileDir "fiber_portmacro.h"
    $macro = Get-Content -LiteralPath $macroPath -Raw
    foreach ($requiredMacroToken in @(
            "FIBER_PORT_ARMV81M_MAINLINE",
            "__CORTEX_M != 55",
    ...
    $probeDir = Join-Path $BuildRoot "arm-cm55-mvef-ntz-construction"
    New-Item -ItemType Directory -Path $probeDir | Out-Null
    $mainHeader = @"
#ifndef MAIN_H_
#define MAIN_H_
...
'@
    & $Compiler -x c "-march=armv8.1-m.main+mve.fp" -mthumb -mfloat-abi=hard `
        -std=c11 -ffreestanding -fno-builtin -Wall -Wextra -Werror `
        "-I$profileDir" -c $typeProbe -o (Join-Path $probeDir "type-only.o")
    if ($LASTEXITCODE -ne 0) {
        throw "ARM_CM55_MVEF_NTZ type-only C header failed without CMSIS"
    }
...
'@
    ...
    $cohortFromLayout = $null
    foreach ($mode in $modes) {
        $object = Join-Path $probeDir ("layout-" + $mode.Name + ".o")
        & $Compiler @($mode.CpuArgs + $baseArgs + @(
            $mode.Optimization, "-c", $fixture, "-o", $object))
        if ($LASTEXITCODE -ne 0) {
            throw "ARM_CM55_MVEF_NTZ layout compile failed: $($mode.Name)"
        ...
        if (($null -ne $cohortFromLayout) -and ($cohortFromLayout -ne $cohorts[0])) {
            throw "ARM_CM55_MVEF_NTZ cohort changed between compiler ABI/optimization modes"
        }
        $cohortFromLayout = $cohorts[0]
    }
    ...
    $invalidCases = @(
        ...
        [pscustomobject]@{
            ...
            CpuArgs = @("-march=armv8.1-m.main+mve.fp", "-mfloat-abi=hard")
            Defines = @("-DFIBER_PORT_RUNTIME_SELECTABLE=1")
            Diagnostic = "runtime-selectable state must not be predefined"
        }
    )
    foreach ($case in $invalidCases) {
    ...
    $bootSource = Get-Content -LiteralPath $bootSourcePath -Raw
    $privateHeader = Get-Content -LiteralPath $privateHeaderPath -Raw
    $frameBody = Get-CFunctionBody -Source $bootSource `
        -Signature "void fiber_port_init_context_frame(FiberContext *const ctx)" `
        -Path $bootSourcePath
    $contextInitBody = Get-CFunctionBody -Source $bootSource `
        -Signature "void fiber_port_context_init(FiberContext *const ctx," `
        -Path $bootSourcePath
    $lastAssignment = -1
    foreach ($assignment in @(
            "fiber_portFRAME_PSPLIM] =",
    ...
    if (([regex]::Matches($bootSource,
            'FIBER_PORT_CONTEXT_COHORT_RETAIN\(\);').Count -ne 1) -or
            ($contextInitBody.IndexOf("FIBER_PORT_CONTEXT_COHORT_RETAIN();",
                [System.StringComparison]::Ordinal) -lt 0)) {
        throw "ARM_CM55_MVEF_NTZ must retain its exact cohort once from one-shot context construction"
    }
    ...
    foreach ($requiredConstructionToken in @(
            ...
            "FIBER_FPU_LAZY",
            "FIBER_GENERAL_REGS_ONLY")) {
        if (($bootSource.IndexOf($requiredConstructionToken,
                    [System.StringComparison]::Ordinal) -lt 0) -and
                ($privateHeader.IndexOf($requiredConstructionToken,
                    [System.StringComparison]::Ordinal) -lt 0)) {
            throw "ARM_CM55_MVEF_NTZ construction source lost contract: $requiredConstructionToken"
    ...
    foreach ($forbiddenRuntimeToken in @(
        ...
        if (($bootSource.IndexOf($forbiddenRuntimeToken,
                ...
                ($privateHeader.IndexOf($forbiddenRuntimeToken,
                    [System.StringComparison]::Ordinal) -ge 0)) {
            throw "ARM_CM55_MVEF_NTZ construction slice leaked runtime surface: $forbiddenRuntimeToken"
        }
    }
    ...
    foreach ($mode in $modes) {
        ...
        $compileArgs = @($mode.CpuArgs + $baseArgs + @(
            ...
            "-DFIBER_FPU_LAZY=$lazy",
            "-save-temps=obj"))
        $bootObject = Join-Path $modeDir "construction.o"
        $groupObject = Join-Path $modeDir "construction-group.o"
        $referenceObject = Join-Path $modeDir "freertos-frame.o"
        & $Compiler @compileArgs -c $bootSourcePath -o $bootObject
        if ($LASTEXITCODE -ne 0) {
            throw "ARM_CM55_MVEF_NTZ construction source failed compile: $($mode.Name)"
        }
        & $Compiler @($mode.CpuArgs + @(
            "-std=c11", "-ffreestanding", "-fno-builtin", "-Wall",
            "-Wextra", "-Werror", $mode.Optimization,
        ...
        & $Compiler -r $bootObject -o $groupObject
        if ($LASTEXITCODE -ne 0) {
            throw "ARM_CM55_MVEF_NTZ construction group failed link: $($mode.Name)"
        }

        $defined = @(& $Nm -g --defined-only $groupObject)
        if ($LASTEXITCODE -ne 0) {
            throw "nm failed for ARM_CM55_MVEF_NTZ construction group: $($mode.Name)"
        }
        foreach ($requiredDefinition in $requiredDefinitions) {
            if ((@($defined | Where-Object {
                    $_ -match ("\bT\s+" +
                        [regex]::Escape($requiredDefinition) + "$")
                })).Count -ne 1) {
                throw "ARM_CM55_MVEF_NTZ construction group must define one $requiredDefinition"
            }
        }
        foreach ($forbiddenDefinition in @(
            ...
            if (@($defined | Where-Object {
                ...
                throw "ARM_CM55_MVEF_NTZ construction group unexpectedly defines $forbiddenDefinition"
            }
        }
        $groupCohorts = @($defined | ForEach-Object {
            if ($_ -match '\b(fiber_port_context_cohort_\S+)$') {
        ...
        if (($groupCohorts.Count -ne 1) -or
                ($groupCohorts[0] -ne $cohortFromLayout)) {
            throw "ARM_CM55_MVEF_NTZ construction group disagrees with exact C55V cohort"
        }
        $undefined = @(& $Nm -u $groupObject)
        if ($LASTEXITCODE -ne 0) {
            throw "nm failed for ARM_CM55_MVEF_NTZ construction undefined surface: $($mode.Name)"
        }
        Assert-ExactSymbolSet -Expected @(
            "fiber_internal_task_return",
            "fiber_panic") -Actual (Get-NmUndefinedSymbolNames `
            -NmOutput $undefined -Path $groupObject) `
            -Description "ARM_CM55_MVEF_NTZ construction reverse/integration surface ($($mode.Name))"

        $bootDisassembly = (& $Objdump -dr $bootObject) -join "`n"
        $referenceDisassembly = (& $Objdump -dr $referenceObject) -join "`n"
        if ($LASTEXITCODE -ne 0) {
            throw "objdump failed for ARM_CM55_MVEF_NTZ construction: $($mode.Name)"
        }
        $generatedFrameBody = Get-DisassemblyFunctionBody `
            -Disassembly $bootDisassembly -Symbol "fiber_port_init_context_frame" `
            -Path $bootObject
        $generatedReferenceBody = Get-DisassemblyFunctionBody `
            -Disassembly $referenceDisassembly `
            -Symbol "fiber_test_freertos_cm55_mvef_initialise_stack" `
            -Path $referenceObject
        foreach ($body in @($generatedFrameBody, $generatedReferenceBody)) {
            if ($body -match '\bv(?:stm|ldm|push|pop|mov|ldr|str)\w*\b') {
                throw "ARM_CM55_MVEF_NTZ basic construction emitted an FP/MVE instruction: $($mode.Name)"
            }
        }
        if (($generatedFrameBody -notmatch '#-?(?:72|0x48)\b') -or
                ($generatedReferenceBody -notmatch '#-?(?:72|0x48)\b')) {
            throw "ARM_CM55_MVEF_NTZ initial construction lost the 72-byte basic frame: $($mode.Name)"
        }
...
function Test-ArmCm33TfmRuntimeContract {
    param(
        [string]$RepositoryRoot,
        [string]$Compiler,
        [string]$Nm,
    ...
    $layoutFixture = Join-Path $RepositoryRoot `
        "tools\fixtures\arm_cm33_tfm_layout_probe.c"
    $runtimeSource = Join-Path $profileDir "fiber_port.c"
    $bootSource = Join-Path $profileDir "fiber_port_boot.c"
    $tfmSource = Join-Path $profileDir "fiber_port_tfm.c"
    $wrapperSource = Join-Path $profileDir "fiber_port_tfm_os_wrapper.c"
    foreach ($required in @(
            (Join-Path $profileDir "fiber_port_types.h"),
            (Join-Path $profileDir "fiber_port_boot_types.h"),
            (Join-Path $profileDir "fiber_portmacro.h"),
            (Join-Path $profileDir "fiber_port_boot.h"),
            (Join-Path $profileDir "fiber_port_private.h"),
            (Join-Path $profileDir "fiber_port_tfm_abi.h"),
            (Join-Path $profileDir "fiber_port_tfm_os_wrapper.h"),
            $runtimeSource, $bootSource, $tfmSource, $wrapperSource,
            (Join-Path $profileDir "FREERTOS_PARITY.md"),
            $layoutFixture)) {
        if (-not (Test-Path -LiteralPath $required)) {
            throw "ARM_CM33_TFM runtime slice is missing: $required"
        }
    }
    ...
    foreach ($forbiddenRuntime in @(
            ...
            "fiber_port_svc.c")) {
        if (Test-Path -LiteralPath (Join-Path $profileDir $forbiddenRuntime)) {
            throw "ARM_CM33_TFM must not expose SecureContext/MPU artifact: $forbiddenRuntime"
        }
    }
    ...
    foreach ($selector in @(
            ...
            (Join-Path $RepositoryRoot "fiber\port\fiber_port_selected.h"))) {
        if ((Get-Content -LiteralPath $selector -Raw).IndexOf(
                "ARM_CM33_TFM", [System.StringComparison]::Ordinal) -ge 0) {
            throw "ARM_CM33_TFM must remain explicitly build-selected: $selector"
        }
    }
    ...
    $typeText = Get-Content -LiteralPath `
        (Join-Path $profileDir "fiber_port_types.h") -Raw
    foreach ($forbiddenTypeDependency in @(
            "mcu_core.h", "fiber_compiler.h", "fiber_portmacro.h",
            "fiber_port_select.h")) {
        $includePattern = '(?m)^\s*#\s*include\s*["<][^">]*' +
            [regex]::Escape($forbiddenTypeDependency) + '[">]'
        if ([regex]::IsMatch($typeText, $includePattern)) {
            throw "ARM_CM33_TFM public storage acquired CPU dependency: $forbiddenTypeDependency"
        }
    }
    ...
    foreach ($requiredParity in @(
            ...
            "TF-M v2.0.0", "tfm_ns_interface_init()",
            "OS_WRAPPER_WAIT_FOREVER", "finite nonzero timeouts",
            "fiber-owned SecureContext = absent",
            "compile/assembly/archive/ELF validated")) {
        if ($parity.IndexOf($requiredParity,
                [System.StringComparison]::Ordinal) -lt 0) {
            throw "ARM_CM33_TFM parity ledger lost evidence: $requiredParity"
        }
    }

    $probeDir = Join-Path $BuildRoot "arm-cm33-tfm"
    New-Item -ItemType Directory -Path $probeDir | Out-Null
    $mainHeader = @"
#ifndef MAIN_H_
#define MAIN_H_
...
_Static_assert(sizeof(FiberContext) == 76u, "context size");
_Static_assert(_Alignof(FiberContext) == 4u, "context alignment");
FiberContext fiber_cm33_tfm_type_only_object;
'@
    ...
    if ($LASTEXITCODE -ne 0) {
        throw "ARM_CM33_TFM type-only C header failed without CMSIS"
    }
...
static_assert(sizeof(FiberContext) == 76u, "context size");
static_assert(alignof(FiberContext) == 4u, "context alignment");
FiberContext fiber_cm33_tfm_cpp_type_only_object;
'@
    & $Compiler -x c++ -mcpu=cortex-m33 -mthumb -std=c++17 `
        -ffreestanding -fno-builtin -Wall -Wextra -Werror `
        "-I$profileDir" -c $cppProbe -o (Join-Path $probeDir "type-only-cpp.o")
    if ($LASTEXITCODE -ne 0) {
        throw "ARM_CM33_TFM type-only C++ headers failed without CMSIS"
    }
    ...
    $layoutObject = Join-Path $probeDir "layout.o"
    & $Compiler @manifestArgs -c $layoutFixture -o $layoutObject
    if ($LASTEXITCODE -ne 0) {
        throw "ARM_CM33_TFM exact layout manifest failed compile"
    }
    $layoutDefined = @(& $Nm -g --defined-only $layoutObject)
    if ($LASTEXITCODE -ne 0) {
        throw "nm failed for ARM_CM33_TFM layout probe"
    }
    ...
    if ($cohorts.Count -ne 1) {
        throw "ARM_CM33_TFM layout must define one exact cohort"
    }
    foreach ($token in @(
            "armv8m_mainline", "p0x43335446u", "l0x00010001u",
            "_h1_j1_v1_d1_r1_", "_z8u_y0_w2_g3_",
            "_i0_o0xFFFFFFBCu_c0_s1_x0_m0_a0_b0_t1_n1_k0_q0")) {
        if ($cohorts[0].IndexOf($token,
                [System.StringComparison]::Ordinal) -lt 0) {
            throw "ARM_CM33_TFM exact cohort lost token ${token}: $($cohorts[0])"
        }
    }

    $runtimeText = Get-Content -LiteralPath $runtimeSource -Raw
    $tfmText = Get-Content -LiteralPath $tfmSource -Raw
    $wrapperText = Get-Content -LiteralPath $wrapperSource -Raw
    $prepareBody = Get-CFunctionBody -Source $runtimeText `
        -Signature "void fiber_port_runtime_prepare_start(void)" `
        -Path $runtimeSource
    $prepareOrder = @(
        "FIBER_RUNTIME_PORT_ABI_RETAIN_V1();",
        "FIBER_PORT_CONTEXT_COHORT_RETAIN();",
        "fiber_port_arm_cm33_tfm_integration_bundle_v1_anchor",
        "fiber_port_require_start_environment();",
        "fiber_port_require_start_interrupt_state();",
        "fiber_port_tfm_require_initialized();",
        "fiber_port_require_start_environment();",
        "fiber_port_require_start_interrupt_state();",
        "fiber_port_runtime_prepare();",
        "fiber_port_prepare_exception_start();")
    $cursor = 0
    foreach ($ordered in $prepareOrder) {
        $index = $prepareBody.IndexOf($ordered, $cursor,
            [System.StringComparison]::Ordinal)
        if ($index -lt 0) {
            throw "ARM_CM33_TFM start preparation lost ordered operation: $ordered"
        }
        $cursor = $index + $ordered.Length
    }
    foreach ($requiredTfmToken in @(
            "extern uint32_t tfm_ns_interface_init(void);",
            "fiber_portTFM_UNINITIALIZED", "fiber_portTFM_INITIALIZING",
            "fiber_portTFM_READY", "fiber_portTFM_FAILED",
            "fiber_port_tfm_capture_cpu_state", "tfm_ns_interface_init();",
            "fiber_port_tfm_validate_cpu_state",
            "void fiber_port_tfm_require_initialized(void)")) {
        if ($tfmText.IndexOf($requiredTfmToken,
                [System.StringComparison]::Ordinal) -lt 0) {
            throw "ARM_CM33_TFM initialization lost contract: $requiredTfmToken"
        }
    }
    foreach ($requiredWrapperToken in @(
            ...
            "FIBER_PORT_TFM_OS_WRAPPER_WAIT_FOREVER",
            "fiber_port_runtime_schedule();",
            "fiber_port_tfm_mutex.owner == current",
            "fiber_port_tfm_mutex.owner != current")) {
        if ($wrapperText.IndexOf($requiredWrapperToken,
                [System.StringComparison]::Ordinal) -lt 0) {
            throw "ARM_CM33_TFM mutex wrapper lost contract: $requiredWrapperToken"
        }
    }
    foreach ($forbiddenToken in @(
            "FreeRTOS.h", "semphr.h", "xSemaphore", "SecureContext_",
            "fiber_secure_context", "fiber_port_secure_context")) {
        if (($runtimeText + $tfmText + $wrapperText).IndexOf($forbiddenToken,
                [System.StringComparison]::Ordinal) -ge 0) {
            throw "ARM_CM33_TFM imported forbidden implementation: $forbiddenToken"
        }
    }

    $compileArgs = @($manifestArgs + @(
        "-DFIBER_ALLOW_PERMISSIVE_ADDRESS_MAP_HOOKS=1",
        "-O2", "-Werror", "-ffunction-sections", "-fdata-sections"))
    $objects = @()
    foreach ($source in @($runtimeSource, $bootSource, $tfmSource,
            $wrapperSource)) {
        $object = Join-Path $probeDir `
        ...
        if ($LASTEXITCODE -ne 0) {
            throw "ARM_CM33_TFM source failed compile: $source"
        }
        $objects += $object
    }

    $groupObject = Join-Path $probeDir "runtime-group.o"
    & $Compiler -r @objects -o $groupObject
    if ($LASTEXITCODE -ne 0) {
        throw "ARM_CM33_TFM object group failed relocatable link"
    }
    $groupDefined = @(& $Nm -g --defined-only $groupObject)
    if ($LASTEXITCODE -ne 0) {
        throw "nm failed for ARM_CM33_TFM object group"
    }
    foreach ($requiredDefinition in @(
            ...
            "fiber_port_runtime_start_first",
            "fiber_port_runtime_schedule",
            "fiber_port_tfm_initialize",
            "fiber_port_tfm_require_initialized",
            "fiber_port_arm_cm33_tfm_integration_bundle_v1_anchor",
            "os_wrapper_mutex_create",
            "os_wrapper_mutex_acquire",
            "os_wrapper_mutex_release",
            "os_wrapper_mutex_delete",
            "SVC_Handler", "PendSV_Handler")) {
        if ((@($groupDefined | Where-Object {
                $_ -match ("\b[TRB]\s+" +
                    [regex]::Escape($requiredDefinition) + "$")
            })).Count -ne 1) {
            throw "ARM_CM33_TFM group must define one $requiredDefinition"
        }
    }
    $groupCohorts = @($groupDefined | ForEach-Object {
        if ($_ -match '\b(fiber_port_context_cohort_\S+)$') {
    ...
    if (($groupCohorts.Count -ne 1) -or ($groupCohorts[0] -ne $cohorts[0])) {
        throw "ARM_CM33_TFM object group disagrees with exact cohort"
    }
    foreach ($forbiddenDefined in @($groupDefined | Where-Object {
            $_ -match '\b(?:T|R|B)\s+(?:fiber_.*secure_context|fiber_secure_)'
        })) {
        throw "ARM_CM33_TFM unexpectedly defines SecureContext symbol: $forbiddenDefined"
    }
    $undefined = Get-NmUndefinedSymbolNames `
        -NmOutput @(& $Nm -u $groupObject) -Path $groupObject
    Assert-ExactSymbolSet -Expected @(
        "fiber_current",
        "fiber_internal_runtime_current_context_slot",
        "fiber_internal_runtime_port_abi_v1_anchor",
        ...
        "ARM_CM33_TFM exact reverse/TF-M dependency surface"

    $fixturePath = Join-Path $probeDir "runtime-fixture.c"
    $competingPath = Join-Path $probeDir "competing-handlers.c"
    $linkerPath = Join-Path $probeDir "runtime.ld"
...
FIBER_API_ATTR_SENSITIVE FIBER_GENERAL_REGS_ONLY
FiberContext *fiber_internal_runtime_select_scheduler_candidate(FiberContext *current)
{
    return current;
}
FIBER_API_ATTR_SENSITIVE FIBER_GENERAL_REGS_ONLY
void fiber_internal_runtime_require_current_context(void)
{
    if (fiber_internal_runtime_current_context_slot == NULL) { fiber_panic('C'); }
}
FIBER_API_ATTR_SENSITIVE FIBER_GENERAL_REGS_ONLY
void fiber_internal_runtime_publish_current_context(FiberContext *next)
{
    if (next == NULL) { fiber_panic('N'); }
    fiber_internal_runtime_current_context_slot = next;
}
...
FIBER_API_NORETURN FIBER_API_ATTR_SENSITIVE FIBER_GENERAL_REGS_ONLY
void fiber_internal_task_return(void) { for (;;) { __asm volatile("wfe"); } }
FIBER_API_NORETURN FIBER_API_ATTR_SENSITIVE FIBER_GENERAL_REGS_ONLY
void fiber_panic(char code) { (void)code; for (;;) { __asm volatile("wfe"); } }
...
{
    unsigned char *o = (unsigned char *)d;
    while (n-- != 0u) { *o++ = (unsigned char)v; }
    return d;
}
#ifndef FIBER_TEST_OMIT_TFM_INIT
uint32_t tfm_ns_interface_init(void)
{
    return os_wrapper_mutex_create() != NULL ? 0u : UINT32_MAX;
}
#endif
void Reset_Handler(void)
{
...
__attribute__((used, section(".isr_vector")))
const FiberVector fiber_vectors[16] = {
    [1] = Reset_Handler, [11] = SVC_Handler, [14] = PendSV_Handler
};
'@
    Set-Content -LiteralPath $competingPath -Encoding ASCII -Value @'
...
'@
    ...
    foreach ($archiveMode in @(
        ...
        New-Item -ItemType Directory -Path $modeDir | Out-Null
        $archiveObjects = @()
        $archiveArgs = @($compileArgs + $archiveMode.Flags)
        foreach ($source in @($runtimeSource, $bootSource, $tfmSource,
                $wrapperSource)) {
            $object = Join-Path $modeDir `
                (([IO.Path]::GetFileNameWithoutExtension($source)) +
                 "-" + $archiveObjects.Count + ".o")
            & $Compiler @archiveArgs -c $source -o $object
            if ($LASTEXITCODE -ne 0) {
                throw "ARM_CM33_TFM archive source failed compile ($($archiveMode.Name)): $source"
            }
            $archiveObjects += $object
        }
        $archivePath = Join-Path $modeDir "libfiber_cm33_tfm.a"
        & $Ar rcs $archivePath @archiveObjects
        if ($LASTEXITCODE -ne 0) {
            throw "ARM_CM33_TFM archive creation failed ($($archiveMode.Name))"
        }
        $fixtureObject = Join-Path $modeDir "fixture.o"
        $missingObject = Join-Path $modeDir "missing-tfm.o"
        $competingObject = Join-Path $modeDir "competing.o"
        & $Compiler @archiveArgs -c $fixturePath -o $fixtureObject
        if ($LASTEXITCODE -ne 0) {
            throw "ARM_CM33_TFM ELF fixture failed compile ($($archiveMode.Name))"
        }
        & $Compiler @($archiveArgs + @("-DFIBER_TEST_OMIT_TFM_INIT=1")) `
            -c $fixturePath -o $missingObject
        if ($LASTEXITCODE -ne 0) {
            throw "ARM_CM33_TFM missing-init fixture failed compile ($($archiveMode.Name))"
        }
        & $Compiler @archiveArgs -c $competingPath -o $competingObject
        if ($LASTEXITCODE -ne 0) {
            throw "ARM_CM33_TFM competing-handler fixture failed compile ($($archiveMode.Name))"
        }

        $elfPath = Join-Path $modeDir "runtime.elf"
        $linkArgs = @("-mcpu=cortex-m33", "-mthumb", "-nostdlib") +
            $archiveMode.Flags + @("-Wl,--gc-sections", "-T", $linkerPath,
                $fixtureObject, $archivePath, "-o", $elfPath)
        & $Compiler @linkArgs
        if ($LASTEXITCODE -ne 0) {
            throw "ARM_CM33_TFM archive/ELF proof failed link ($($archiveMode.Name))"
        }
        $objectNm = if ($archiveMode.Name -eq "lto") { $GccNm } else { $Nm }
        $elfDefined = @(& $objectNm -g --defined-only $elfPath)
        if ($LASTEXITCODE -ne 0) {
            throw "nm failed for ARM_CM33_TFM ELF ($($archiveMode.Name))"
        }
        foreach ($requiredElfSymbol in @(
                "SVC_Handler", "PendSV_Handler", "fiber_port_tfm_initialize",
                "fiber_port_arm_cm33_tfm_integration_bundle_v1_anchor",
                "os_wrapper_mutex_create", "os_wrapper_mutex_acquire",
                "os_wrapper_mutex_release", "os_wrapper_mutex_delete")) {
            if ((@($elfDefined | Where-Object {
                    $_ -match ("\b[TR]\s+" +
                        [regex]::Escape($requiredElfSymbol) + "$")
                })).Count -ne 1) {
                throw "ARM_CM33_TFM ELF lost $requiredElfSymbol ($($archiveMode.Name))"
            }
        }
        if (@($elfDefined | Where-Object {
                $_ -match '\b(?:T|R|B)\s+(?:fiber_.*secure_context|fiber_secure_)'
            }).Count -ne 0) {
            throw "ARM_CM33_TFM ELF contains forbidden SecureContext symbol ($($archiveMode.Name))"
        }

        $svcLine = @($elfDefined | Where-Object {
            $_ -match '^([0-9a-fA-F]+)\s+T\s+SVC_Handler$'
        })
        $pendSvLine = @($elfDefined | Where-Object {
            $_ -match '^([0-9a-fA-F]+)\s+T\s+PendSV_Handler$'
        })
        if (($svcLine.Count -ne 1) -or ($pendSvLine.Count -ne 1)) {
            throw "ARM_CM33_TFM ELF handlers are not unique and strong ($($archiveMode.Name))"
        }
        [void]($svcLine[0] -match '^([0-9a-fA-F]+)')
        $svcAddress = [Convert]::ToUInt32($Matches[1], 16)
        [void]($pendSvLine[0] -match '^([0-9a-fA-F]+)')
        $pendSvAddress = [Convert]::ToUInt32($Matches[1], 16)
        $vectorBinary = Join-Path $modeDir "vectors.bin"
        & $Objcopy -O binary --only-section=.isr_vector $elfPath $vectorBinary
        if ($LASTEXITCODE -ne 0) {
            throw "ARM_CM33_TFM vector extraction failed ($($archiveMode.Name))"
        }
        $vectorBytes = [IO.File]::ReadAllBytes($vectorBinary)
        if ($vectorBytes.Length -lt (16 * 4)) {
            throw "ARM_CM33_TFM vector table is truncated ($($archiveMode.Name))"
        }
        if ([BitConverter]::ToUInt32($vectorBytes, 11 * 4) -ne
                ($svcAddress -bor [uint32]1)) {
            throw "ARM_CM33_TFM vector slot 11 lost SVC_Handler ($($archiveMode.Name))"
        }
        if ([BitConverter]::ToUInt32($vectorBytes, 14 * 4) -ne
                ($pendSvAddress -bor [uint32]1)) {
            throw "ARM_CM33_TFM vector slot 14 lost PendSV_Handler ($($archiveMode.Name))"
        }

        $missing = Invoke-CompilerProbe -Compiler $Compiler -Arguments `
            (@("-mcpu=cortex-m33", "-mthumb", "-nostdlib") +
             $archiveMode.Flags + @("-Wl,--gc-sections", "-T", $linkerPath,
                $missingObject, $archivePath, "-o",
                (Join-Path $modeDir "missing-tfm.elf"))) `
            -LogPath (Join-Path $modeDir "missing-tfm.log")
        if (($missing.ExitCode -eq 0) -or
                ($missing.Output -notmatch 'tfm_ns_interface_init')) {
            throw "ARM_CM33_TFM without TF-M interface must fail on tfm_ns_interface_init ($($archiveMode.Name)).`n$($missing.Output)"
        }

        $duplicate = Invoke-CompilerProbe -Compiler $Compiler -Arguments `
            (@("-mcpu=cortex-m33", "-mthumb", "-nostdlib") +
             $archiveMode.Flags + @("-Wl,--gc-sections", "-T", $linkerPath,
                $fixtureObject, $competingObject, $archivePath, "-o",
                (Join-Path $modeDir "duplicate.elf"))) `
            -LogPath (Join-Path $modeDir "duplicate.log")
        if (($duplicate.ExitCode -eq 0) -or
                ($duplicate.Output -notmatch
                    'multiple definition.*(?:SVC_Handler|PendSV_Handler)')) {
            throw "ARM_CM33_TFM competing strong handlers must fail link ($($archiveMode.Name)).`n$($duplicate.Output)"
        }
    }

    $secureCaseDir = Join-Path $probeDir "invalid-secure"
    New-Item -ItemType Directory -Path $secureCaseDir | Out-Null
    Set-Content -LiteralPath (Join-Path $secureCaseDir "main.h") `
        -Value $mainHeader -Encoding ASCII
    $secureResult = Invoke-CompilerProbe -Compiler $Compiler -Arguments @(
        "-mcpu=cortex-m33", "-mthumb", "-mcmse", "-std=c11",
        "-DFIBER_PORT_BUILD_SELECTED=1", "-DFIBER_PORT_ARMV8M_MAINLINE=1",
        "-I$secureCaseDir", "-I$profileDir",
        "-I$(Join-Path $RepositoryRoot 'fiber\port')",
        "-I$(Join-Path $RepositoryRoot 'fiber')", "-I$RepositoryRoot",
        "-I$CmsisPath", "-c", $layoutFixture, "-o",
        (Join-Path $secureCaseDir "invalid.o")) `
        -LogPath (Join-Path $secureCaseDir "compile.log")
    if (($secureResult.ExitCode -eq 0) -or
            ($secureResult.Output.IndexOf("does not accept a Secure CMSE build",
                [System.StringComparison]::Ordinal) -lt 0)) {
        throw "ARM_CM33_TFM Secure manifest did not fail closed.`n$($secureResult.Output)"
    }
}
...
. (Join-Path $PSScriptRoot "arm_cm55f_mpu_svc_checks.ps1")
. (Join-Path $PSScriptRoot "arm_cm55f_mpu_pendsv_checks.ps1")
. (Join-Path $PSScriptRoot "arm_cm55_mvef_mpu_slice1_checks.ps1")
. (Join-Path $PSScriptRoot "arm_cm55_mvef_mpu_svc_checks.ps1")
. (Join-Path $PSScriptRoot "arm_cm55_mvef_mpu_pendsv_checks.ps1")
...
# Generic Cortex-M55 selector coverage must compile only the selected v8-M
# fallback group. Concrete ARM_CM7 sources include CMSIS before the selector;
# that injects __CORTEX_M == 55 and deliberately upgrades detection to v8.1-M.
# It is not the source group selected by a plain -mcpu=cortex-m55 build, whose
# compiler-only predefines may still report ARMv8-M Mainline.
$selectorPortSourcesByConfig = @{
    "cortex-m55" = @(
        "fiber\port\transitional_v8m\fiber_port_transitional_v8m.c",
        "fiber\port\transitional_v8m\fiber_port_boot.c",
        "fiber\port\transitional_v8m\fiber_port_exception.c"
...
$portProfiles = @{
    ...
    # GCC may expose plain -mcpu=cortex-m55 as ARMv8-M Mainline before a
    # selected port includes CMSIS. The generic selector test intentionally
    # covers that compiler-only result; concrete M55 ports use build-selected
    # ARMv8.1-M contracts instead.
...
try {
    ...
    Test-ArmCm23NtzRuntimeIntegration -RepositoryRoot $RepoRoot `
        ...
        -BuildRoot $buildRoot
    Write-Host "== ARM_CM33_MPU layout/trait contract =="
    ...
    Test-ArmCm33MpuConstructionContract -RepositoryRoot $RepoRoot `
        ...
        -BuildRoot $buildRoot
    Write-Host "== ARM_CM33_MPU protected SVC/PendSV runtime contract =="
    Test-ArmCm33MpuProtectedRuntimeContract -RepositoryRoot $RepoRoot `
        -Compiler $gcc -Nm $nm -GccNm $gccNm -Objdump $objdump `
        -Objcopy $objcopy -CmsisPath $cmsis -BuildRoot $buildRoot
    Write-Host "== ARM_CM33_MPU full runtime/archive/MPU-linker contract =="
    ...
    Test-ArmCm55fMpuConstructionContract -RepositoryRoot $RepoRoot `
		-Compiler $gcc -Nm $nm -GccNm $gccNm -Objdump $objdump `
		-CmsisPath $cmsis -BuildRoot $buildRoot
	Write-Host "== ARM_CM55F_MPU scalar-FP protected SVC contract =="
	Test-ArmCm55fMpuSvcContract -RepositoryRoot $RepoRoot `
		-Compiler $gcc -Nm $nm -GccNm $gccNm -Objdump $objdump `
		-Objcopy $objcopy -CmsisPath $cmsis -BuildRoot $buildRoot
	Write-Host "== ARM_CM55F_MPU scalar-FP protected PendSV contract =="
	Test-ArmCm55fMpuPendSvContract -RepositoryRoot $RepoRoot `
		-Compiler $gcc -Nm $nm -GccNm $gccNm -Objdump $objdump `
		-Objcopy $objcopy -CmsisPath $cmsis -BuildRoot $buildRoot
    Write-Host "== ARM_CM55F_MPU scalar-FP full runtime/archive/ELF contract =="
    Test-ArmCm55fMpuRuntimeIntegration -RepositoryRoot $RepoRoot -Compiler $gcc -Nm $nm -GccNm $gccNm -Objdump $objdump -Objcopy $objcopy -Ar $ar -CmsisPath $cmsis -BuildRoot $buildRoot

    Write-Host "== ARM_CM55_MVEF_MPU MVE-FP construction/linker contract =="
    Test-ArmCm55MvefMpuSlice1Contract -RepositoryRoot $RepoRoot `
        -Compiler $gcc -Nm $nm -GccNm $gccNm -Objdump $objdump `
        -CmsisPath $cmsis -BuildRoot $buildRoot

    Write-Host "== ARM_CM55_MVEF_MPU MVE-FP protected SVC contract =="
    Test-ArmCm55MvefMpuSvcContract -RepositoryRoot $RepoRoot `
        -Compiler $gcc -Nm $nm -GccNm $gccNm -Objdump $objdump `
        -Objcopy $objcopy -CmsisPath $cmsis -BuildRoot $buildRoot

    Write-Host "== ARM_CM55_MVEF_MPU MVE-FP protected PendSV contract =="
    Test-ArmCm55MvefMpuPendSvContract -RepositoryRoot $RepoRoot `
        -Compiler $gcc -Nm $nm -GccNm $gccNm -Objdump $objdump `
        -Objcopy $objcopy -CmsisPath $cmsis -BuildRoot $buildRoot

    Write-Host "== ARM_CM55_MVEF_MPU MVE-FP full runtime/archive/ELF contract =="
    Test-ArmCm55MvefMpuRuntimeIntegration -RepositoryRoot $RepoRoot `
        -Compiler $gcc -Nm $nm -GccNm $gccNm -Objdump $objdump `
    ...
    Test-ArmCm55NtzRuntimeContract -RepositoryRoot $RepoRoot `
        -Compiler $gcc -Nm $nm -GccNm $gccNm -Objdump $objdump -Ar $ar `
        -Objcopy $objcopy -CmsisPath $cmsis -BuildRoot $buildRoot
    Write-Host "== ARM_CM55F_NTZ full scalar-FP runtime/archive/ELF contract =="
    ...
    if (-not $SettingsOnly) {
        ...
        $selectorSources = $selectorPortSources
        if ($selectorPortSourcesByConfig.ContainsKey($cfg.Name)) {
            $selectorSources = $selectorPortSourcesByConfig[$cfg.Name]
        }

        $selectionModes = @(
            [pscustomobject]@{ Name = "auto";           Defines = @(); ExtraArgs = @(); PortSources = $selectorSources },
            [pscustomobject]@{ Name = "explicit";       Defines = @("-DFIBER_PORT_PROFILE=$profile"); ExtraArgs = @(); PortSources = $selectorSources },
