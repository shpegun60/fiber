...
| ARMv8.1-M / M55 TrustZone SecureContext (staged) | Build-selected `ARM_CM55/non_secure` Slice 1 freezes the separate scalar `C55S` Non-secure dictionary: `[SecureContext handle][PSPLIM][EXC_RETURN][r4-r11]`, a pre-start `secure_stack_bytes` request, `__ARM_FEATURE_CMSE == 1`, and no FPU/MVE/MPU/PAC/BTI. Slice 2 adds only a versioned C55S-named Secure identity gateway with four immutable NSC veneers and real CMSE import/mismatch proof. It intentionally defines no runtime cohort symbol, forward ABI operation, handler, attachment API, stateful Secure gateway, or hardware claim. | Add lifecycle attachment, first-start Secure allocation/load, PendSV save/load, and their generated-assembly/archive/vector/hardware proofs in isolated slices. |
