...
| ARMv8.1-M / M55 MVE-FP NTZ | Non-selectable `ARM_CM55_MVEF_NTZ/non_secure` slice 1 freezes a distinct `C55V` non-MPU MVE-FP cohort with the same 72-byte basic and future 212-byte extended context geometry as the pinned `configENABLE_FPU=1`, `configENABLE_MVE=1` FreeRTOS branch. It requires `-march=armv8.1-m.main+mve.fp`, records MVE in the exact cohort, and retains the reference no-VPR-software-slot frame. `fiber_port_boot.c` provides only sealed construction and CPACR/FPCCR policy; paired hard/softfp `-O2`/`-Os` constructor proof passes. No forward runtime ABI, SVC/PendSV handler, vector/archive/ELF runtime, or hardware claim exists yet. | Add strict SVC, MVE-FP PendSV with the reference conditional `s16-s31` backbone, then archive/vector/cohort and hardware evidence. MPU, TrustZone/SecureContext or TF-M, and PAC/BTI remain distinct profiles. |
| Source layout | ARMv6-M, ARMv7-M, Cortex-M4 ARMv7E-M, concrete CM7, M23 NTZ, M33 NTZ, M33F NTZ, M33 SecureContext, M33 TF-M, scalar M55 NTZ, and scalar-FP M55 NTZ have separate runtime source groups. M55 MVE-FP has a separate construction-only source group. Global v8-M auto/profile routing and unported M55/security roles still use `transitional_v8m`. | Replace remaining transitional v8-M roles with one concrete runtime source group per runtime-image security/profile ABI and bind its identity-matched Secure artifact or TF-M component where required. |
