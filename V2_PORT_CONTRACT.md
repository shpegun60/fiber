...
| ARMv8.1-M / M55 MPU | Non-selectable `ARM_CM55_MPU/non_secure` slice 4 freezes the no-FPU/no-MVE/no-TrustZone/no-PAC `C55M` 8/16-region protected context. `fiber_port_boot.c` owns sealed construction, exact RBAR/RLAR encoding, linker-boundary validation, and a four-region image; staged `fiber_port.c` owns strong SVC/PendSV handlers, FreeRTOS-shaped first MPU activation/restore, private SVC 71 yield, and protected PendSV save/select/MAIR0-plus-context-MPU replacement/restore. PendSV preflights the live image before cursor access, calls reverse-ABI policy under BASEPRI, preserves scheduler CPU/MPU state, and holds PRIMASK across the disabled-MPU interval. Paired `-O2`/`-Os` assembly plus normal/LTO ELF proofs retain protected source placement, slots 11/14, and duplicate-SVC/PendSV failure. The forward runtime ABI deliberately remains absent. M55-only RLAR.PXN remains available for a future configurable-region ABI. | Activate the eight forward operations and add archive/cohort proof, then validate MPU isolation. FPU, MVE, TrustZone/SecureContext, TF-M, PAC, and BTI remain separate cohorts. |
| Source layout | ARMv6-M, ARMv7-M, Cortex-M4 ARMv7E-M, concrete CM7, M23 NTZ, M33 NTZ, M33F NTZ, M33 SecureContext, M33 TF-M, scalar M55 NTZ, scalar-FP M55 NTZ, and M55 MVE-FP have separate source groups. Global v8-M auto/profile routing and unported M55/security roles still use `transitional_v8m`. | Replace remaining transitional v8-M roles with one concrete runtime source group per runtime-image security/profile ABI and bind its identity-matched Secure artifact or TF-M component where required. |
