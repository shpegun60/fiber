...
| ARMv8.1-M / M55F MPU | Explicit build-selected `ARM_CM55F_MPU/non_secure` slice 1 freezes a separate `C55P` scalar-FP/no-MVE/no-TrustZone/no-PAC 8/16-region construction cohort from the pinned FreeRTOS MPU+FPU branch. `FiberPortProtectedContext` is the exact shared 54-word privileged backing store: basic first-start data uses words 0-20, while an extended FP save later uses words 0-53. The slice owns sealed construction, exact MAIR0/RBAR/RLAR storage, FPU CPACR/FPCCR policy, protected linker boundaries, and exact cohort identity; hard/softfp `-O2`/`-Os` constructor parity and construction/linker/cohort proofs pass. It deliberately exports no generic forward runtime ABI, SVC/PendSV handlers, optional MPU API, or global selector route. | Preserve the frozen overlap while adding protected scalar-FP SVC, FP-aware PendSV, eight-operation/archive/vector proof, then validate M55F MPU execution and isolation. MVE, TrustZone/SecureContext, TF-M, PAC, and BTI remain distinct profiles. |
