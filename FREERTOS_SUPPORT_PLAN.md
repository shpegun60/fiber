...
| Cortex-M55 TrustZone/SecureContext (staged) | Build-selected `ARM_CM55/non_secure` Slice 1 freezes the distinct `C55S` scalar Non-secure layout: the eleven-word `[SecureContext handle][PSPLIM][EXC_RETURN][r4-r11]` frame, `secure_stack_bytes` pre-start request, ARMv8.1-M/Cortex-M55/CMSE-level-1 manifest, and no-FPU/no-MVE/no-MPU/no-PAC/no-BTI cohort. Slice 2 adds exactly four cohort-named, versioned identity NSC veneers in `ARM_CM55/secure`; real `-mcmse` import/foreign/v2/missing-link proof passes. It still has no context constructor, forward ABI, handlers, attachment API, allocator, stateful Secure gateway, archive/vector runtime proof, generated SVC/PendSV proof, or hardware claim. | Add stateful Secure lifecycle only after the identity-matched gateway: attachment, one-shot initialization, Secure allocation/save/load, SVC/PendSV, and the normal parity/archive/vector/hardware proofs remain separate slices. |
