...
| Cortex-M33 TrustZone/SecureContext | Build-selected `ARM_CM33/non_secure` slices 1-2 freeze the distinct no-MPU/no-FPU companion-aware frame from `GCC/ARM_CM33/non_secure`: `[xSecureContext][PSPLIM][EXC_RETURN][r4-r11]`, a sealed pre-start `secure_stack_bytes` request, and a distinct exact cohort. A paired `ARM_CM33/secure` gateway exports four versioned immutable NSC identity queries. Secure slice 3 adds a manifest-budgeted Secure-private fixed pool with the FreeRTOS record prefix, zero-invalid index-plus-one handles, and exact two-word `0xFEF5EDA5` stack seals, deliberately replacing `secure_heap`/free with static lifetime. The matrix builds a real GNU CMSE Secure image/import library and proves matching, missing-import, v1/v2 mismatch, exact NSC surface, Secure-RAM pool placement, and invalid manifests at `-O2`, `-Os`, and `-O2 -flto`. It exposes no selected runtime, attach API, NSC allocation, save/load, handler, or hardware claim. | Add the selected-port attachment/first-start allocation-load slice, then FreeRTOS-shaped SVC/PendSV SecureContext mechanics and hardware validation. |
