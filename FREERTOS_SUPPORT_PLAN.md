...
| Cortex-M0+ MPU | Exact build-selected `ARM_CM0_MPU` slices 1-4 implement protected construction/linker isolation, strong first-start and task-return SVC services, one-time MPU activation/readback, and the FreeRTOS-derived Thumb-1 first restore. It remains non-selectable and intentionally has no PendSV, yield, or forward runtime ABI. `-O2`/`-Os`, LTO, direct SVC-vector, VTOR-present/absent, and negative linker proofs pass. | Add the protected PendSV save/select/MPU-replace/restore slice, then the forward ABI/archive proof and Cortex-M0+ MPU hardware isolation suite. |
