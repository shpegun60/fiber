...
| Cortex-M55 MVE-FP NTZ | `ARM_CM55_MVEF_NTZ/non_secure` slice 1 freezes a distinct non-MPU `C55V` context cohort under `-march=armv8.1-m.main+mve.fp`: MVE-FP compiler facts, the FreeRTOS 72-byte basic and future 212-byte extended geometry, no VPR software slot, sealed construction, CPACR/FPCCR policy, and paired hard/softfp `-O2`/`-Os` constructor assembly proof. It intentionally exports no SVC/PendSV handler and no forward runtime ABI surface. | Add strict SVC first start, MVE-FP PendSV, archive/vector/cohort proof, then validate matching M55 hardware. Keep MPU, TrustZone/SecureContext or TF-M, and PAC/BTI as separate cohorts. |
| Cortex-M55 feature cohorts / STM32N6 | Scalar and scalar-FP runtimes are complete software cohorts; the separate MVE-FP cohort is construction-only. `transitional_v8m` remains only for unported v8-M roles. | Add MVE-FP runtime, MPU, TrustZone/SecureContext or TF-M, and PAC/BTI cohorts, then validate matching hardware. |
