...
portable scheduler/current checks and before port preparation. The staged
`ARM_CM33/non_secure` attachment module now retains it and exports only its
profile-specific pre-start API; the common runtime remains SecureContext-blind.
...
It records fourteen runtime-capable concrete profiles, including the explicitly
build-selected `ARM_CM0_MPU`, `ARM_CM33_MPU/non_secure`, and scalar
`ARM_CM55_NTZ/non_secure` runtimes with their archive/linker proofs, then the
remaining CM33 security/FP-MPU, M55/N6 feature cohorts, and announced CM85/V8
work, planning ranges, and the exact gate before Context extraction.
...
| Cortex-M33 TrustZone/SecureContext | Build-selected `ARM_CM33/non_secure` slices 1-6 complete the distinct no-MPU/no-FPU companion-aware runtime from `GCC/ARM_CM33/non_secure`: exact 19-word construction, sealed pre-start attachment, all eight forward operations, strong SVC/PendSV, one-shot Secure initialization, first-context allocation/load, and switch-time Secure save/unload, selection, lazy allocation, and owned load. The paired Secure artifact exports four immutable identity veneers plus an eight-function stateful gateway backed by the static Secure pool. It preserves FreeRTOS PRIS, PSP/PSPLIM/CONTROL, allocation/save/load, and eleven-word frame ordering while narrowing initialization to exact SVC, save to exact PendSV, and allocation/load to those two runtime exceptions. The matrix proves `-O2`/`-Os` generated construction/init/allocation/save/load and segmented SVC/PendSV parity, real GNU CMSE compatibility, exact twelve-function veneer surface, strong vector slots 11/14, assembly-load-only current slot, optional lifecycle-anchor retention, missing-import/lifecycle, v1/v2, duplicate-handler failures, pool placement, ordinary one-pass normal/LTO archive extraction through independent handler/attachment anchors, bounded lazy handles, and invalid manifests. | Run TrustZone hardware validation; keep FPU, MPU, TF-M, PAC/BTI, and MVE as distinct profiles. |
| Cortex-M33 TF-M | Exact build-selected `ARM_CM33_TFM/non_secure` uses distinct `C3TF` identity with the unchanged NTZ ten-word frame, all eight forward operations, strong SVC/PendSV, and automatic one-shot TF-M initialization before CPU start. It pins FreeRTOS `ThirdParty/GCC/ARM_TFM` and TF-M v2.0.0 semantics, exports the exact four-function OS mutex wrapper with one static owner-checked cooperative mutex, and deliberately provides no fiber SecureContext. `-O2`/`-Os` independent CM33 NTZ assembly parity, exact external symbol surface, normal/LTO archive extraction, slots 11/14, missing-interface and duplicate-handler failures, cohort, and SecureContext-exclusion proofs pass. | Integrate and run a matching TF-M v2.0 Secure image, generated NS interface/veneers, concurrent PSA calls, and hardware security/vector/PSPLIM tests. Keep FPU, MPU, MVE, PAC/BTI, and other TF-M CPU profiles separate. |
...
| Cortex-M33 MPU | Exact build-selected `ARM_CM33_MPU/non_secure` slice 5 completes the pinned `GCC/ARM_CM33_NTZ/non_secure` no-FPU/no-TrustZone 8- or 16-region cohort. It provides sealed unprivileged construction, all eight forward ABI operations, strong SVC/PendSV handlers, public SVC-71 `fiber_schedule()`, protected first start, protected PendSV frame copy/select/restore, and PRIMASK-protected MAIR0/context-pair replacement. RNR5 is a fixed 32-byte current-slot RO/XN overlay so unprivileged `fiber_current()` remains direct; its update occurs only with MPU disabled. `-O2`/`-Os` FreeRTOS assembly parity, normal/LTO archive extraction, exact cohort expectation, stale 8/16 archive rejection, MPU linker placement, slots 11/14, and duplicate-handler negative links pass. It remains outside global auto/profile selection and exports no heterogeneous MPU API or hardware claim. | Run Cortex-M33 MPU runtime and isolation validation. Keep FPU, SecureContext, TF-M, PAC/BTI, and MVE as separate cohorts. |
| Cortex-M55 scalar NTZ | Exact build-selected `ARM_CM55_NTZ/non_secure` freezes the pinned FreeRTOS ARMv8.1-M scalar Non-secure ten-word `[PSPLIM][EXC_RETURN][r4-r11]` frame under the distinct `C55N` cohort. It requires CMSIS M55 plus soft-float, rejects MVE/FP/MPU/CMSE/PAC/BTI, and owns sealed construction, strict SVC first start, PSPLIM-aware PendSV, all eight forward operations, strong handlers, and normal/LTO archive/vector/cohort proof. Paired `-O2`/`-Os` FreeRTOS assembly parity passes. | Run a scalar M55 board suite before any hardware claim. Keep FPU, MVE, MPU, TrustZone/SecureContext or TF-M, and PAC/BTI as separate M55 profiles. |
| Cortex-M55 TrustZone/SecureContext (staged) | Build-selected `ARM_CM55/non_secure` Slice 1 freezes the distinct `C55S` scalar Non-secure layout: the eleven-word `[SecureContext handle][PSPLIM][EXC_RETURN][r4-r11]` frame, `secure_stack_bytes` pre-start request, ARMv8.1-M/Cortex-M55/CMSE-level-1 manifest, and no-FPU/no-MVE/no-MPU/no-PAC/no-BTI cohort. Slice 2 adds four cohort-named identity NSC veneers; Slice 3 adds four immutable capacity veneers plus a manifest-budgeted, retained Secure-only static pool. Real `-mcmse` matching, foreign/v2/missing-link, capacity-manifest, and pool-placement proof passes. It still has no context constructor, forward ABI, handlers, attachment API, initialization, allocation/save/load, archive/vector runtime proof, generated SVC/PendSV proof, or hardware claim. | Add stateful Secure lifecycle only after the identity-matched capacity foundation: one-shot initialization, sealed attachment, Secure allocation/save/load, SVC/PendSV, and the normal parity/archive/vector/hardware proofs remain separate slices. |
| Cortex-M55F scalar NTZ | Exact build-selected `ARM_CM55F_NTZ/non_secure` slices 1-4 complete the `C55F` scalar-FP/no-MVE/non-MPU cohort. They freeze the pinned FreeRTOS 72-byte basic and 212-byte extended PSPLIM frame, FPU/extended-FP/FPCA traits, and `-march=armv8.1-m.main+fp` contract. `fiber_port_boot.c` provides sealed construction plus CPACR/FPCCR eager/lazy readback; `fiber_port_svc.c` provides strict FreeRTOS-shaped first SVC start; and `fiber_port.c` provides `fiber_port_runtime_schedule()` plus strong basic/extended FP PendSV with the FreeRTOS `s16-s31` ordering. Hard/softfp `-O2`/`-Os` construction/SVC/PendSV parity, all eight forward operations, exact reverse surface, normal/LTO archive/vector, external cohort, and duplicate-handler proofs pass. Global selector routing and hardware support remain absent. | Run first-start, basic/extended FP switching, vector/priority, and long-run FPU stress on matching M55F Non-secure hardware. Keep MVE, MPU, TrustZone/SecureContext or TF-M, and PAC/BTI as separate M55 profiles. |
| Cortex-M55 MVE-FP NTZ | Non-selectable `ARM_CM55_MVEF_NTZ/non_secure` slices 1-3 complete the distinct non-MPU `C55V` cohort under `-march=armv8.1-m.main+mve.fp`: MVE-FP compiler facts, 72-byte basic and 212-byte bounded extended geometry, no VPR software slot, sealed construction, CPACR/FPCCR policy, strict integer-only first SVC start, PSPLIM-aware conditional `s16-s31` PendSV, all eight forward operations, and strong slots 11/14 handlers. Hard/softfp `-O2`/`-Os` construction/SVC/PendSV parity, exact reverse surface, normal/LTO archive/vector/cohort, duplicate-handler, and no-VPR proofs pass. Global selector routing and hardware support remain absent. | Run basic/extended MVE-FP switching, vector/priority, and long-run stress on matching M55 hardware. Keep MPU, TrustZone/SecureContext or TF-M, and PAC/BTI as separate cohorts. |
| Cortex-M55F MPU | Explicit build-selected `ARM_CM55F_MPU/non_secure` slice 4 completes the scalar-FP/no-MVE/no-TrustZone/no-PAC `C55P` 8/16-region protected cohort from the `configENABLE_MPU=1`, `configENABLE_FPU=1` branch of `GCC/ARM_CM55_NTZ/non_secure`. Its exact overlapping 54-word backing store uses words 0-20 for basic first start and words 0-53 for an extended FP save. Boot owns construction; the mandatory SVC object owns the seven remaining forward operations, public SVC-71 yield, strong SVC, the private handler-bundle coordinator, and runtime cohort definition. The separate protected PendSV component retains reverse/cohort identities, is forced from the archive through that component anchor, validates before cursor access, preserves the FreeRTOS `s16-s31` plus copied `s0-s15/FPSCR` order, selects under BASEPRI, and replaces MAIR0/context regions with PRIMASK asserted. Separate hard/softfp `-O2`/`-Os` construction/SVC/PendSV parity plus representative archive extraction under hard-float `-O2`, softfp `-Os`, and hard-float `-O2` LTO, external cohort, stale 8/16 archive, protected placement, vector, and duplicate-handler proofs pass. It remains outside global auto/profile selection and exposes no heterogeneous MPU API or hardware claim. | Validate M55F MPU runtime and isolation. Keep MVE, TrustZone/SecureContext, TF-M, PAC, and BTI separate. |
| Cortex-M55 MVE-FP MPU | Explicit build-selected `ARM_CM55_MVEF_MPU/non_secure` slice 4 completes the separate `C55W` 8/16-region protected MVE-FP/no-TrustZone/no-PAC cohort from `configENABLE_MPU=1`, `configENABLE_FPU=1`, `configENABLE_MVE=1` in `GCC/ARM_CM55_NTZ/non_secure`. It requires `-march=armv8.1-m.main+mve.fp`, records MVE in its exact cohort, and retains the same overlapping 54-word protected backing image as C55P without a VPR software slot. Boot owns sealed construction; the mandatory SVC/runtime object defines the cohort, owns the other seven forward operations, public SVC-71 yield, SVC #70 first start, private SVC #72 task return, strong slot 11, and a private PendSV bundle anchor. The separate PendSV component owns strong slot 14, preflights before cursor access, preserves the exact `s16-s31` plus copied `s0-s15/FPSCR` move order, selects through reverse ABI v1 under BASEPRI, and replaces MAIR0/context regions with PRIMASK asserted. Hard/softfp `-O2`/`-Os` construction/SVC/PendSV parity, exact external cohort, stale 8/16 archive rejection, normal/LTO archive extraction, placement, slots 11/14, invalid-manifest, and duplicate-handler proofs pass. It deliberately has no global selector route, optional MPU API, or hardware claim. | Keep TrustZone/SecureContext, TF-M, PAC, and BTI separate; validate matching M55 hardware and MPU isolation. |
| Cortex-M55 MPU | Explicit build-selected `ARM_CM55_MPU/non_secure` slice 5 completes the no-FPU/no-MVE/no-TrustZone/no-PAC `C55M` protected 8/16-region layout from the `configENABLE_MPU=1` branch of `GCC/ARM_CM55_NTZ/non_secure`: MAIR0, per-context RBAR/RLAR pairs, a 20-active-word protected image plus one-past cursor, exact cohort identity, sealed unprivileged construction, linker-boundary validation, a four-region MPU image, all eight forward operations, strict first-start SVC, public SVC-71 yield, and protected PendSV. PendSV preflights the running frame before cursor access, saves the complete basic frame to privileged storage, selects under BASEPRI via reverse ABI v1, replaces MAIR0/context pairs with PRIMASK asserted, publishes current while MPU access is disabled, then restores the inverse image. `-O2`/`-Os` parity plus normal/LTO archive, external cohort, stale 8/16 archive, section-isolation, vector-slot, and duplicate-SVC/PendSV proofs pass. It remains outside global auto/profile selection and exposes no heterogeneous-MPU API or hardware claim. M55-only RLAR.PXN is retained for a later configurable-region ABI. | Validate M55 MPU runtime and isolation. Keep FPU, MVE, TrustZone/SecureContext, TF-M, PAC, and BTI separate. |
| Cortex-M55 feature cohorts / STM32N6 | Scalar, scalar-FP, MVE-FP, no-feature MPU, scalar-FP MPU `C55P`, and MVE-FP MPU `C55W` are complete explicit build-selected software cohorts. C55W now has all eight forward operations, public SVC-71 yield, split-handler archive extraction, and full compile/assembly/archive/ELF evidence while retaining no VPR state. `transitional_v8m` remains only for unported v8-M roles. | Complete TrustZone/SecureContext or TF-M, and PAC/BTI cohorts, then validate matching hardware. |
