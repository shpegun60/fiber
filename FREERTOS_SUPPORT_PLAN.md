...
| Cortex-M4F/M7F MPU | Non-selectable `port/ARM_CM4_MPU` slices 1-4 freeze the pinned FreeRTOS 53-word protected FP context, exact 8/16-region MPU construction, strong fail-closed SVC/PendSV, unprivileged yield/return veneers, dynamic basic/extended FP save/restore, exact MPU replacement/readback, CPACR/FPCCR policy, scheduler BASEPRI envelope, and M7 errata-safe BASEPRI writes; full forward ABI selection is absent | Complete the eight-function ABI/build selection, then establish separate M4F/M7F hardware claims |
