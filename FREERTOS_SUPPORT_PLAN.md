...
| Cortex-M33 MPU | `ARM_CM33_MPU/non_secure` slices 1-4 freeze the explicit build-selected `GCC/ARM_CM33_NTZ/non_secure` no-FPU/no-TrustZone 8- or 16-region cohort, sealed unprivileged construction, exact RBAR/RLAR/MAIR0/global-image/linker policy, protected SVC first start, and private protected PendSV. PendSV preflights the running image before cursor access, copies the complete basic frame into privileged storage, invokes reverse-ABI scheduler selection under BASEPRI, replaces MAIR0/context pairs with PRIMASK asserted, then restores `PSP`/`PSPLIM`/`CONTROL` and the selected protected frame. SVC 71 is an exact private yield veneer that only pends PendSV. `-O2`/`-Os` parity, LTO, synthetic vector slots 11/14, and duplicate-SVC/PendSV negative links pass. Forward runtime ABI, selector routing, public MPU API, and hardware support remain unclaimed. | Activate the frozen forward ABI and add archive/ELF/cohort proof, then validate MPU isolation on hardware. Keep FPU, SecureContext, TF-M, PAC/BTI, and MVE as separate cohorts. |
