...
| Cortex-M3 MPU | Exact reference audit plus slices 1-4 and 6 are complete: type/layout, MPU construction, linker isolation, protected SVC/PendSV, all eight forward ABI operations, portable-application archive extraction, exact MPU section placement, cohort, section-GC, normal/LTO, vector, and duplicate-handler proofs pass; no selector route or runtime claim exists yet | Enable build selection as compile-covered only, then run the complete hardware suite |
