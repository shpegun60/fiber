...
| Cortex-M55 MPU | Non-selectable `ARM_CM55_MPU/non_secure` slice 4 freezes the no-FPU/no-MVE/no-TrustZone/no-PAC `C55M` protected 8/16-region layout from the `configENABLE_MPU=1` branch of `GCC/ARM_CM55_NTZ/non_secure`: MAIR0, per-context RBAR/RLAR pairs, a 20-active-word protected image plus one-past cursor, exact cohort identity, sealed unprivileged construction, linker-boundary validation, a four-region MPU image, strict first-start SVC, and private protected PendSV. PendSV preflights the running frame before cursor access, saves the complete basic frame to privileged storage, selects under BASEPRI via reverse ABI v1, replaces MAIR0/context pairs with PRIMASK asserted, then restores the inverse image. `-O2`/`-Os` parity plus normal/LTO ELFs prove section isolation, slots 11/14, and duplicate-SVC/PendSV failure. The forward ABI remains absent. M55-only RLAR.PXN is retained for a later configurable-region ABI. | Activate the eight forward operations and add runtime archive/cohort proof, then validate M55 MPU isolation. Keep FPU, MVE, TrustZone/SecureContext, TF-M, PAC, and BTI separate. |
| Cortex-M55 feature cohorts / STM32N6 | Scalar, scalar-FP, and MVE-FP non-MPU runtimes are complete software cohorts; the no-feature MPU cohort now has the staged protected SVC/PendSV engine and remains pending forward-ABI/archive activation. `transitional_v8m` remains only for unported v8-M roles. | Complete M55 MPU runtime activation, TrustZone/SecureContext or TF-M, and PAC/BTI cohorts, then validate matching hardware. |
