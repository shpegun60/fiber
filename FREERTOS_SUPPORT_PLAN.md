...
| Cortex-M33 | Exact build-selected `ARM_CM33_NTZ/non_secure` slices 1-2 freeze the pinned FreeRTOS Mainline NTZ non-MPU/no-FPU ten-word `[PSPLIM][EXC_RETURN][r4-r11]` layout, `0xFFFFFFBC` Non-secure return, BASEPRI/FAULTMASK/PSPLIM traits, cohort identity, sealed boot record, and validated initial context construction. It intentionally has no SVC/PendSV/scheduler runtime, archive, or hardware claim. | Add SVC first start, PendSV PSPLIM save/restore, archive/ELF proof, then hardware validation; keep MPU, SecureContext, TF-M, and M33F as separate profiles. |
