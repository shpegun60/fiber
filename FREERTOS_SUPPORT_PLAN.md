...
| Cortex-M33 MPU | `ARM_CM33_MPU/non_secure` slice 1 freezes an explicit build-selected `GCC/ARM_CM33_NTZ/non_secure` no-FPU/no-TrustZone 8- or 16-region storage and trait cohort: MAIR0, RBAR/RLAR pairs, 20 active protected words, final one-past cursor target, CONTROL/PSPLIM, MPU feature identity, and exact cohort spelling. It has no constructor, MPU register write, SVC/PendSV, forward runtime ABI, or hardware claim. | Add sealed construction and linker/global MPU image, then generated SVC/PendSV assembly parity, archive/ELF/cohort proof, and hardware MPU-isolation validation. Keep FPU, SecureContext, TF-M, PAC/BTI, and MVE as separate cohorts. |
