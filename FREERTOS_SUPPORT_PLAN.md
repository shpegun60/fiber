...
| Cortex-M55F MPU | Explicit build-selected `ARM_CM55F_MPU/non_secure` slice 1 freezes the separate scalar-FP/no-MVE/no-TrustZone/no-PAC `C55P` 8/16-region protected construction cohort from the `configENABLE_MPU=1`, `configENABLE_FPU=1` branch of `GCC/ARM_CM55_NTZ/non_secure`. It owns the exact overlapping 54-word protected backing store: basic first-start data uses words 0-20 while later extended FP data uses words 0-53 of the same array. It also owns MAIR0, per-context RBAR/RLAR storage, sealed construction, linker boundaries, exact cohort identity, and CPACR/FPCCR policy helpers. Hard/softfp `-O2`/`-Os` constructor parity and construction/linker/cohort negative proofs pass. It intentionally exports no forward runtime ABI or handlers yet. | Add strict protected scalar-FP SVC, FP-aware protected PendSV, the eight-operation runtime/archive/vector proof, then validate M55F MPU runtime and isolation. Keep MVE, TrustZone/SecureContext, TF-M, PAC, and BTI separate. |
...
| Cortex-M55 feature cohorts / STM32N6 | Scalar, scalar-FP, MVE-FP, and no-feature MPU are complete explicit build-selected software cohorts. Scalar-FP MPU `C55P` has only its construction contract frozen. `transitional_v8m` remains only for unported v8-M roles. | Complete C55P protected SVC/PendSV/runtime proof, M55 MPU MVE-FP protected variants, TrustZone/SecureContext or TF-M, and PAC/BTI cohorts, then validate matching hardware. |
