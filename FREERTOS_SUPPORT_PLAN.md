...
| Cortex-M33 MPU | `ARM_CM33_MPU/non_secure` slices 1-2 freeze the explicit build-selected `GCC/ARM_CM33_NTZ/non_secure` no-FPU/no-TrustZone 8- or 16-region cohort, then add sealed unprivileged context construction, exact 32-byte RBAR/RLAR encoding, MAIR0, four in-memory global MPU pairs, and a linker isolation contract. The current slot stays inside privileged SRAM; hardware region numbers are explicitly distinct from per-context pair indexes. `-O2`/`-Os` synthetic linker proofs pass without SVC, PendSV, MPU register writes, forward runtime ABI, or hardware claim. | Add protected SVC first start plus MPU activation/readback, then PendSV generated-assembly parity, archive/ELF/cohort proof, and hardware MPU-isolation validation. Keep FPU, SecureContext, TF-M, PAC/BTI, and MVE as separate cohorts. |
