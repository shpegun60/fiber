...
| Cortex-M4F/M7F MPU | Non-selectable `port/ARM_CM4_MPU` slices 1-3 freeze the pinned FreeRTOS 53-word protected FP context, exact 8/16-region MPU construction, strong fail-closed SVC first-start, unprivileged yield/return veneers, first MPU activation/readback, CPACR/FPCCR setup, and M7 errata-safe BASEPRI clear; no PendSV or full runtime integration exists | Implement protected FP-aware PendSV, complete the eight-function ABI/build selection, then establish separate M4F/M7F hardware claims |
