...
| Cortex-M33 TrustZone/SecureContext | Build-selected `ARM_CM33/non_secure` slice 1 freezes the distinct no-MPU/no-FPU companion-aware frame from `GCC/ARM_CM33/non_secure`: `[xSecureContext][PSPLIM][EXC_RETURN][r4-r11]`, a sealed pre-start `secure_stack_bytes` request, and a distinct exact cohort. C/C++ type, manifest, cohort, and negative-manifest proofs pass; it exposes no runtime, attach API, gateway, handler, or hardware claim. | Add the optional pre-publication lifecycle ABI, matched Secure companion and cross-image version proof, then FreeRTOS-shaped SVC/PendSV SecureContext mechanics and hardware validation. |
